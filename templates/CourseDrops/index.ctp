									...
									<tr>
										...
										<td class="center">
											...
											if ($courseDrop['CourseDrop']['department_approval'] == 1) {
												...
												if (!$courseDrop['CourseDrop']['forced']) {
													...
													/* if (isset($courseDrop['CourseRegistration']['PublishedCourse']['Course']['id']) && $this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT && isset($courseDrop['CourseRegistration']['PublishedCourse']['id']) && ClassRegistry::init('ExamGrade')->isGradeSubmittedForPublishedCourse($courseDrop['CourseRegistration']['PublishedCourse']['id']) == 0) {
														echo '<br>'. $this->Form->postLink(__('[Approve Anyway]'), array('action' => 'approve_auto_rejected_course_add', $courseDrop['CourseDrop']['id']), array('confirm' => __('Are you sure you want to cancel the auto rejected Course Drop of %s for %s? Cancelling this auto rejection will auto ' . ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT ? ' approve' : ' comfirm') . ' the Course Drop Request. Are you sure you want cancel the auto rejection anyway?', $courseDrop['Student']['full_name'] . ' ('.  $courseDrop['Student']['studentnumber'] . ')', $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_title'] . ' (' .  $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_code']. ') course  with ' . $courseDrop['CourseRegistration']['PublishedCourse']['Course']['credit']. ' ' . (count(explode('ECTS', $courseDrop['CourseRegistration']['PublishedCourse']['Course']['Curriculum']['type_credit'])) >= 2  ? 'ECTS' : 'Credit'))));
													} */
												}
											} ?>
										</td>
										<td class="center">
											...
											if (!$courseDrop['CourseDrop']['forced']) {
												...
												if (isset($courseDrop['CourseRegistration']['PublishedCourse']['Course']['id']) && $this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && isset($courseDrop['CourseRegistration']['PublishedCourse']['id']) && ClassRegistry::init('ExamGrade')->isGradeSubmittedForPublishedCourse($courseDrop['CourseRegistration']['PublishedCourse']['id']) == 0) {
													echo '<br>'. $this->Form->postLink(__('[ Cancel Course Drop ]'), array('action' => 'delete', $courseDrop['CourseDrop']['id']), array('confirm' => __('Are you sure you want to cancel the Course Drop of %s for %s? Cancelling this Course Drop will make the student available for course assigned instructor for grade submission. Are you sure you want cancel the Course Drop anyway?', $courseDrop['Student']['full_name'] . ' ('.  $courseDrop['Student']['studentnumber'] . ')', $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_title'] . ' (' .  $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_code']. ') course  with ' . $courseDrop['CourseRegistration']['PublishedCourse']['Course']['credit']. ' ' . (count(explode('ECTS', $courseDrop['CourseRegistration']['PublishedCourse']['Course']['Curriculum']['type_credit'])) >= 2  ? 'ECTS' : 'Credit'))));
												}
											} ?>
										</td>
										<?php
										if ($this->Session->read('Auth.User')['role_id'] == ROLE_STUDENT) { ?>
											<td class="center"><?= (empty($courseDrop['CourseDrop']['department_approval']) && !($courseDrop['CourseDrop']['forced']) ? $this->Form->postLink(__('[ Cancel Drop ]'), array('action' => 'delete', $courseDrop['CourseDrop']['id']), array('confirm' => __('Are you sure you want to cancel the Course Drop for %s, you requested previously? Cancelling this Course Drop will allow you to continue with the course and your will be available for course assigned instructor for grade submission. Are you sure you want cancel the Course Drop anyway?', (isset($courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_title']) ? $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_title'] . ' (' .  $courseDrop['CourseRegistration']['PublishedCourse']['Course']['course_code']. ') course with ' . $courseDrop['CourseRegistration']['PublishedCourse']['Course']['credit']. ' ' . (count(explode('ECTS', $courseDrop['CourseRegistration']['PublishedCourse']['Course']['Curriculum']['type_credit'])) >= 2  ? 'ECTS' : 'Credit') : '')))) : ''); ?></td>
											<?php
										} ?>
									</tr>
									<tr id="c<?= $count++; ?>" style="display:none">
										...
										<td colspan=<?= $this->Session->read('Auth.User')['role_id'] == ROLE_STUDENT ? "11" : "10"; ?> style="background-color: white;">
											...
											if (isset($courseDrop['CourseRegistration']['PublishedCourse']['Course']) && !empty($courseDrop['CourseRegistration']['PublishedCourse']['Course']['id'])) { ?>
												...
												<?php
												if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR) {
													echo '<br>'. $this->Form->postLink(__('[ Cancel Course Drop ]'), array('action' => 'delete', $courseDrop['CourseDrop']['id']), array('confirm' => __('Are you sure you want to cancel the Course Drop of %s? Cancelling this Course Drop will make the student available for course assigned instructor for grade submission. Are you sure you want cancel the Course Drop anyway?', $courseDrop['Student']['full_name'] . ' ('.  $courseDrop['Student']['studentnumber'] . ')')));
												}  
											} ?>
										</td>
									</tr>
