<?php
	...
	$no_of_semester_passed =  $no_of_semester;
	...
	foreach ($student_copies as $k => $student_copy) {
        ...
        $pdf->SetTitle('Student Copy for ' .$student_copy['student_detail']['Student']['full_name'] .' (' .$student_copy['student_detail']['Student']['studentnumber'].')');
        $pdf->SetSubject($student_copy['student_detail']['Student']['studentnumber']);
        $pdf->SetKeywords('Student, Copy, '.$student_copy['student_detail']['Student']['studentnumber'].', SMiS');
																...
																<tr>
																	<td>
																		<table style="padding:' . $course_justification . 'px; margin:0px; background-image:url(\'/img/sc-diagonal-line.gif\'); background-position:left top; background-attachment:scroll; background-repeat:no-repeat">
																			...
																			if (isset($student_copy['student_detail']['ExemptionList']) && !empty($student_copy['student_detail']['ExemptionList']) && $pdf->getPage() == 1) {
																				$student_copy_content .= '
																				<tr>
																					<td></td>
																					<td></td>
																			...
																			if (isset($student_copy['student_detail']['ExemptionList']) && !empty($student_copy['student_detail']['ExemptionList']) && $pdf->getPage() == 1) {
																				...
																				/* if (count($student_copy['student_detail']['ExemptionList']) > 10 && $pdf->getPage() == 1) {
																					$no_of_semester = 1;
																				} else {
																					$no_of_semester = 2;
																				}

																				if (count($student_copy['student_detail']['ExemptionList']) > 15 && $pdf->getPage() == 1) {
																					$no_of_semester_passed = 1;
																				} else if (count($student_copy['student_detail']['ExemptionList']) <= 15 && $pdf->getPage() == 1) {
																					$no_of_semester_passed = 1;
																				} else if ($pdf->getPage() != 1) {
																					$no_of_semester = 1;
																					//$no_of_semester = $no_of_semester_passed;
																				} */

																				//$no_of_semester = 2;

																				// Added to correct missing semesters while managing exempted courses. 
																				//for loop variables are bit modified in the first page of the firt section (half of the page), the other parts of the student copy except the first block, continue as normal.
																				// this modification affects only transfered students from other universities.
																				
																				//for regular students the original variable are intact or continues in the else part.

																				for ($j = $i * 2; $j < (($i * 2) + ($no_of_semester_passed)) && $j < count($student_copy['courses_taken']); $j++) {
																				...
																				// End Added to correct missing semesters while managing exempted courses.
																			} else {
	...
	$pdf->Output('Student_Copy_'.str_replace('/','-',$student_copy['student_detail']['Student']['studentnumber']).'_'.$student_copy['student_detail']['Student']['first_name'].'_'.$student_copy['student_detail']['Student']['middle_name'].'_'.$student_copy['student_detail']['Student']['last_name'].'_'.date('Y-m-d').'.pdf', 'I');
