	...
	<div class="box-body">
		<div class="row">
			<div class="large-12 columns">
				...
				<?= $this->Form->create('ColleagueEvalutionRate', array('onSubmit' => 'return checkForm(this);')); ?>
				...
				<div class="examGrades ">
					...
					<div id="ListSection" style="display:<?= (!empty($colleagueLists) ? 'none' : 'display'); ?>">
					...
					if (isset($this->request->data['getInstructorList']) && !empty($this->request->data['Search']['name']) && empty($colleagueLists)) { ?>
						<!-- <div class='info-box info-message' style="font-family: 'Times New Roman', Times, serif; font-weight: bold;"><span style='margin-right: 15px;'></span>No staff is found with name starting with <b><i>"<?php //echo $this->request->data['Search']['name']; ?>"</i></b> who is assigned for a course for <?php //echo ($this->request->data['Search']['semester'] == 'I' ? '1st' : ($this->request->data['Search']['semester'] == 'II' ? '2nd' : '3rd')) . '  semester of ' . $this->request->data['Search']['acadamic_year']; ?> academic year or you already evaluated him/her previously. </div> -->
						<?php
					} else if (empty($colleagueLists) && isset($this->request->data['getInstructorList'])) { ?>
						<!-- <div class='info-box info-message' style="font-family: 'Times New Roman', Times, serif; font-weight: bold;"><span style='margin-right: 15px;'></span>No instructor found to evaluate for now. Either all instructors have been evaluated by you, or academic calendar for staff evaluation is not opened/deadline passed for <i><u><?php //echo ($this->request->data['Search']['semester'] == 'I' ? '1st' : ($this->request->data['Search']['semester'] == 'II' ? '2nd' : '3rd')) . '  semester of ' . $this->request->data['Search']['acadamic_year']; ?></u></i> academic year. </div> -->
						<?php
					}
								...
								<tbody>
									...
									foreach ($instructorEvalutionQuestionsObjective as $kc => $vc) { ?>
										<tr>
											<td>
												<?= $count; ?> 
												<?= $this->Form->hidden('ColleagueEvalutionRate.' . $count . '.instructor_evalution_question_id', array('label' => false, 'size' => 4, 'div' => false, 'value' => $vc['InstructorEvalutionQuestion']['id'])); ?>
											</td>
											...
											<td class="vcenter"><p style="padding:2%;"><?= $this->Form->radio('ColleagueEvalutionRate.' . $count . '.rating', $options, $attributes); ?></p></td>
										</tr>
										<?php
										$count++;
									} ?>
