...
class FxResitRequestTable extends Table
{
    ...
    public function doesStudentAppliedFxSit($course_reg_add_id, $register = 1)
    {

        if ($register == 1) {
            //find registration details
            $registration = $this->CourseRegistration->find('first', array(
                'conditions' => array('CourseRegistration.id' => $course_reg_add_id),
                'recursive' => -1
            ));
            $regListRegistered = $this->CourseRegistration->find(
                'list',
                array(
                    'conditions' => array(
            ...
            $applied = $this->find(
                'first',
                array(
                    'conditions' => array('FxResitRequest.course_registration_id' => $regListRegistered),
                    'recursive' => -1
                )
            );
            ...
            $courseAdds = $this->CourseAdd->find('first', array(
                'conditions' => array('CourseAdd.id' => $course_reg_add_id),
                'recursive' => -1
            ));
            $regListAdded = $this->CourseAdd->find(
                'list',
                array(
                    'conditions' => array(
            ...
            $applied = $this->find(
                'first',
                array(
                    'conditions' => array('FxResitRequest.course_add_id' => $regListAdded),
                    'recursive' => -1
                )
            );
            if (isset($applied) && !empty($applied)) {
    ...
    public function fxresetId($course_reg_add_id, $register = 1)
    {

        if ($register == 1) {
            $applied = $this->find('first', array(
                'conditions' => array('FxResitRequest.course_registration_id' => $course_reg_add_id),
                'recursive' => -1
            ));
            debug($applied);
            ...
            $applied = $this->find(
                'first',
                array(
                    'conditions' => array('FxResitRequest.course_add_id' => $course_reg_add_id),
                    'recursive' => -1
                )
            );
            return $applied['FxResitRequest']['id'];
    ...
    public function publishedCourseSelected($published_course_id)
    {
        ...
        $applied = $this->find(
            'count',
            array(
                'conditions' => array('FxResitRequest.published_course_id' => $published_course_id),
                'recursive' => -1
            )
        );
        return $applied;
    ...
    public function doesFxAppliedandQuotaUsed($student_id, $academic_year)
    {
        ...
        $applicationCountRecentACS = 0;
        if (
            isset($mostRecentRegistrationAC)
            && !empty($mostRecentRegistrationAC)
        ) {
            $registrationsList = $this->CourseRegistration->find('list', array(
                'conditions' => array(
                    'CourseRegistration.academic_year' => $mostRecentRegistrationAC['academic_year'],
            ...
            if (isset($registrationsList) && !empty($registrationsList)) {
                $applicationCountRecentACS += $this->find(
                    'count',
                    array(
                        'conditions' => array('FxResitRequest.course_registration_id' => $registrationsList),
                        'recursive' => -1
                    )
                );
            }

            $addList = $this->CourseAdd->find('list', array(
                'conditions' => array(
                    'CourseAdd.academic_year' => $mostRecentRegistrationAC['academic_year'],
            ...
            if (isset($addList) && !empty($addList)) {
                $applicationCountRecentACS += $this->find('count', array(
                    'conditions' => array('FxResitRequest.course_add_id' => $addList),
                    'recursive' => -1
                ));
                debug($addList);
        ...
        if ($applicationCountRecentACS > 0 && $applicationCountRecentACS < 3) {
        ...
        } elseif ($applicationCountRecentACS == 3 || $this->find(
                'count',
                array('conditions' => array('FxResitRequest.student_id' => $student_id))
            ) == 3) {
            //check
            return 3; // quota of fx full, dont allow
        } else {
            $applicationCountRecentACS = 0;
            $recent_add = $this->CourseAdd->find('first', array(
                'conditions' => array(
                    'CourseAdd.student_id' => $student_id,
                    'CourseAdd.department_approval' => 1,
                    'CourseAdd.registrar_confirmation' => 1,
            ...
            if (
                ...
                $addList = $this->CourseAdd->find('list', array(
                    'conditions' => array(
                        'CourseAdd.academic_year' => $recent_add['CourseAdd']['academic_year'],
                ...
                if (isset($addList) && !empty($addList)) {
                    $applicationCountRecentACS += $this->find(
                        'count',
                        array('conditions' => array('FxResitRequest.course_add_id' => $addList), 'recursive' => -1)
                    );
                }
            }
            $recent_registration =
                $this->CourseRegistration->find(
                    'first',
                    array(
                        'conditions' => array(
                            'CourseRegistration.student_id' => $student_id
            ...
            if (isset($recent_registration) && !empty($recent_registration)) {
                $registrationsList = $this->CourseRegistration->find('list', array(
                    'conditions' => array(
                        'CourseRegistration.academic_year' => $recent_registration['CourseRegistration']['academic_year'],
            ...
            } elseif ($applicationCountRecentACS > 0) {
                return 2;
            }
        }
        return 2;
    ...
    public function getLastestStudentSemesterAndAcademicYearForFx($student_id = null)
    {

        $academicYearSem = array();
        $regFirst = $this->CourseRegistration->find('first', array(
            'conditions' => array(
                'CourseRegistration.student_id' => $student_id,
        ...
        debug($regFirst);
        $addFirst = $this->CourseAdd->find('first', array(
            'conditions' => array(
                'CourseAdd.student_id' => $student_id,
        ...
        } elseif (isset($regFirst) && !empty($regFirst)) {
            $academicYearSem['academic_year'] = $regFirst['CourseRegistration']['academic_year'];
            $academicYearSem['semester'] = $regFirst['CourseRegistration']['semester'];
        } elseif (isset($addFirst) && !empty($addFirst)) {
            $academicYearSem['academic_year'] = $addFirst['CourseAdd']['academic_year'];
            $academicYearSem['semester'] = $addFirst['CourseAdd']['semester'];
        }
    ...
    public function makeUpExamApplied($student_id, $published_course_id, $reg_add_id, $reg = 0)
    {
        ...
        if (
            isset($return['FxResitRequest']['id'])
