...
class StaffStudiesTable extends Table
{
    ...
    public function preparedAttachment($data = null, $group = null)
    {
    ...
    public function getStaffCompletedHDPStatistics($acadamic_year = null, $department_id = null, $sex = 'all')
    {
        ...
        if (isset($department_id) && !empty($department_id)) {
            ...
            if (count($college_id) > 1) {
                ...
                $departments = $this->Staff->Department->find('all', array(
                    'conditions' => array(
        ...
        $acadamicYearBegDate = $AcademicYear->getAcademicYearBegainingDate($acadamic_year);
            ...
            foreach ($sexList as $skey => $svalue) {
                foreach ($completed as $ckey => $cvalue) {
                    $check = $this->find('all', array(
                        'conditions' => array(
                            'StaffStudy.education' => 'HDP',
                            'StaffStudy.study_completed' => $ckey,
                    ...
                    debug($check);
                    if (!empty($check)) {
                        $hdpTrainningStatistics[$value['College']['name']][$value['Department']['name']][$ckey][$skey] = $this->find(
                            'count',
                            array(
                                'conditions' => array(
                                    'StaffStudy.education' => 'HDP',
