...
class StudentsTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ...
            ->notEmptyString('studentnumber', 'Student ID Number is required')
            ->add('studentnumber', 'unique', [
                'rule' => 'validateUnique',
                'provider' => 'table',
                'message' => 'This student number is already taken.'
            ])
            ->notEmptyString('phone_mobile', 'Please enter mobile phone number')
            ->add('phone_mobile', 'custom', [
                'rule' => ['custom', '/^\+251\d{9}$/'],
                'message' => 'Phone number must be in +251999999999 format.'
            ])
            ->notEmptyString('region_id', 'Region is required')
            ->notEmptyString('zone_id', 'Zone is required')
            ->notEmptyString('woreda_id', 'Woreda is required');
