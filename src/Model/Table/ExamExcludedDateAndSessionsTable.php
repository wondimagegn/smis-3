...
class ExamExcludedDateAndSessionsTable extends Table
{
    ...
    public function count_ExamExcludedDateAndSessions_in_this_ExamPeriod($id = null)
    {
    ...
    public function count_ExamExcludedDateAndSessions_out_side_edited_ExamPeriod(
        $id = null,
        $start_date = null,
        $end_date = null
    ) {
        ...
        $count = $this->find(
            ...
            array(
                'conditions' => array(
                    'ExamExcludedDateAndSession.exam_period_id' => $id,
                    "OR" => array(
                        'ExamExcludedDateAndSession.excluded_date <' => $start_date,
                        'ExamExcludedDateAndSession.excluded_date >' => $end_date
                    )
                )
            )
        );
    ...
    public function get_list_of_exam_period_dates($exam_period_id = null)
    {

        $examPeriods = $this->ExamPeriod->find(
            'first',
            array('conditions' => array('ExamPeriod.id' => $exam_period_id), 'recursive' => -1)
        );
    ...
    public function get_already_excluded_date_and_session($exam_period_id = null)
    {

        if (!empty($exam_period_id)) {
            $already_excluded_date_and_session_array = array();
            $examExcludedDateAndSessions = $this->find(
                'all',
                array(
                    'conditions' => array('ExamExcludedDateAndSession.exam_period_id' => $exam_period_id),
                    'order' => array('ExamExcludedDateAndSession.excluded_date', 'ExamExcludedDateAndSession.session')
                )
            );
