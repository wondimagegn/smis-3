...
class MealHallAssignmentsTable extends Table
{
    ...
    public function get_maximum_year_levels_of_college($college_id = null)
    {
        ...
        foreach ($departments as $department_id) {
            $yearLevel_count_latest = $this->Student->Department->YearLevel->find(
                'count',
        ...
        if (!empty($largest_yearLevel_department_id)) {
            $yearLevels = $this->Student->Department->YearLevel->find(
                'list',
                array(
    ...
    public function get_student_have_mealhall($academicyear = null)
    {

        $student_ids = $this->find(
            'list',
            array(
                'fields' => array('MealHallAssignment.student_id'),
                'conditions' => array(
                    'MealHallAssignment.academic_year' => $academicyear,
    ...
    public function get_accepted_student_have_mealhall($academicyear = null)
    {

        $accepted_student_ids = $this->find(
            'list',
            array(
                'fields' => array('MealHallAssignment.accepted_student_id'),
                'conditions' => array(
                    'MealHallAssignment.academic_year' => $academicyear,
    ...
    public function get_formatted_mealhall($academic_year = null)
    {

        $mealHalls = $this->MealHall->find(
            'all',
            array('contain' => array('Campus' => array('fields' => array('Campus.name'))))
        );
        $formatted_mealhalls = array();
        foreach ($mealHalls as $mealHall) {
            $formatted_mealhalls[$mealHall['Campus']['name']][$mealHall['MealHall']['id']] = $mealHall['MealHall']['name'] . ' (Currently has:' . $this->get_mealhall_assigned_students_count(
                    $academic_year,
                    $mealHall['MealHall']['id']
                ) . ' students)';
        }
    ...
    public function get_mealhall_assigned_students_count($academic_year = null, $meal_hall_id = null)
    {
    ...
    public function get_formatted_mealhall_for_view()
    {
    ...
    public function is_meal_hall_ever_used($meal_hall_id = null)
    {
        ...
        if (!empty($meal_hall_id)) {
            ...
            $count = $this->find(
                'count',
                array('conditions' => array('MealHallAssignment.meal_hall_id' => $meal_hall_id), 'limit' => 2)
            );
            if ($count == 0) {
                return false;
            } else {
