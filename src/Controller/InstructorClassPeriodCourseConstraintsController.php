...
class InstructorClassPeriodCourseConstraintsController extends AppController
{
    ...
    public $name = 'InstructorClassPeriodCourseConstraints';
    ...
    public function index()
    {
        ...
        $this->paginate = array(
            'conditions' => array('InstructorClassPeriodCourseConstraint.college_id' => $this->college_id),
            'contain' => array(
                'Staff' => array(
                    'fields' => array(
                        'Staff.id',
    ...
    public function view($id = null)
    {
        ...
        $this->set(
            'instructorClassPeriodCourseConstraint',
            $this->InstructorClassPeriodCourseConstraint->read(null, $id)
        );
    }
    ...
    public function add()
    {
        ...
        $programTypes = $this->InstructorClassPeriodCourseConstraint->College->PublishedCourse->ProgramType->find(
            'list'
        );
        $departments = $this->InstructorClassPeriodCourseConstraint->College->Department->find(
            'list',
            array('conditions' => array('Department.college_id' => $this->college_id))
        );
        $yearLevels = null;
        $yearLevels = $this->InstructorClassPeriodCourseConstraint->College->PublishedCourse->YearLevel->find(
            'list',
        ...
        if (!empty($this->request->data) && isset($this->request->data['search'])) {
            ...
            switch ($this->request->data) {
                case empty($this->request->data['InstructorClassPeriodCourseConstraint']['academicyear']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the academic year that you want to add instructor class period course constraints. '
                        ),
                        'default',
            ...
            if ($everythingfine) {
                ...
                $publishedCourses_id_array = $this->InstructorClassPeriodCourseConstraint->College->PublishedCourse->find(
                    'list',
                    array('fields' => array('PublishedCourse.id'), 'conditions' => $conditions)
                );

                $instructors = $this->InstructorClassPeriodCourseConstraint->Staff->CourseInstructorAssignment->find(
                    'all',
                    array(
                        'conditions' => array('CourseInstructorAssignment.published_course_id' => $publishedCourses_id_array),
                        'fields' => array('DISTINCT CourseInstructorAssignment.staff_id'),
                        'contain' => array(
                            'Staff' => array(
                                'fields' => array(
                                    'Staff.id',
                ...
                if (empty($instructors)) {
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'There is no Instructor to add class period constraints in the selected criteria.'
                        ),
                        'default',
                        array('class' => 'info-box info-message')
                    );
                } else {
                    $week_days = $this->InstructorClassPeriodCourseConstraint->ClassPeriod->find('all', array(
        ...
        if (!empty($this->request->data) && isset($this->request->data['submit'])) {
            if (!empty($this->request->data['InstructorClassPeriodCourseConstraint']['instructor'])) {
                if (!empty($this->request->data['InstructorClassPeriodCourseConstraint']['week_day'])) {
                    ...
                    if (!empty($this->request->data['InstructorClassPeriodCourseConstraint']['Selected'])) {
                        ...
                        if (count($selected_class_period_id_array) != 0) {
                            ...
                            $isalready_record_this_constraints = $this->InstructorClassPeriodCourseConstraint->find(
                                'count',
                                array(
                                    'conditions' => array(
                                        'InstructorClassPeriodCourseConstraint.staff_id' => $this->request->data['InstructorClassPeriodCourseConstraint']['instructor'],
                                        'InstructorClassPeriodCourseConstraint.class_period_id' => $selected_class_period_id_array,
                                        'InstructorClassPeriodCourseConstraint.college_id' => $this->college_id,
                                        'InstructorClassPeriodCourseConstraint.academic_year' => $this->request->data['InstructorClassPeriodCourseConstraint']['academicyear'],
                                        'InstructorClassPeriodCourseConstraint.semester' => $this->request->data['InstructorClassPeriodCourseConstraint']['semester']
                                    )
                                )
                            );
                            if ($isalready_record_this_constraints == 0) {
                                ...
                                foreach ($selected_class_period_id_array as $class_period_id) {
                                    ...
                                    $this->InstructorClassPeriodCourseConstraint->save(
                                        $this->request->data['InstructorClassPeriodCourseConstraints']
                                    );
                                }
                                $this->Session->setFlash(
                                    '<span></span>' . __(
                                        'The instructor class period course constraint has been saved'
                                    ),
                                    'default',
                                ...
                                $this->Session->setFlash(
                                    '<span></span>' . __(
                                        'The Instructor class period constraint for the one or more seclected period is already recored, Please exclude already recored periods.'
                                    ),
                                    'default',
                                    array('class' => 'error-box error-message')
                                );
                ...
                $this->Session->setFlash(
                    '<span></span>' . __(' Please select instructor'),
                    'default',
                    array('class' => 'error-box error-message')
                );
            }
        ...
        $conditions = array(
            'InstructorClassPeriodCourseConstraint.academic_year' => $selected_academicyear,
            'InstructorClassPeriodCourseConstraint.college_id' => $this->college_id,
            'InstructorClassPeriodCourseConstraint.semester' => $selected_semester
        );

        $instructorClassPeriodCourseConstraints = $this->InstructorClassPeriodCourseConstraint->find('all', array(
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($this->request->data)) {
            if ($this->InstructorClassPeriodCourseConstraint->save($this->request->data)) {
                ...
                $this->Session->setFlash(
                    __('The instructor class period course constraint could not be saved. Please, try again.')
                );
            }
        }
        if (empty($this->request->data)) {
    ...
    public function delete($id = null, $from = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span> ' . __('Invalid id for instructor class period course constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
        ...
        $areyou_eligible_to_delete = $this->InstructorClassPeriodCourseConstraint->beforeDeleteCheckEligibility(
            $id,
            $this->college_id
        );
        if ($areyou_eligible_to_delete == true) {
            if ($this->InstructorClassPeriodCourseConstraint->delete($id)) {
                $this->Session->setFlash(
                    '<span></span> ' . __('Instructor class period course constraint deleted'),
                    'default',
                    array('class' => 'success-box success-message')
                );
                if (empty($from)) {
            ...
            $this->Session->setFlash(
                '<span></span> ' . __('Instructor class period course constraint was not deleted.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
            ...
            $this->Session->setFlash(
                '<span></span> ' . __('You are not eligible to delete this Instructor Class period course constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
    ...
    public function _get_periods_data($week_id = null, $program_id = null, $program_type_id = null)
    {
        ...
        if ($week_id) {
            $periods_data = $this->InstructorClassPeriodCourseConstraint->ClassPeriod->find(
                'all',
                array(
