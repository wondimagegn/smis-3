...
class CourseExemptionsController extends AppController
{
    ...
    public function list_exemption_request()
    {
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $duplicated = $this->CourseExemption->find(
                'count',
                array('conditions' => $this->request->data['CourseExemption'])
            );
            ...
            if ($duplicated == 0) {
                ...
                $this->Flash->warning(
                    'The course exemption could not be saved. You have already requested course exemptions for the selected courses.'
                );
                $this->redirect(array('action' => 'index'));
            }
        }
        ...
        $student_section_exam_status = $this->CourseExemption->Student->get_student_section(
            $this->student_id,
    ...
    public function delete($id = null)
    {
        ...
        if ($is_deletion_allowed > 0) {
            ...
            $this->Flash->error(
                __(
                    'Course exemption could not be cancelled. You request has been approved/rejected by your department.'
                )
            );
        }
    ...
    public function approve_request($id = null)
    {
            ...
            if ($elgibile_to_approve > 0) {
                if ($this->role_id == ROLE_DEPARTMENT) {
                    ...
                    if ($this->role_id == ROLE_REGISTRAR) {
                        $this->request->data['CourseExemption']['registrar_approve_by'] = $this->Auth->user(
                            'full_name'
                        );
                    }
                }
        ...
        $student_section_exam_status = $this->CourseExemption->Student->get_student_section(
            $this->request->data['CourseExemption']['student_id'],
            $current_academic_year
        );
    ...
    public function list_approved()
    {
        ...
        if ($this->role_id == ROLE_REGISTRAR) {
            if (!empty($this->department_ids)) {
                $departments = $this->CourseExemption->Student->Department->find(
                    'list',
    ...
    public function add_student_exempted_course($student_id)
    {
        ...
        $takenCourses = $this->CourseExemption->Student->getStudentRegisteredAddDropCurriculumResult(
            $student_id,
            null,
            /* $for_document =  */
            1,
            ...
            foreach ($takenCourses as $key => $courseRegAddExempt) {
                foreach ($courseRegAddExempt as $key => $course) {
                    if (isset($course['course_id']) && !empty($course['course_id'])) {
                        ...
                        $equivalentCourses = ClassRegistry::init('EquivalentCourse')->find(
                            'list',
                            array(
                                'conditions' => array('EquivalentCourse.course_be_substitued_id' => $course['course_id']),
                                'fields' => array(
                                    'EquivalentCourse.course_for_substitued_id',
                                    'EquivalentCourse.course_for_substitued_id'
                                )
                            )
                        );
        ...
        if (!empty($excludeCoursesList)) {
            ...
            $courses = $this->CourseExemption->Course->find(
                'list',
                array(
                    'conditions' => array('Course.curriculum_id' => $student_detail['Student']['curriculum_id']),
                    'fields' => array('id', 'course_title')
                )
            );
        }
        ...
        if (!empty($exemptedCourseLists)) {
            ...
            $coursesForList = $this->CourseExemption->Course->find(
                'list',
                array(
                    'conditions' => array(
                        'Course.curriculum_id' => $student_attached_curriculum_ids,
                        'Course.id' => $already_exempted_course_ids
                    ),
                    'fields' => array('id', 'course_title')
                )
            );
            ...
            $coursesForList = $this->CourseExemption->Course->find(
                'list',
                array(
                    'conditions' => array('Course.curriculum_id' => $student_attached_curriculum_ids),
                    'fields' => array('id', 'course_title')
                )
            );
        }
    ...
    public function add_student_exemption()
    {
            ...
            if (!empty($this->request->data['CourseExemption'])) {
                ...
                foreach ($this->request->data['CourseExemption'] as $k => $v) {
                    if (isset($student_id) && isset($v['course_id'])) {
                        ...
                        $formattedCourseExemption['CourseExemption'][$count]['department_approve_by'] = $this->Auth->user(
                            'full_name'
                        );
                        $formattedCourseExemption['CourseExemption'][$count]['registrar_approve_by'] = $this->Auth->user(
                            'full_name'
                        );
                ...
                if (!empty($formattedCourseExemption)) {
                    if ($this->CourseExemption->saveAll(
        ...
        $this->redirect(
            array(
                ...
                $this->request->data['CourseExemption'][0]['student_id']
            )
        );
    }
}
