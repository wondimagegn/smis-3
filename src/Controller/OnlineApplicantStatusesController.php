...
class OnlineApplicantStatusesController extends AppController
{
    ...
    public function index()
    {
        ...
        if (isset($this->passedArgs['OnlineApplicantStatus.request_to.year'])) {
            $this->paginate['conditions'][] = array(
                'OnlineApplicantStatus.created <= \'' . $this->passedArgs['OnlineApplicantStatus.request_to.year']
                . '-' . $this->passedArgs['OnlineApplicantStatus.request_to.month'] . '-' . $this->passedArgs['OnlineApplicantStatus.request_to.day'] . '\''
            );

            $this->paginate['conditions'][] = array(
                'OnlineApplicantStatus.created >= \'' . $this->passedArgs['OnlineApplicantStatus.request_from.year']
        ...
        if (empty($onlineApplicantStatuses) && isset($this->request->data) && !empty($this->request->data)) {
            $this->Session->setFlash(
                '<span></span>' . __('There is no online admission request status based on the given criteria.'),
                'default',
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->OnlineApplicantStatus->save($this->request->data)) {
                ...
                if ($this->request->data['OnlineApplicantStatus']['status'] == "Accepted") {
                    ...
                    if ($this->request->data['OnlineApplicantStatus']['status'] == "Rejected") {
                        $request_processed = -1;
                    } else {
                        if ($this->request->data['OnlineApplicantStatus']['status'] == 'Pending') {
                            $request_processed = 0;
                        }
                    }
                }
                ...
                $Email->subject(
                    'Online Admission Status Updated: ' . $applicantDetail['OnlineApplicant']['first_name'] . ' ' . $applicantDetail['OnlineApplicant']['father_name'] . ' for ' . $applicantDetail['OnlineApplicant']['academic_year'] . ' academic year'
                );
                ...
                try {
                    if ($Email->send()) {
                        $this->Session->setFlash(
                            '<span></span>' . __(
                                "Status updated and notification sent to " . $applicantDetail['OnlineApplicant']['email'] . " email address. "
                            ),
                            'default',
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __("Status updated but unable to send notification to . "),
                            'default',
                            array('class' => 'success-box success-message')
                        );
                    }
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __(
                            "Someting went wrong when sending notification  to " . $applicantDetail['OnlineApplicant']['email'] . " email address."
                        ),
                        'default',
                        array('class' => 'success-box success-message')
                    );
                ...
                debug($error);
                $this->Session->setFlash(
                    '<span></span>' . __('The online applicant status could not be saved. Please, try again.'),
                    'default',
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $options = array(
                'conditions' => array('OnlineApplicantStatus.' . $this->OfficialRequestStatus->primaryKey => $id),
                'contain' => 'OnlineApplicant'
            );
    ...
    public function delete($id = null)
    {
        ...
        if ($this->OfficialRequestStatus->delete()) {
            $this->Session->setFlash(
                '<span></span>' . __('The online applicant status  has been deleted.'),
                'default',
                array('class' => 'success-box success-message')
            );
        } else {
            $this->Session->setFlash(
                '<span></span>' . __('The online applicant status could not be deleted. Please, try again.'),
                'default',
