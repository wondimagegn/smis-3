...
class EquivalentCoursesController extends AppController
{
	...
	function add()
	{
		...
		if (!empty($this->request->data['EquivalentCourse']['course_be_substitued_id'])) {
			$curriculum_id = $this->EquivalentCourse->CourseBeSubstitued->field('curriculum_id', array('CourseBeSubstitued.id' => $this->request->data['EquivalentCourse']['course_be_substitued_id']));
