...
class StudentEvalutionRatesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($isInstructorEvaluated == 0) {
                ...
                if ($this->StudentEvalutionRate->saveAll($this->request->data['StudentEvalutionRate'])) {
                    if ($this->StudentEvalutionRate->InstructorEvalutionQuestion->StudentEvalutionComment->saveAll(
                        $this->request->data['StudentEvalutionComment']
                    )) {
                        // nothig to process here.
                    }
                    $this->Flash->success(
                        __(
                            'Thank you!,  You have evaluated ' . $this->request->data['Instructor']['full_name'] . '. Please fill the next instructor evaluation.'
                        )
                    );
                    return $this->redirect(array('action' => 'add'));
                } else {
                ...
                $this->Flash->warning(
                    __(
                        "You have already evaluated " . $this->request->data['Instructor']['full_name'] . ", you don not need to evaluate again."
                    )
                );
            }
        }
        ...
        if (empty($courseList)) {
            ...
            if (!empty($ac['academicYear'])) {
                ...
                return $this->redirect(
                    array(
                        'controller' => 'exam_grades',
                        'action' => "student_grade_view/$academicYear/$academicYear/$semester"
                    )
                );
            }
        ...
        $instructorEvalutionQuestionsObjective = $this->StudentEvalutionRate->InstructorEvalutionQuestion->find(
            'all',
            array(
                'conditions' => array(
        ...
        $instructorEvalutionQuestionsOpenEnded = $this->StudentEvalutionRate->InstructorEvalutionQuestion->find(
            'all',
            array(
                'conditions' => array(
        ...
        $this->set(
            compact('instructorEvalutionQuestionsObjective', 'instructorEvalutionQuestionsOpenEnded', 'courseList')
        );
    }
}
