    ...
    public function index()
    {
        ...
        if (!empty($this->passedArgs)) {
            ...
            if (!empty($this->passedArgs['Search.academic_year'])) {
                $this->request->data['Search']['academic_year'] = $selected_academic_year = str_replace(
                    '-',
                    '/',
                    $this->passedArgs['Search.academic_year']
                );
            }
        ...
        if (!empty($this->request->data) /* && isset($this->request->data['viewClearance']) */) {
            ...
            if ($this->role_id == ROLE_STUDENT) {
                ...
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                    $departments = array();
                    if (!$this->onlyPre) {
                        $departments = $this->Clearance->Student->Department->find(
                            'list',
                            array(
                                'conditions' => array(
                    ...
                    if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
                        ...
                        if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR) {
                            if (!empty($this->department_ids)) {
                                ...
                                if (!empty($this->college_ids)) {
                                    $departments = array();
                                    $colleges = $this->Clearance->Student->College->find(
                                        'list',
                                        array(
                                            'conditions' => array(
                            ...
                            if (!empty($colleges) && !empty($departments)) {
                                ...
                                if (!empty($departments)) {
                                    $options['conditions'][] = array(
                                        'Student.department_id' => array_keys(
                                            $departments
                                        )
                                    );
                                } else {
                    ...
                    'Student' => array(
                        ...
                        'StudentExamStatus' => array(
                            'order' => array(
                                ...
                                'StudentExamStatus.sgpa' => 'DESC',
                                'StudentExamStatus.cgpa' => 'DESC'
                            )
                        ),
        ...
        if (!empty($departments) && count($departments) > 1) {
            ...
            if (!empty($colleges) && count($colleges) > 1) {
        ...
        $programs = $this->Clearance->Student->Program->find(
            'list',
            array('conditions' => array('Program.id' => $this->program_ids, 'Program.active' => 1))
        );
        $program_types = $programTypes = $this->Clearance->Student->ProgramType->find(
            'list',
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Clearance->validateStudentClearance($this->student_id)) {
                ...
                $checkDorm = ClassRegistry::init('DormitoryAssignment')->takeNDormAndReturn(
                    $this->student_id,
                    $this->request->data['Clearance']['request_date']
                );
                ...
                if ($checkDorm == 0) {
                    if ($this->Clearance->checkDuplication($this->request->data)) {
                        ...
                        if ($this->Clearance->saveAll($this->request->data, array('validate' => 'first'))) {
                            $this->Flash->success(
                                'Your clearance request has been sent to registrar for approval and you  can cancel your clearance request before the registrar confirms it.'
                            );
                    ...
                    $this->Flash->error(
                        'You can not cleared before returning dormitory. Please return, and come back again.'
                    );
                }
            } else {
                ...
                if (isset($error['not_returned_item'])) {
                    ...
                    $this->Session->setFlash(
                        '<span></span> &nbsp;&nbsp;' . __(
                            'You can not be cleared before returning taken properties from the university, please return the properties and come back for clearance request.' . $string . ''
                        ),
                        'default',
        ...
        $student_section_exam_status = $this->Clearance->Student->get_student_section(
            $this->student_id,
            ...
            if (!empty($this->request->data['Clearance'])) {
                ...
                if (count($this->request->data['Clearance']) > 0) {
                    if ($this->Clearance->saveAll($this->request->data['Clearance'], array('validate' => 'first'))) {
                        $this->Flash->success(
                            'The selected clearance applicant(s) request has been approved and applicant(s) will be notified.'
                        );
                        $this->redirect(array('action' => 'index'));
                    ...
                    if (!empty($this->request->data['Clearance'])) {
                        foreach ($this->request->data['Clearance'] as $seindex => $secvalue) {
                            if ($secvalue['forced_withdrawal'] == 1) {
                                $this->Clearance->Student->StudentsSection->id = $this->Clearance->Student->StudentsSection->field(
                                    'StudentsSection.id',
                                    array(
                                        'StudentsSection.student_id' => $secvalue['student_id'],
