...
class GraduationStatusesController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid graduation status'),
                'default',
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
            $this->request->data['GraduationStatus']['status'] = trim(
                $this->request->data['GraduationStatus']['status']
            );
            ...
            $cgpa_duplicate_check = $this->GraduationStatus->find(
                'count',
                array(
                    'conditions' =>
                        array(
            ...
            $status_duplicate_check = $this->GraduationStatus->find(
                'count',
                array(
                    'conditions' =>
                        array(
            ...
            if ($cgpa_duplicate_check > 0) {
                $this->Session->setFlash(
                    '<span></span>' . __(
                        'There is already graduation status with ' . $this->request->data['GraduationStatus']['cgpa'] . ' CGPA for ' . $this->request->data['GraduationStatus']['academic_year'] . ' academic year. Please use edit to apply changes.'
                    ),
                    'default',
                    array('class' => 'error-box error-message')
                );
            } else {
                if ($status_duplicate_check > 0) {
                    $this->Session->setFlash(
                        '<span></span>' . __(
                            'There is already graduation status with "' . $this->request->data['GraduationStatus']['status'] . '" status for ' . $this->request->data['GraduationStatus']['academic_year'] . ' academic year. Please use edit to apply changes.'
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                    ...
                    if ($this->GraduationStatus->save($this->request->data)) {
                        $this->Session->setFlash(
                            '<span></span>' . __('The graduation status has been saved'),
                            'default',
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __('The graduation status could not be saved. Please, try again.'),
                            'default',
                            array('class' => 'error-box error-message')
                        );
                    }
                ...
                if ($status_duplicate_check > 0) {
                    ...
                    if ($this->GraduationStatus->save($this->request->data)) {
                        ...
                        return $this->redirect(
                            array('action' => 'index', $this->request->data['GraduationStatus']['program_id'])
                        );
                    } else {
                        $this->Session->setFlash(
                            '<span></span>' . __('The graduation status could not be saved. Please, try again.'),
    ...
    public function delete($id = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid id for graduation status'),
                'default',
                array('class' => 'error-box error-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        if ($this->GraduationStatus->delete($id)) {
            $this->Session->setFlash(
                '<span></span>' . __('Graduation status deleted'),
                'default',
                array('class' => 'success-box success-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        $this->Session->setFlash(
            '<span></span>' . __('Graduation status was not deleted'),
            'default',
            array('class' => 'error-box error-message')
        );
        return $this->redirect(array('action' => 'index'));
