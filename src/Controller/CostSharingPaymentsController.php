...
class CostSharingPaymentsController extends AppController
{
    ...
    public $name = 'CostSharingPayments';
        ...
        if (!empty($this->request->data) && isset($this->request->params['form']['search'])) {
            ...
            if (!empty($this->request->data['CostSharingPayment']['reference_number'])) {
                $options[] = array(
                    ...
                    "CostSharingPayment.reference_number" => trim(
                        $this->request->data['CostSharingPayment']['reference_number']
                    ),
            ...
            if (!empty($this->request->data['CostSharingPayment']['stud'])) {
                $options[] = array(
                    ...
                    "Student.studentnumber like " => trim(
                            $this->request->data['CostSharingPayment']['studentnumber']
                        ) . '%'
                );
            }
    ...
    public function add()
    {
        ...
        if (
            ...
            if ($this->CostSharingPayment->save($this->request->data)) {
                ...
                $this->Session->setFlash(
                    '<span></span>' . __('The cost sharing payment could not be saved. Please, try again.'),
                    'default',
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            ...
            if (!empty($this->request->data['CostSharingPayment']['studentID'])) {
                ...
                if ($check_id_is_valid > 0) {
                    ...
                    $cost_share_summery = ClassRegistry::init('CostShare')->find(
                        'all',
                        array(
                ...
                $this->Session->setFlash(
                    '<span></span> ' . __('Please provide student number to maintain student cost sharing payment .'),
                    'default',
                    array('class' => 'error-box error-message')
                );
            }
