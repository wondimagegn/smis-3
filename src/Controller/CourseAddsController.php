...
class CourseAddsController extends AppController
{
	...
	function index()
	{
		...
		if (!empty($this->request->data)) {
			...
			if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
				...
				if ($this->request->data['Search']['status'] == 'notprocessed') {
					$options['conditions'][] = array('CourseAdd.department_approval is null');
				...
				if ($this->request->data['Search']['status'] == 'notprocessed') {
					$options['conditions'][] = array('CourseAdd.registrar_confirmation is null');
				} else if ($this->request->data['Search']['status'] == 'accepted') {
					$options['conditions'][] = array(
						'OR' => array(
							'CourseAdd.department_approval' => 1,
			...
			if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
				$departments = $this->CourseAdd->Student->Department->find('list', array('conditions' => array('Department.college_id' => $this->college_ids, 'Department.active' => 1)));
				$options['conditions'][] = array(
		...
		$this->set(compact(/* 'courseAdds', */ 'programs', 'programTypes', 'departments', 'colleges', 'limit', 'name', 'studentnumber', 'turn_off_search', 'default_department_id', 'default_college_id'));
	}
		...
		if ($getRegistrationDeadLine != 1) {
			...
			if (!empty($this->request->data)) {
				...
				if ($selected > 0) {
					...
					$this->Session->error('Please select atleast one course you want to add.');
				}
				//debug($this->request->data);
			}
		}
	...
	function get_published_add_courses($section_id = null, $student_id = null, $academicYearSemester = null)
	{
		...
		if ($student_section_id == $section_id) {
			...
			$otherpublished = $this->CourseAdd->PublishedCourse->find('all', array(
				...
				'contain' => array(
					'Course' => array(
						'fields' => array('course_code', 'credit', 'id', 'course_title', 'active'),
	...
	function cancel_mass_add()
	{
		...
		if ($this->role_id == ROLE_REGISTRAR) {
			$yearLevels = $this->CourseAdd->YearLevel->distinct_year_level();
			$programs = $this->CourseAdd->Student->Program->find('list', array('conditions' => array('Program.id' => $this->program_id)));
			$programTypes = $this->CourseAdd->Student->ProgramType->find('list', array('conditions' => array('ProgramType.id' => $this->program_type_id)));
			$departments = $this->CourseAdd->Student->Department->find('list', array('conditions' => array('Department.id' => $this->department_ids, 'Department.active' => 1)));
		} else if ($this->role_id == ROLE_COLLEGE) {
			$yearLevels = $this->CourseAdd->YearLevel->distinct_year_level();
			...
			$departments = $this->CourseAdd->Department->find('list', array('conditions' => array('Department.id' => $this->department_id,)));
			$yearLevels = $this->CourseAdd->YearLevel->find('list', array('conditions' => array('YearLevel.department_id' => $this->department_id)));
			$programs = $this->CourseAdd->Student->Program->find('list');
			$programTypes = $this->CourseAdd->Student->ProgramType->find('list');
		} else {
			...
			if ($one_is_selected) {
				if (!empty($selected_published_courses)) {
					...
					if (isset($selected_courses_add['CourseAdd']) && count($selected_courses_add['CourseAdd']) > 0) {
						...
						$this->Flash->warning('The mass add for ' . $totalStudentAlreadyAdded . ' student(s) has already approved earlier and currently the system could not find more students for approval.');
					}
				}
			} else {
	...
	private function __cancel_course_add($selected = null)
	{
		...
		*/
		...
		if (is_numeric(YEARS_BACK_FOR_NG_F_FX_W_DO_I_CANCELATION) && YEARS_BACK_FOR_NG_F_FX_W_DO_I_CANCELATION) {
			$acyear_list = $this->AcademicYear->academicYearInArray(((explode('/', $current_acy)[0]) - YEARS_BACK_FOR_NG_F_FX_W_DO_I_CANCELATION), (explode('/', $current_acy)[0]));
