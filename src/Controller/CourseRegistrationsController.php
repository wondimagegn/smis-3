...
class CourseRegistrationsController extends AppController
{
	...
	public function beforeRender()
	{
		...
		//$acYearMinuSeparated = $this->AcademicYear->acYearMinuSeparated(date('Y') - 5, date('Y'));
		//$defaultacademicyearMinusSeparted = str_replace('/', '-', $defaultacademicyear);
	...
	*/
	...
	private function __register_student($student_id = null, $register_selected_section = null)
	{
				...
				if (!empty($this->request->data)) {
					if (!$this->Session->check('Message.flash')) {
						if (!empty($this->request->data['Student']['section_id'])) {
							...
							$this->Flash->info('No result found for ' . $sectionName . ' section that needs course registration maintaince for ' . ($this->request->data['Student']['semester'] == 'I' ? '1st' : ($this->request->data['Student']['semester'] == 'II' ? '2nd' : '3rd'))  . ' semester of ' . $this->request->data['Student']['academicyear'] . ' academic year.');
						} else if (empty($this->request->data['Student']['studentnumber'])) {
							$this->Flash->info('No result found in the given criteria that needs course registration maintaince for ' . ($this->request->data['Student']['semester'] == 'I' ? '1st' : ($this->request->data['Student']['semester'] == 'II' ? '2nd' : '3rd'))  . ' semester of ' . $this->request->data['Student']['academicyear'] . ' academic year.');
						}
					}
				} else {
					$this->Flash->info('No result found in the given criteria that needs course registration maintaince for ' . ($this->request->data['Student']['semester'] == 'I' ? '1st' : ($this->request->data['Student']['semester'] == 'II' ? '2nd' : '3rd'))  . ' semester of ' . $this->request->data['Student']['academicyear'] . ' academic year.');
		...
		$this->set(compact('departments', 'colleges', 'programs', 'programTypes', 'latest_semester_academic_year', 'sections', 'yearLevels'));
								...
								foreach ($student_list as $stk => $stv) {
									...
									if (is_numeric($maxLoad) && $maxLoad > 0) { // > 0, only registers students who are registered for at least one course, >= 0 all students in the section, with or without registration
										...
										if ((($maxLoad + $publishedCourseDetailedS['Course']['credit']) <= $allowedMaximum) && (($maxLoad + $total_selected_credit) <= $allowedMaximum)) {
											...
											if (!($this->CourseRegistration->courseRegistered($p_id, $publishedCourseDetailedS['PublishedCourse']['semester'], $publishedCourseDetailedS['PublishedCourse']['academic_year'], $stv)) && $this->CourseRegistration->CourseDrop->course_taken($stv, $publishedCourseDetailedS['PublishedCourse']['course_id']) == 3) {
		...
		if (!empty($this->request->data) && isset($this->request->data['getsection'])) {
			...
			if ($everythingfine) {
				...
				if (empty($listOfPublishedCourses) && !isset($this->request->data['registerIndivdualCourse']) ) {
					$this->Flash->info('No result is found in a given criteria. Either all students are registered or no active section is found with published course that needs mass course registration.');
				} else {
					$this->set('hide_search', true);
				...
				if (!empty($this->request->data['Search']['studentnumber']) && $this->role_id == ROLE_REGISTRAR) {
					...
					if (!empty($acSemylist)) {
						...
						foreach ($acSemylist as $acskey => $acsemval) {
							...
							$student_copy = ClassRegistry::init('ExamGrade')->getStudentCopy($student_id, $acsemval['CourseRegistration']['academic_year'], $acsemval['CourseRegistration']['semester']);
					...
					if (!empty($courseRegistrations)) {
						...
						foreach ($courseRegistrations as $k => $v) {
							...
							if (isset($this->request->data['getGradeReport'])) {
								$status_generated = 0;
								$check_status_generated0 = $this->CourseRegistration->Student->StudentExamStatus->get_student_exam_status($v['CourseRegistration']['student_id'], $v['CourseRegistration']['academic_year'], $v['CourseRegistration']['semester']);

								$check_status_generated = $this->CourseRegistration->Student->StudentExamStatus->find('count', array('conditions' => array('StudentExamStatus.student_id' => $v['CourseRegistration']['student_id'], 'StudentExamStatus.academic_year' => $v['CourseRegistration']['academic_year'], 'StudentExamStatus.semester' => $v['CourseRegistration']['semester'])));
	...
	public function getIndividualRegistration($parameters)
	{
		...
		if (!empty($criteriaLists) && count($criteriaLists) > 2) {
			...
			$passed_or_failed = $this->CourseRegistration->Student->StudentExamStatus->get_student_exam_status($student, $latest_academic_year['academic_year'], $semester);
			debug($passed_or_failed);
		...
		$this->set(compact('publishedCourses', 'studentDetail', 'show_manage_ng_link'));
	}
