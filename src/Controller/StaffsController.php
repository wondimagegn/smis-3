...
class StaffsController extends AppController
{
    ...
    public function __init_search_staff()
    {
    ...
    public function index()
    {
        ...
        if (isset($this->request->data) && !empty($this->request->data)) {
            ...
            if ($this->role_id == ROLE_DEPARTMENT) {
                ...
                if ($this->role_id == ROLE_COLLEGE) {
                    ...
                    if ($this->role_id != ROLE_SYSADMIN && $this->Session->read('Auth.User')['is_admin'] == 1) {
                        $departments = array();
            ...
            if ($this->request->data['Staff']['status'] == 1) {
                ...
                if ($this->request->data['Staff']['status'] == 0) {
            ...
            if ($this->request->data['Staff']['haveuser'] == 1) {
                ...
                if ($this->request->data['Staff']['haveuser'] == 0) {
                    $options[] = array(
                        'OR' => array(
        ...
        if (!empty($options)) {
            $this->Paginator->settings = array(
                ...
                'order' => array(
                    'User.role_id' => 'ASC',
                    'Staff.college_id' => 'ASC',
                    'Staff.department_id' => 'ASC',
                    /* 'Staff.position_id'  => 'ASC', */
        ...
        if ($this->role_id == ROLE_SYSADMIN) {
            ...
            if (!empty($this->request->data) && isset($this->request->data['Staff']['college_id'])) {
                $departments = $this->Staff->Department->find(
                    ...
                    array(
                        'conditions' => array(
                            'Department.college_id' => $this->request->data['Staff']['college_id'],
                            'Department.active' => 1
                        )
                    )
                );
    ...
    public function edit($id = null)
    {
        ...
        if (empty($this->request->data)) {
            $this->request->data = $this->Staff->find(
                'first',
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Staff->canItBeDeleted($id)) {
            ...
            $this->Flash->error(
                '<span></span>' . __(
                    'Staff can not be deleted. It is involved in course assignments or it has associated user account.'
                )
            );
        }
    ...
    public function get_department_staffs($department_id = null)
    {
        ...
        if (isset($this->request->data['getReport']) || isset($this->request->data['getReportExcel'])) {
            if ($this->request->data['Staff']['report_type'] == 'distributionStatsGenderTeachersByGender') {
                ...
                if ($this->request->data['Staff']['report_type'] == 'distributionStatsByAcademicRank') {
                    ...
                    $positions = $this->Staff->Position->find(
                        'list',
                        array(
            ...
            if ($this->request->data['Staff']['report_type'] == 'distributionStatsByStudents') {
                ...
                if ($this->request->data['Staff']['report_type'] == 'active_staff_list') {
                    ...
                    if ($this->request->data['Staff']['report_type'] == 'inactive_staff_list') {
                        $distributionStatistics = ClassRegistry::init('Staff')->getActiveStaffList(
                            $this->request->data['Staff']['department_id'],
                            $this->request->data['Staff']['gender'],
                            0
    ...
    public function maintain_staff_profile()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $userExists = $this->Staff->User->find(
                'first',
                array(
                    'conditions' => array('User.email' => $this->request->data['Staff']['email']),
            ...
            if (isset($userExists) && !empty($userExists)) {
                if ($userExists['User']['role_id'] != 3) {
                    ...
                    $staffDetails = $this->Staff->find(
                        'first',
                        array(
                            'conditions' => array(
                                'OR' => array(
                                    'Staff.first_name' => $userExists['User']['first_name'],
                                    'Staff.middle_name' => $userExists['User']['middle_name'],
                                    'Staff.last_name' => $userExists['User']['last_name']
                                ),
                                'Staff.user_id IS NOT NULL',
                                'Staff.email' => $this->request->data['Staff']['email']
                            ),
                            'fields' => array('id', 'first_name', 'middle_name', 'last_name', 'user_id', 'email'),
                            'recursive' => -1
                        )
                    );
                    //debug($staffDetails);
                    ...
                    if (isset($staffDetails)) {
                        $this->Flash->error(
                            __(
                                'User account for ' . $userExists['User']['first_name'] . ' ' . $userExists['User']['middle_name'] . ' ' . $userExists['User']['last_name'] . '(' . $userExists['User']['username'] . ')' . ' aready exists. You do not neeed to add it again.'
                            )
                        );
                    }
                } else {
                    $this->Flash->error(
                        __(
                            'The provided email is already in use for a student ' . $userExists['User']['first_name'] . ' ' . $userExists['User']['middle_name'] . ' ' . $userExists['User']['last_name'] . '(' . $userExists['User']['username'] . ')' . '. Please correct that before continuing.'
                        )
                    );
                }
            } else {
                //check if there is staff profile with provided email that does not have a user account
                $staffProfileExists = $this->Staff->find(
                    'first',
                    array(
                        'conditions' => array(
                            'OR' => array('Staff.user_id IS NULL', 'Staff.user_id = ""'),
                            'Staff.email' => $this->request->data['Staff']['email']
                        ),
                        'fields' => array(
                            'id',
                ...
                if (isset($staffProfileExists) && !empty($staffProfileExists)) {
                    $this->Flash->error(
                        __(
                            'Staff Profile for ' . $staffProfileExists['Staff']['first_name'] . ' ' . $staffProfileExists['Staff']['middle_name'] . ' ' . $staffProfileExists['Staff']['last_name'] . '(' . $staffProfileExists['Staff']['email'] . ')' . ' already exists. Use create a user account feature instead.'
                        )
                    );
                } else {
                    $this->Staff->create();
        ...
        $this->set(
            compact('positions', 'titles', 'countries', 'colleges', 'educations', 'servicewings', 'departments')
        );
    }

    public function update_staff_profile($id = null)
    {
        ...
        $this->set(
            compact('positions', 'titles', 'countries', 'colleges', 'educations', 'departments', 'servicewings')
        );
    ...
    public function staff_profile($staff_id = null)
    {
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            if (!empty($this->request->data['Staff']['staffid'])) {
                $checkIdIsValid = $this->Staff->find(
                    'count',
    ...
    public function ajax_add_study($staff_id, $editId = null)
    {
        ...
        if (!empty($editId)) {
            $this->request->data = $this->Staff->StaffStudy->find(
                'first',
