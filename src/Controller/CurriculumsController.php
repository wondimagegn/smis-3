...
class CurriculumsController extends AppController
{
    ...
    public $name = 'Curriculums';
    public $helpers = array('DatePicker', 'Media.Media');
    ...
    public function __init_search_curriculum()
    {
    ...
    public function index($data = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->request->data['Curriculum']['active'] == '1') {
                ...
                if ($this->request->data['Curriculum']['active'] == '0') {
                    $active = 0;
                } else {
                    $active = array(0, 1);
                }
            ...
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
                ...
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                    $department_ids = $this->Curriculum->Department->find(
                        'list',
                        array(
                            'conditions' => array(
                    ...
                    $departments = $this->Curriculum->Department->find(
                        'list',
                        array(
                            'conditions' => array(
                                'Department.college_id' => $this->college_id,
                    ...
                    if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR) {
                        ...
                        if (!empty($this->request->data['Curriculum']['department_id'])) {
                            $options['conditions'][] = array(
                                'Curriculum.department_id' => $this->request->data['Curriculum']['department_id'],
                                'Curriculum.program_id' => $this->program_ids
                            );
                        } else {
                            if (empty($this->request->data['Curriculum']['department_id']) && !empty($this->request->data['Curriculum']['college_id'])) {
                                $departments = $this->Curriculum->Department->find(
                                    'list',
                                    array(
                                        'conditions' => array(
                                            'Department.college_id' => $this->request->data['Curriculum']['college_id'],
                                            'Department.active' => 1
                                        )
                                    )
                                );
                        ...
                        $college_ids = $this->Curriculum->Department->find(
                            'list',
                            array(
                                'conditions' => array(
                                    'Department.id' => $this->department_ids,
                        ...
                        $colleges = $this->Curriculum->Department->College->find(
                            'list',
                        ...
                        if (!empty($this->request->data['Curriculum']['department_id'])) {
                            ...
                            if (empty($this->request->data['Curriculum']['department_id']) && !empty($this->request->data['Curriculum']['college_id'])) {
                                ...
                                $options['conditions'][] = array(
                                    'Curriculum.department_id' => array_keys(
                                        $departments
                                    )
                                );
                            } else {
            ...
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                ...
                if (!empty($this->department_ids)) {
                    ...
                    $options['conditions'][] = array(
                        'Curriculum.department_id' => $this->department_ids,
                        'Curriculum.program_id' => $this->program_id,
                        'Curriculum.active' => 1
                    );
                }
            } else {
        ...
        if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
            $department_college_id = $this->Curriculum->Department->field(
                'Department.college_id',
                array('Department.id' => $this->department_id)
            );
            $college_type = $this->Curriculum->Department->College->field(
                'College.type',
                array('College.id' => $department_college_id)
            );
        } else {
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                $college_type = $this->Curriculum->Department->College->field(
                ...
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR) {
                    $programs = $this->Curriculum->Program->find(
                        'list',
        ...
        $program_types = $programTypes = $this->Curriculum->ProgramType->find(
            'list',
            array('conditions' => array('ProgramType.id' => $this->program_type_ids, 'ProgramType.active' => 1))
        );
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data) && !empty($this->request->data['saveCurriculum'])) {
            ...
            if (!empty($this->request->data['Curriculum'])) {
                if ($this->Curriculum->saveAll($this->request->data, array('validate' => 'first'))) {
                    $this->Flash->success(
                        $this->request->data['Curriculum']['name'] . ' been saved. You can make any required modifications or updates.'
                    );
        ...
        if (!empty($this->request->data) && isset($this->request->data['copyCurriculum'])) {
            if (!empty($this->request->data['Curriculum']['from_curriculum'])) {
                $curriculums = $this->Curriculum->find(
                    'first',
                    array(
                        'conditions' => array('Curriculum.id' => $this->request->data['Curriculum']['from_curriculum']),
                ...
                if (!empty($formatCurriculumForSaveAll)) {
                    ...
                    if ($this->Curriculum->saveAll($formatCurriculumForSaveAll, array('validate' => false))) {
                        ...
                        $this->Flash->success(
                            $curriculums['Curriculum']['name'] . ' been copied successfully to ' . $formatCurriculumForSaveAll['Curriculum']['name'] . '. You can now make the required modifications or updates.'
                        );
        ...
        $earlierCurriculums = $this->Curriculum->find(
            'list',
            array(
                'conditions' => array('Curriculum.department_id' => $this->department_id, 'Curriculum.active' => 1),
                'order' => array('Curriculum.year_introduced' => 'DESC', 'Curriculum.created' => 'DESC')
            )
        );
        ...
        $programsss = $this->Curriculum->Program->find(
            'list',
            array('conditions' => array(/* 'Program.id' => $this->program_ids,  */ 'Program.active' => 1))
        );
        $program_types = $programTypes = $this->Curriculum->ProgramType->find(
        ...
        $year_based_curriculum_allowed = $this->Curriculum->Department->field(
            'allow_year_based_curriculums',
            array('Department.id' => $this->department_id)
        );
    ...
    public function edit($id = null)
    {
        ...
        $elgible_user = $this->Curriculum->find(
            'count',
            array('conditions' => array('Curriculum.id' => $id, 'Curriculum.department_id' => $this->department_id))
        );
        ...
        $qualification_ids = ClassRegistry::init('Qualification')->find(
            'list',
            array(
                'fields' => array('Qualification.id'),
                'conditions' => array('Qualification.program_id' => (isset($curriculum['Curriculum']['program_id']) && !empty($curriculum['Curriculum']['program_id']) ? $curriculum['Curriculum']['program_id'] : $this->request->data['Curriculum']['program_id']))
            )
        );
        ...
        $getDepartmentStudyProgramList = $this->Curriculum->getDepartmentStudyProgramDetails(
            $this->department_id,
            null,
            $qualification_ids
        );
        ...
        $programsss = $this->Curriculum->Program->find(
            ...
            array(
                'conditions' => array(
                    'Program.id' => $this->request->data['Curriculum']['program_id'],
                    'Program.active' => 1
                )
            )
        );
        ...
        $this->set(
            compact(
                ...
                'department_id',
                'course_category_values',
                'getDepartmentStudyProgramList'
            )
        );
    }

    public function deleteCourseCategory($id = null, $action_controller_id = null)
    {
        ...
        if (!empty($action_controller_id)) {
            ...
            $doesCourseCategoryBelongsToCourses = ClassRegistry::init('Course')->find(
                'count',
                array('conditions' => array('Course.course_category_id' => $id))
            );
            ...
            if (!$doesCourseCategoryBelongsToCourses) {
                if ($this->Curriculum->CourseCategory->delete($id)) {
                    ...
                    if (!empty($course_category[0]) && !empty($course_category[1]) && !empty($course_category[2])) {
                        $this->redirect(
                            array(
                                'controller' => $course_category[1],
                                'action' => $course_category[0],
                                $course_category[2]
                            )
                        );
                    }
                ...
                $this->Flash->warning(
                    '"' . $categoryName . '" course category is not deleted.' . ($doesCourseCategoryBelongsToCourses > 0 ? ' This course category is associated to ' . $doesCourseCategoryBelongsToCourses . ' courses.' : ' This course category is associated to courses.')
                );
            }

            if (!empty($course_category[0]) && !empty($course_category[1]) && !empty($course_category[2])) {
                $this->redirect(
                    array('controller' => $course_category[1], 'action' => $course_category[0], $course_category[2])
                );
            } else {
                $this->redirect(array('action' => 'index'));
            }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Curriculum->canItBeDeleted($id)) {
            ...
            $this->Flash->error(
                'You can not delete this curriculum, It is attached to other models like Courses, Categories etc.'
            );
            $this->redirect(array('action' => 'index'));
        }
    }

    public function get_courses($curriculum_id = null)
    {
    ...
    public function get_curriculums($department_id = null)
    {
    ...
    public function get_curriculum_combo($department_id = null, $program_id = null, $appr = null)
    {
    ...
    public function get_course_category_combo($curriculum_id = null)
    {
    ...
    public function lock($id = null)
    {
        ...
        $curriculums = $this->Curriculum->find(
            'first',
            array(
                'conditions' => array('Curriculum.id' => $this->Curriculum->id),
        ...
        if ($this->Curriculum->saveField('lock', $lock)) {
            $this->Flash->success(
                '"' . $curriculums['Curriculum']['curriculum_detail'] . '" curriculum is now ' . $message . '.'
            );
        } else {
            $this->Flash->error(
                '"' . $curriculums['Curriculum']['curriculum_detail'] . '" curriculum could not be ' . $message . ' right now, please try again later.'
            );
        }

        if (!empty($page)) {
            return $this->redirect(
                array('action' => 'index', $data, 'page' => $page, 'sort' => $sort, 'direction' => $direction)
            );
        } else {
            return $this->redirect(array('action' => 'index', $data));
        }
    ...
    public function approve($id = null)
    {
        ...
        if (((isset($curriculums['Curriculum']['department_study_program_id']) && REQUIRE_STUDY_PROGRAMS_SELECTED_FOR_CURRICULUM_APPROVAL == 1) || REQUIRE_STUDY_PROGRAMS_SELECTED_FOR_CURRICULUM_APPROVAL == 0) || ((!empty($curriculums['Attachment']) && REQUIRE_CURRICULUM_PDF_UPLOAD_FOR_CURRICULUM_APPROVAL == 1) || REQUIRE_CURRICULUM_PDF_UPLOAD_FOR_CURRICULUM_APPROVAL == 0)) {
            ...
            if ((!isset($curriculums['Curriculum']['department_study_program_id']) && REQUIRE_STUDY_PROGRAMS_SELECTED_FOR_CURRICULUM_APPROVAL == 1) && (empty($curriculums['Attachment']) && REQUIRE_CURRICULUM_PDF_UPLOAD_FOR_CURRICULUM_APPROVAL == 1)) {
                $this->Flash->warning(
                    '"' . $curriculums['Curriculum']['curriculum_detail'] . '" curriculum  must have Curriculum Program study defined and a soft copy of the curriculum PDF must be attached to it before approval.'
                );
            } else {
                if ((!isset($curriculums['Curriculum']['department_study_program_id']) && REQUIRE_STUDY_PROGRAMS_SELECTED_FOR_CURRICULUM_APPROVAL == 1)) {
                    $this->Flash->warning(
                        '"' . $curriculums['Curriculum']['curriculum_detail'] . '" curriculum  must have Curriculum Program study defined to it before approval.'
                    );
                } else {
                    if ((empty($curriculums['Attachment']) && REQUIRE_CURRICULUM_PDF_UPLOAD_FOR_CURRICULUM_APPROVAL == 1)) {
                        $this->Flash->warning(
                            '"' . $curriculums['Curriculum']['curriculum_detail'] . '" curriculum  must have have a soft copy of the curriculum PDF must be attached to it before approval.'
                        );
                    }
                }
            }
    ...
    public function add_departmernt_study_program_for_curriculum()
    {
        ...
        if (isset($this->request->data['Curriculum']['id']) && isset($this->request->data['Curriculum']['department_study_program_id'])) {
            ...
            $curriculum = $this->Curriculum->find(
                'first',
                array(
                    'conditions' => array('Curriculum.id' => $this->request->data['Curriculum']['id']),
                    'recursive' => -1
                )
            );
            ...
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && in_array(
                    $curriculum['Curriculum']['department_id'],
                    $this->department_ids
                ) && in_array($curriculum['Curriculum']['program_id'], $this->program_id)) {
                $this->Curriculum->id = $this->request->data['Curriculum']['id'];
                if ($this->Curriculum->saveField(
                    'department_study_program_id',
                    $this->request->data['Curriculum']['department_study_program_id']
                )) {
                    $this->Flash->success(
                        'Department study program added to "' . $curriculum['Curriculum']['curriculum_detail'] . '" curriculum successfully.'
                    );
                    ...
                    $this->Flash->error(
                        'Department study program was not added to "' . $curriculum['Curriculum']['curriculum_detail'] . '" curriculum. please try again later.'
                    );
                    //$this->redirect(array('controller' => 'curriculums', 'action' => 'index'));
                }
            } else {
    ...
    public function get_curriculums_based_on_program_combo($program_id = null)
    {
