...
class StudentStatusPatternsController extends AppController
{
    ...
    public $name = 'StudentStatusPatterns';
    ...
    public function beforeRender(Event $event)
    {
        ...
        $acyear_array_data = $this->AcademicYear->academicYearInArray(
            ((explode('/', $current_academicyear)[0]) - 8),
    ...
    public function regenerate_status($published_course_id = null)
    {
        ...
        if (!empty($this->department_id) && $this->role_id == ROLE_DEPARTMENT) {
            ...
            if (!empty($this->department_ids) && $this->role_id == ROLE_REGISTRAR) {
                ...
                if (!empty($this->college_ids) && $this->role_id == ROLE_REGISTRAR) {
            ...
            $programs = $this->StudentStatusPattern->Program->find(
                'list',
                array('conditions' => array('Program.id' => $this->program_ids))
            );
            $program_types = $this->StudentStatusPattern->ProgramType->find(
                'list',
        ...
        if (!empty($this->request->data)) {
            ...
            if (is_array($college_id) && count($college_id) > 1) {
                ...
                $publishedCourses = ClassRegistry::init(
                    'CourseInstructorAssignment'
                )->listOfCoursesCollegeFreshTakingOrgBySection(
                    $college_id,
                ...
                $publishedCourses = ClassRegistry::init(
                    'CourseInstructorAssignment'
                )->listOfCoursesSectionsTakingOrgBySection(
                    $department_id,
                    $this->request->data['StudentStatusPattern']['acadamic_year'],
                    $this->request->data['StudentStatusPattern']['semester'],
                    $this->request->data['StudentStatusPattern']['program_id'],
        ...
        if (!empty($published_course_id) || (isset($this->request->data['StudentStatusPattern']['published_course_id']) && $this->request->data['StudentStatusPattern']['published_course_id'] != 0)) {
            ...
            if (empty($published_course) || (!empty($published_course['PublishedCourse']['department_id']) && !in_array(
    ...
    public function regenerate_individual_academic_status()
    {
                ...
                foreach ($listPublishedCourseTakenBySection as $value) {
                    ...
                    if (!$checkIfStatusIsGenerated) {
                        $statusgenerated = ClassRegistry::init(
                            'Student'
                        )->StudentExamStatus->updateAcdamicStatusByPublishedCourseOfStudent(
        ...
        if (!empty($this->request->data) && isset($this->request->data['regeneratestudentstatus'])) {
            ...
            if ($everythingfine) {
                ...
                if ($check_id_is_valid > 0) {
                    ...
                    if ($check_id_is_valid) {
                        ...
                        $this->Flash->warning(
                            $studentDetails['Student']['full_name'] . ' (' . $studentDetails['Student']['studentnumber'] . ') is a graduated student. Status Regeneration for graduated student is not allowed.'
                        );
                    } else {
                        $check_id_is_valid = ClassRegistry::init('Student')->find('count', array(
                            'conditions' => array(
                        ...
                        if (!$check_id_is_valid) {
                            $this->Flash->error(
                                'The provided student number is not valid, Please check and try again.'
                            );
                        } else {
                            $this->Flash->warning(
                                'You don not have the privilege to access the selected student\'s profile.'
                            );
                        }
                    }
                }
            ...
            if ($everythingfine) {
                ...
                $studentDbId = ClassRegistry::init('Student')->field(
                    'Student.id',
                    array('Student.studentnumber' => trim($this->request->data['Student']['studentnumber']))
                );
    ...
    public function __init_search()
    {
        ...
        if (!empty($this->request->data['Student'])) {
            ...
            if ($this->Session->check('search_data_statusgeneration')) {
                $this->request->data['Student'] = $this->Session->read('search_data_statusgeneration');
            }
        }
    }

    public function regenerate_academic_status($section_id = null)
    {
        ...
        */
        ...
        if (isset($this->request->data['regenerateStatus'])) {
            ...
            if (!empty($student_ids)) {
                if ($this->_generateBySection(
                    $this->request->data['StudentStatusPattern']['section_id'],
                    $student_ids
                )) {
                    $this->Flash->success(
                        'Status regenerated for ' . (count($student_ids)) . ' ' . (count(
                            $student_ids
                        ) > 1 ? 'students' : 'student') . ' successfully.'
                    );
        ...
        if ($freshman_program == 0 && !empty($this->department_ids)) {
            $yearLevels = ClassRegistry::init('YearLevel')->find(
                'list',
                array(
                    'conditions' => array('YearLevel.department_id' => $this->department_ids),
                    'fields' => array('name', 'name')
                )
            );
        } else {
        ...
        if (isset($this->request->data['listSections'])) {
            ...
            if (!empty($this->request->data['StudentStatusPattern']['department_id'])) {
                $year_level_selected_id = ClassRegistry::init('YearLevel')->field(
                    'YearLevel.id',
                    array(
            ...
            if (!empty($this->request->data['StudentStatusPattern']) || (!empty($section_id) && ($section_id != 0 || strcasecmp(
                            $section_id,
                            "pre"
                        ) == 0))) {
                if (!empty($this->request->data['StudentStatusPattern']['department_id'])) {
                ...
                if (strcasecmp($section_id, "pre") == 0) {
                    $students_in_section = ClassRegistry::init('Student')->listStudentByAdmissionYear(
                        null,
                        $this->request->data['StudentStatusPattern']['college_id'],
                        $this->request->data['StudentStatusPattern']['acadamic_year'],
                        $this->request->data['StudentStatusPattern']['name'],
                        0
                    );
                    // $sections['pre']="All";
                    // asort($sections);
                    ...
                    $students_in_section = ClassRegistry::init('Section')->getSectionStudentsForStatus(
                        $section_id,
                        null /* $this->request->data['StudentStatusPattern']['acadamic_year'] */
                    );
                }
        ...
        if (!empty($section_id) && is_numeric($section_id) && $section_id > 0) {
            $selectedSectionsDetail = ClassRegistry::init('Section')->find(
                'first',
                array('conditions' => array('Section.id' => $section_id))
            );
    ...
    public function __init_search_student()
    {
        ...
        if (!empty($this->request->data['StudentStatusPattern'])) {
            ...
            if ($this->Session->check('search_data_student_status_pattern')) {
                $this->request->data['StudentStatusPattern'] = $this->Session->read(
                    'search_data_student_status_pattern'
                );
            }
        }
    }
    ...
    private function _generateBySection($section_id, $studentLists = array())
    {
        ...
        if (!empty($studentSectionAttended)) {
            foreach ($studentSectionAttended as $sec_id => $student) {
                ...
                if (!empty($listPublishedCourseTakenBySection)) {
                    foreach ($listPublishedCourseTakenBySection as $value) {
                        $checkIfStatusIsGenerated = ClassRegistry::init('Student')->StudentExamStatus->find(
                            'count',
                            array(
                                'conditions' => array(
                                    'StudentExamStatus.student_id' => $student['StudentsSection']['student_id'],
