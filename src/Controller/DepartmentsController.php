...
class DepartmentsController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->Paginator->settings = array(
            'contain' => array('College' => array('Campus' => array('id', 'name'))),
            'limit' => 200,
            'maxLimit' => 200,
            'order' => array(
                'College.campus_id' => 'ASC',
                'Department.college_id' => 'ASC',
                'Department.name' => 'ASC'
            ),
            'recursive' => -1
        );
        ...
        if ($this->Session->read('Auth.User')['role_id'] == ROLE_SYSADMIN) {
            ...
            if ($this->Session->read(
                ...
                $colleges = $this->Department->College->find(
                    'list',
                ...
                if ($this->Session->read(
                        'Auth.User'
                    )['role_id'] == ROLE_DEPARTMENT/*  && $this->Session->read('Auth.User')['is_admin'] == 1 */) {
                    $conditions['conditions'] = ['Department.id' => $this->department_id, 'Department.active = 1'];
                    $college_ids = $this->Department->find(
                        'list',
                        array(
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $this->request->data['Department']['name'] = trim(
                ucwords(strtolower($this->request->data['Department']['name']))
            );
            $this->request->data['Department']['shortname'] = trim($this->request->data['Department']['shortname']);
            $this->request->data['Department']['amharic_name'] = trim(
                $this->request->data['Department']['amharic_name']
            );
            $this->request->data['Department']['amharic_short_name'] = trim(
                $this->request->data['Department']['amharic_short_name']
            );
            $this->request->data['Department']['institution_code'] = trim(
                $this->request->data['Department']['institution_code']
            );
    ...
    public function edit($id = null)
    {
        ...
        if (empty($this->request->data)) {
            $department = $this->Department->find(
                'first',
                array('conditions' => array('Department.id' => $id), 'recursive' => -1)
            );
    ...
    public function get_department_combo($college_id = null, $all = 0, $active_only = '')
    {
    ...
    public function get_department_for_transfer($college_id = null, $exclude_department = null)
    {
