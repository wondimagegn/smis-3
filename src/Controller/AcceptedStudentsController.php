...
class AcceptedStudentsController extends AppController
{
	...
	public function index()
	{
		...
		if (/* isset($this->request->data) &&  */!empty($this->request->data)) {
			...
			if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
				...
				if (!empty($this->department_ids)) {
					...
					if (!empty($colleges)) {
						$options['conditions'][] = array('AcceptedStudent.college_id' => array_keys($colleges), 'AcceptedStudent.department_id IS NULL', 'AcceptedStudent.program_id' => $this->program_ids, 'AcceptedStudent.program_type_id' => $this->program_type_ids);
					}
				}
		...
		$this->set(compact('colleges', 'departments',/*  'acceptedStudents', */ 'turn_off_search', 'limit', 'name', 'selected_academic_year'));
	}
					...
					if ($this->AcceptedStudent->check_program_type($this->request->data, $this->role_id)) {
						...
						if ($this->AcceptedStudent->save($this->request->data)) {
							...
							if (!empty($studentDetail)) {
								...
								if(debug($this->AcceptedStudent->Student->saveAll($updateMinorStudentFields, array('validate' => true)))){
								...
								if (!empty($this->request->data['AcceptedStudent']['college_id'])) {
									if (!empty($this->request->data['AcceptedStudent']['academicyear'])) {
										...
										if (strcasecmp($this->request->data['Program']['name'], 'Undergraduate') != 0) {
											...
											} else if (strcasecmp($this->request->data['Program']['name'], 'PGDT') == 0) {
												$generatedStudentId = 'PGDT/' . '/' . $count . '/' . $ETshortAcyear;
											} else if (strcasecmp($this->request->data['Program']['name'], 'Post graduate') == 0) {
	...
	public function move_readmitted_to_freshman()
	{
		...
		if ($this->role_id == ROLE_REGISTRAR || ROLE_REGISTRAR == $this->Session->read('Auth.User')['Role']['parent_id']) {
			$colleges = $this->AcceptedStudent->College->find('list', array('conditions' => array('College.active' => 1),'order' => 'College.name ASC'));
		} else if (isset($this->college_id) && !empty($this->college_id)) {
			$colleges = $this->AcceptedStudent->College->find('list', array('conditions' => array('College.id' => $this->college_id, 'College.active' => 1), 'order' => 'College.name ASC'));
		} else {
			$colleges = $this->AcceptedStudent->College->find('list', array('conditions' => array('College.active' => 1),'order' => 'College.name ASC'));
		}
		...
		if (!empty($this->request->data) && isset($this->request->data['continue'])) {
			...
			$selected_colleges = $this->AcceptedStudent->College->find('list', array('conditions' => array('College.active' => 1),'order' => 'College.name ASC'));
	...
	function download_csv()
	{
		...
		if (!empty($acceptedStudents)) {
			foreach ($acceptedStudents as $key => $acceptedStudent) {
				...
				$compactData[$key]['AcceptedStudent']['id'] = $acceptedStudent['AcceptedStudent']['id'];
				$compactData[$key]['AcceptedStudent']['full_name'] = $acceptedStudent['AcceptedStudent']['full_name'];
				...
				$compactData[$key]['AcceptedStudent']['student_id'] = $acceptedStudent['AcceptedStudent']['studentnumber'];
				...
				$compactData[$key]['AcceptedStudent']['national_id'] = (isset($acceptedStudent['Student']) && !empty($acceptedStudent['Student']['student_national_id']) ? $acceptedStudent['Student']['student_national_id'] : ''); 
			}
		}
	...
	public function import_newly_students()
	{
		...
		if (!empty($this->request->data) && is_uploaded_file($this->request->data['AcceptedStudent']['File']['tmp_name'])) {
			...
			if (empty($data->sheets[0]['cells'][1])) {
				$this->Flash->error('Importing Error!!. Please insert your filed name (first_name,  middle_name, last_name, EHEECE_total_results, program_type, college, region, sex)  at first row of your excel file.');
				return;
			}
				...
				if ($data->sheets[0]['numRows']) {
					...
					for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
						...
						for ($j = 1; $j <= count($fields_name_acceptedStudents_table); $j++) {
							...
							if ($fields_name_acceptedStudents_table[$j] == "college" && in_array(trim($data->sheets[0]['cells'][$i][$j]), $colleges) == 0 ) {
							...
							if ($program_type_is_different_from_regular == $i || $check_program_is_different_from_undergraduate == $i) {
								...
								if ($fields_name_acceptedStudents_table[$j] == "EHEECE_total_results" && isset($data->sheets[0]['cells'][$i][$j]) && $data->sheets[0]['cells'][$i][$j] != "" && !is_numeric($data->sheets[0]['cells'][$i][$j])) {
							...
							if ($program_type_is_different_from_regular != $i && $check_program_is_different_from_undergraduate != $i) {
								if ($fields_name_acceptedStudents_table[$j] == "EHEECE_total_results" && (!isset($data->sheets[0]['cells'][$i][$j]) || (trim($data->sheets[0]['cells'][$i][$j]) == "" || !is_numeric($data->sheets[0]['cells'][$i][$j])))) {
									$non_valide_rows[] = "Please enter a valid EHEECE result at row " . $i;
									continue;
								}
							}
							...
							if (!$name_error_duplicate && in_array($fields_name_acceptedStudents_table[$j], array('first_name', 'middle_name', 'last_name')) && (!isset($data->sheets[0]['cells'][$i][$j]) || is_null(trim($data->sheets[0]['cells'][$i][$j])) || empty(trim($data->sheets[0]['cells'][$i][$j])))) {
							...
							if ($fields_name_acceptedStudents_table[$j] == "college" && (!isset($data->sheets[0]['cells'][$i][$j]) || !in_array(strtoupper(trim($data->sheets[0]['cells'][$i][$j])), $colleges))) {
								$non_valide_rows[] = "Please enter a valid college name at row " . $i;
								continue;
							}

							if (strcasecmp($fields_name_acceptedStudents_table[$j], "program_type") == 0 && (!isset($data->sheets[0]['cells'][$i][$j]) || !in_array(strtoupper(trim($data->sheets[0]['cells'][$i][$j])), $program_types))) {
						...
						if (INCLUDE_STUDENT_NUMBER_IN_IMPORT_TEMPLATE_FILE) {
							if (!empty($row_data['studentnumber'])) {
								$student_number_depulicated = $this->AcceptedStudent->find('count', array('conditions' => array('AcceptedStudent.studentnumber' => trim($row_data['studentnumber'])), 'recursive' => -1));
			...
			if (!empty($xls_data)) {
				...
				if ($this->AcceptedStudent->saveAll($reformat_for_saveAll['AcceptedStudent'], array('validate' => 'first'))) {
					...
					$this->Flash->error('Unable to import records. Please try again.');
				}
			} else {
