...
class WoredasController extends AppController
{
    ...
    public function edit($id = null)
    {
        ...
        if (empty($this->request->data)) {
            ...
            $this->request->data = $this->Woreda->find(
                'first',
                array('conditions' => array('Woreda.id' => $id), 'contain' => array('Zone'), 'recursive' => -1)
            );
        } else {
            $woreda = $this->Woreda->find(
                'first',
                array('conditions' => array('Woreda.id' => $id), 'contain' => array('Zone'), 'recursive' => -1)
            );
        }

        $zones = $this->Woreda->Zone->find(
            'list',
            array(
                'conditions' => array(
                    'Zone.id' => (isset($this->request->data['Woreda']['zone_id']) ? $this->request->data['Woreda']['zone_id'] : $woreda['Woreda']['zone_id']),
                    /* 'Zone.active' => 1 */
                )
            )
        );
