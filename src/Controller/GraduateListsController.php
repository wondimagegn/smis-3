...
class GraduateListsController extends AppController
{
    ...
    public function initialize()
    {
        ...
        $this->viewBuilder()->setHelpers(['Media.Media', 'Csv']);
    }
    ...
    public function beforeRender(Event $event)
    {
        ...
        $this->set(
            compact('acyear_array_data', 'defaultacademicyear'/* , 'program_types', 'programTypes', 'programs' */)
        );
    ...
    public function __init_search_graduate_list()
    {
    ...
    public function index($data = null)
    {
        ...
        if (!empty($this->passedArgs)) {
            if (isset($this->passedArgs['GraduateList.department_id'])) {
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                        'Auth.User'
                    )['is_admin'] != 1 && $this->passedArgs['GraduateList.department_id'] == 0) {
                    ...
                    if (!empty($this->passedArgs['GraduateList.department_id'])) {
            ...
            if (isset($this->passedArgs['GraduateList.program_id'])) {
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                        'Auth.User'
                    )['is_admin'] != 1 && $this->passedArgs['GraduateList.program_id'] == 0) {
                    ...
                    if (!empty($this->passedArgs['GraduateList.program_id'])) {
            ...
            if (isset($this->passedArgs['GraduateList.program_type_id'])) {
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                        'Auth.User'
                    )['is_admin'] != 1 && $this->passedArgs['GraduateList.program_type_id'] == 0) {
                    ...
                    if (!empty($this->passedArgs['GraduateList.program_type_id'])) {
            ...
            if (isset($this->passedArgs['GraduateList.minute_number']) && !empty($this->passedArgs['GraduateList.minute_number'])) {
                ...
                $this->request->data['GraduateList']['minute_number'] = str_replace(
                    '-',
                    '/',
                    trim($this->passedArgs['GraduateList.minute_number'])
                );
        ...
        $programs = $this->GraduateList->Student->Program->find(
            'list',
            array(
                'conditions' => array(
                    'Program.id' => (!empty($this->program_ids) ? $this->program_ids : $this->program_id)
                    /* , 'Program.active' => 1 */
                )
            )
        );
        $program_types = $programTypes = $this->GraduateList->Student->ProgramType->find(
            'list',
            array(
                'conditions' => array(
                    'ProgramType.id' => (!empty($this->program_type_ids) ? $this->program_type_ids : $this->program_type_id)
        ...
        if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
            $departments = $this->GraduateList->Student->Department->find(
                'list',
                array('conditions' => array('Department.id' => $this->department_id))
            );
        } else {
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                $departments = $this->GraduateList->Student->Department->allDepartmentsByCollege2(
                    1,
                    array(),
                    $this->college_id,
                    1
                ...
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                    ...
                    if (!empty($this->program_ids) || !empty($this->program_id)) {
                        $programs = $this->GraduateList->Student->Program->find(
                            'list',
                            array('conditions' => array('Program.id' => (!empty($this->program_ids) ? $this->program_ids : $this->program_id)))
                        );
                    ...
                    if (!empty($this->program_type_ids) || !empty($this->program_type_id)) {
                        $program_types = $this->GraduateList->Student->ProgramType->find(
                            'list',
                            array('conditions' => array('ProgramType.id' => (!empty($this->program_type_ids) ? $this->program_type_ids : $this->program_type_id)))
                        );
    ...
    public function add($department_id = null, $program_id = null, $program_type_id = null)
    {
        ...
        if (isset($this->request->data['listStudentsForGraduateList']) && $this->Session->check(
                'graduate_list_department_id'
            )) {
            ...
            $this->Session->write(
                'graduate_list_program_type_id',
                $this->request->data['GraduateList']['program_type_id']
            );
        }
        ...
        if (isset($this->request->data) && isset($this->request->data['addStudentToGraduateList'])) {
            if (trim($this->request->data['GraduateList']['minute_number']) == "") {
                ...
                if (!empty($this->request->data['Student'])) {
                    foreach ($this->request->data['Student'] as $key => $student) {
                        if ($student['include_graduate'] == 1) {
                            $sl_count = $this->GraduateList->find(
                                'count',
                                array('conditions' => array('GraduateList.student_id' => $student['id']))
                            );
                            if ($sl_count == 0) {
                                $sl_index = count($graduate_list);

                                $graduate_list[$sl_index]['student_id'] = $student['id'];
                                $graduate_list[$sl_index]['minute_number'] = trim(
                                    $this->request->data['GraduateList']['minute_number']
                                );
                                ...
                                $studentDetail = $this->GraduateList->Student->find(
                                    'first',
                ...
                if (empty($graduate_list)) {
                    $this->Flash->error(
                        'You are required to select at least one student to be included in the graduate list.'
                    );
                } else {
                    if ($this->GraduateList->saveAll($graduate_list, array('validate' => false))) {
                        $this->Flash->success(
                            count($graduate_list) . '' . (count(
                                $graduate_list
                            ) > 1 ? ' students are' : ' student is') . ' included in the graduate list.'
                        );
                        //archiveSection
                        if (isset($studentIdForSectionArchive) && !empty($studentIdForSectionArchive)) {
                            $sectionDeactivate = $this->GraduateList->query(
                                "UPDATE students_sections SET archive = 1 WHERE student_id in (" . join(
                                    ', ',
                                    $studentIdForSectionArchive
                                ) . ") "
                            );
                        }
                        if (isset($studentIdForSectionArchive) && !empty($studentIdForSectionArchive)) {
                            $studentGraduate = $this->GraduateList->query(
                                "UPDATE students SET graduated = 1 WHERE id in (" . join(
                                    ', ',
                                    $studentIdForSectionArchive
                                ) . ") "
                            );
                        ...
                        $this->Session->write(
                            'graduate_list_program_id',
                            $this->request->data['GraduateList']['program_id']
                        );
                        $this->Session->write(
                            'graduate_list_program_type_id',
                            $this->request->data['GraduateList']['program_type_id']
                        ...
                        $this->Flash->error(
                            'The system unable to include the selected students in the graduate list. Please try again.'
                        );
                    }
                }
            }
        ...
        if (!empty($department_id) || !empty($this->department_ids)) {
            ...
            if (!empty($deptCollID)) {
                $college_shortname = $this->GraduateList->Student->College->field(
                    'shortname',
    ...
    public function delete($id = null)
    {
        ...
        $valid_deletion_time = date(
            ...
            mktime(
                ...
                substr($graduate_detail['GraduateList']['created'], 8, 2) + Configure::read(
                    'Calendar.daysAvaiableForGraduateDeletion'
                ),
        ...
        if ($valid_deletion_time < date('Y-m-d')) {
            ...
            if ($this->GraduateList->delete($id)) {
                $this->GraduateList->query(
                    "UPDATE students set graduated = 0, modified = NOW() where id =" . $graduate_detail['Student']['id'] . ""
                );
                $this->Flash->success(
                    $graduate_detail['Student']['full_name'] . ' is successfully removed from the graduate list.'
                );
                return $this->redirect(array('action' => 'index'));
            }
        }
        $this->Flash->error(
            $graduate_detail['Student']['full_name'] . ' is not removed from the graduate list. Please try again.'
        );
        return $this->redirect(array('action' => 'index'));
    }

    public function temporary_degree($student_id = null)
    {
        ...
        if (!empty($student_id) || !empty($this->request->data['continueTemporaryDegreePrint'])) {
            ...
            if (empty($student_detail)) {
                ...
                return $this->redirect(array('action' => 'temporary_degree'));
            } elseif (!empty($student_detail['Student']['department_id']) && !in_array(
                    $student_detail['Student']['department_id'],
                    $this->department_ids
                )) {
                if (!empty($student_detail['Student']['department_id'])) {
                    $department_name = $this->GraduateList->CourseRegistration->Student->Department->field(
                        'name',
                        array(
                ...
                $this->Flash->error(
                    'You do not have the privilege to manage ' . $department_name . ' students. Please contact the registrar or system administrator to get privilege on ' . $department_name . '.'
                );
                return $this->redirect(array('action' => 'temporary_degree'));
            } else {
                ...
                if (empty($student_detail['GraduateList']) || $student_detail['GraduateList']['id'] == "") {
                    $this->Flash->error(
                        $student_detail['Student']['full_name'] . ' is not graduated to display ' . (strcasecmp(
                            $student_detail['Student']['gender'],
                            'male'
                        ) == 0 ? 'his' : 'her') . ' temporary degree.'
                    );
                } else {
    ...
    public function language_proficiency($student_id = null)
    {
        ...
        //debug($this->request->data);
        if (isset($this->request->data['continueLanguageProficiencyLetterPrint']) && !empty(
            trim(
                $this->request->data['GraduateList']['studentnumber']
            )
            )) {
            debug($this->request->data);
            ...
            if (isset($student_detail['Student']) && !empty($student_detail['Student'])) {
                if ((!empty($student_detail['Student']['department_id']) && !in_array(
                            $student_detail['Student']['department_id'],
                            $this->department_ids
                    ...
                    if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                        ...
                        $this->Flash->error(
                            'You need to switch your role ' . (empty($this->department_ids) ? 'from College to Department' : ' from Department to College') . ' in "Security > Users > List Users > Assign" for managing ' . $department_name . ' students.'
                        );
                    } else {
                        $this->Flash->error(
                            'You do not have the privilege to manage ' . $department_name . ' students. Please contact the registrar to get the privilege for managing ' . $department_name . ' students.'
                        );
                    }
                } else {
                    $student_curriculum_id = $this->GraduateList->Student->field(
                        'curriculum_id',
                        array('Student.id' => $student_detail['Student']['id'])
                    );
                    $is_student_graduated = $this->GraduateList->Student->field(
                        'graduated',
                        array('Student.id' => $student_detail['Student']['id'])
                    );
                    ...
                    if (!$is_student_graduated) {
                        $this->Flash->error(
                            $student_detail['Student']['full_name'] . ' (' . $student_detail['Student']['studentnumber'] . ')' . ' is not graduated.'
                        );
                    } else {
                        if (!is_numeric($student_curriculum_id) || is_null($student_curriculum_id)) {
                            $this->Flash->error(
                                $student_detail['Student']['full_name'] . ' (' . $student_detail['Student']['studentnumber'] . ')' . ' is not attached to any curriculum.'
                            );
                        } else {
                            ...
                            $degree_nomenclature_formatted = $this->__format_title(
                                $graduation_letter['Student']['Curriculum']['english_degree_nomenclature']
                            );
                ...
                $this->Flash->error(
                    'Student ID not found. Check for Typo Errors and Provide a valid Student ID without spaces.'
                );
            }
        } else {
        ...
        if (!empty($student_ids_array)) {
            ...
            if (!empty($graduation_letters) && isset($this->request->data['displayLanguageProficiencyLetterPrint'])) {
                ...
                $this->set(
                    compact('graduation_letters', 'degree_nomenclature_formatted', 'correct_degree_designation')
                );
                $this->response->type('application/pdf');
                $this->layout = '/pdf/default';
    ...
    public function to_whom_it_may_concern($student_id = null)
    {
    ...
    private function _graduation_letter($student_id = null, $language_proficiency = null)
    {
            ...
            if (isset($this->request->data['GraduateList']['studentnumber']) && isset($this->request->data['continueLanguageProficiencyLetterPrint'])) {
                if (trim($this->request->data['GraduateList']['studentnumber']) == "") {
                    ...
                    return $this->redirect(
                        array('action' => ($language_proficiency == 1 ? 'language_proficiency' : 'to_whom_it_may_concern'))
                    );
                } else {
                    $student_detail = $this->GraduateList->Student->find('first', array(
                        'conditions' => array(
                ...
                if (empty($student_detail['GraduateList']) || $student_detail['GraduateList']['id'] == "") {
                    $this->Flash->error(
                        $student_detail['Student']['full_name'] . ' is not graduated to display ' . (strcasecmp(
                            $student_detail['Student']['gender'],
                            'male'
                        ) == 0 ? 'his' : 'her') . ' ' . ($language_proficiency == 1 ? '<u>language proficiency</u>' : '<u>to whom it may concern</u>') . ' letter.'
                    );
                } else {
                    $graduation_letter_template = ClassRegistry::init('GraduationLetter')->getGraduationLetter(
                    ...
                    if (isset($this->request->data['displayLanguageProficiencyLetterPrint']) && isset($this->request->data['GraduateList']['id'])) {
                        ...
                        $e_g_day = $this->EthiopicDateTime->GetEthiopicDay(
                            substr($g_d, 8, 2),
                            substr($g_d, 5, 2),
                            substr($g_d, 0, 4)
                        );
                        $e_g_month = $this->EthiopicDateTime->GetEthiopicMonth(
                            substr($g_d, 8, 2),
                            substr($g_d, 5, 2),
                            substr($g_d, 0, 4)
                        );
                        ...
                        $e_g_month_name = $this->EthiopicDateTime->GetEthiopicMonthName(
                            substr($g_d, 8, 2),
                            substr($g_d, 5, 2),
                            substr($g_d, 0, 4)
                        );
        ...
        if (isset($student_detail) && !empty($student_detail)) {
            ...
            if (empty($graduation_letter_template)) {
                $this->Flash->error(
                    'The system is unable to find template for ' . ($language_proficiency == 1 ? '<u>language proficiency</u>' : '<u>to whom it may concern</u>') . ' letter. Please first record language proficiency letter template for ' . $student_detail['Program']['name'] . ' program and ' . $student_detail['ProgramType']['name'] . ' program type.'
                );
            }
        }
    ...
    public function graduation_certificate($student_id = null)
    {
            ...
            if (empty($student_detail)) {
                ...
                if (!empty($student_detail['Student']['department_id']) && !in_array(
                    ...
                    if (!empty($student_detail['Student']['department_id'])) {
                        ...
                        $department_name = $this->GraduateList->CourseRegistration->Student->College->field(
                            'name',
                            array(
                                'College.id' => $student_detail['Student']['college_id']
                            )
                    ...
                    if (empty($student_detail['GraduateList']) || $student_detail['GraduateList']['id'] == "") {
                        ...
                        $graduation_certificate_template = ClassRegistry::init(
                            'GraduationCertificate'
                        )->getGraduationCertificate($student_detail['Student']['id']);
                        $graduation_certificate = $this->GraduateList->temporaryDegree(
                            $student_detail['Student']['id'],
                            "GC"
                        );
                        ...
                        if (isset($this->request->data['displayGraduationCertificatePrint']) && isset($this->request->data['GraduateList']['id'])) {
                            ...
                            $e_month = $this->EthiopicDateTime->GetEthiopicMonth(date('j'), date('n'), date('Y'));
                            $e_year = $this->EthiopicDateTime->GetEthiopicYear(date('j'), date('n'), date('Y'));
                            $e_month_name = $this->EthiopicDateTime->GetEthiopicMonthName(
                                date('j'),
                                date('n'),
                                date('Y')
                            );
                            ...
                            $graduate_date = date(
                                ...
                                mktime(
                                    ...
                                    0,
                                    substr(
                                        $graduation_certificate['student_detail']['GraduateList']['graduate_date'],
                                        5,
                                        2
                                    ),
                                    substr(
                            ...
                            $this->set(
                                compact(
                                    'graduation_certificate',
                                    'graduation_certificate_template',
                                    'e_day',
                                    'e_month',
                                    'e_year',
                                    'e_month_name',
        ...
        if (isset($student_detail) && !empty($student_detail)) {
            $graduation_certificate_template = ClassRegistry::init('GraduationCertificate')->getGraduationCertificate(
                $student_detail['Student']['id']
            );
            if (empty($graduation_certificate_template)) {
                $this->Flash->error(
                    'The system is unable to find template for the graduation certificate. Please first record graduation certificate template for ' . $student_detail['Program']['name'] . ' program and ' . $student_detail['ProgramType']['name'] . ' program type.'
                );
            }
        }
    ...
    public function mass_certificate_print()
    {

        $this->__mass_certificate_print(null, null, null, null);
    }

    public function __mass_certificate_print($program_id = null, $program_type_id = null, $department = null)
    {
        ...
        */
        ...
        if (isset($this->request->data['listStudentsForCertficatePrint'])) {
            ...
            if (isset($this->request->data['GraduateList']['graduated']) && $this->request->data['GraduateList']['graduated'] == 1) {
                $student_lists = $this->GraduateList->getStudentListGraduated(
                    ...
                    null,
                    $this->request->data['GraduateList']['studentnumber'],
                    $this->request->data['GraduateList']['name'],
                    $this->request->data['GraduateList']['graduated'],
                    $this->request->data['GraduateList']['graduate_date_from'],
                    $this->request->data['GraduateList']['graduate_date_to']
                );
            } else {
                $student_lists = $this->GraduateList->getStudentListGraduated(
                    $this->request->data['GraduateList']['acadamic_year'],
                    $program_id,
                    $program_type_id,
                    $department_id,
                    null,
        ...
        if (isset($this->request->data['getStudentCertficate'])) {
                ...
                foreach ($this->request->data['Student'] as $key => $student) {
                    ...
                    if (empty($certificate_template)) {
                        if ($this->request->data['GraduateList']['certificate_type'] == 'graduation_certificate') {
                            $certificate_template = ClassRegistry::init(
                                'GraduationCertificate'
                            )->getGraduationCertificate($student['student_id']);
                        } else {
                            if ($this->request->data['GraduateList']['certificate_type'] == 'to_whom_it_may_concern') {
                                $certificate_template = ClassRegistry::init('GraduationLetter')->getGraduationLetter(
                                    $student['student_id'],
                                    0
                                );
                            } else {
                                if ($this->request->data['GraduateList']['certificate_type'] == 'language_proficiency') {
                                    $certificate_template = ClassRegistry::init(
                                        'GraduationLetter'
                                    )->getGraduationLetter($student['student_id'], 1);
                                }
                            }
                        }
                    }
            ...
            if (empty($student_ids)) {
                ...
                if ($this->request->data['GraduateList']['certificate_type'] == 'graduation_certificate') {
                    ...
                    $graduation_certificates = $this->GraduateList->getTemporaryDegreeCertificateForMassPrint(
                        $student_ids,
                        'GC'
                    );
                    ...
                    if (!empty($graduation_certificates)) {
                        ...
                        $this->Flash->error(
                            'The system is unable to find template for graduation certificate. Please define graduation certificate first.'
                        );
                    }
                } else {
                    if ($this->request->data['GraduateList']['certificate_type'] == 'student_copy') {
                        ...
                        if ($this->request->data['GraduateList']['certificate_type'] == 'to_whom_it_may_concern') {
                            ...
                            $graduation_letters = $this->GraduateList->getTemporaryDegreeCertificateForMassPrint(
                                $student_ids,
                                'TC'
                            );
                            ...
                            if (!empty($graduation_letter_template)) {
                                ...
                                $this->Flash->error(
                                    'The system is unable to find template for "to whom it may concern" letter. Please define to whom it may concern template first.'
                                );
                            }
                        } else {
                            if ($this->request->data['GraduateList']['certificate_type'] == 'language_proficiency') {
                                ...
                                if ($this->request->data['GraduateList']['certificate_type'] == 'temporary_degree') {
                                    $temporary_degrees = $this->GraduateList->getTemporaryDegreeCertificateForMassPrint(
                                        $student_ids,
                                        'TD'
                                    );
        ...
        $departmentsss = array();
        if (isset($this->department_ids) && !empty($this->department_ids)) {
            $departmentsss = $this->GraduateList->Student->Department->find(
                'list',
    ...
    public function check_graduate()
    {
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            $isStudentValid = $this->GraduateList->Student->find(
                'count',
                array(
                    'conditions' => array(
                        'Student.studentnumber' => trim(
                            $this->request->data['GraduateList']['studentID']
                        )
                    )
                )
            );
            if ($isStudentValid > 0) {
                ...
                $this->Flash->info(
                    'The student number provided is not in our system. If you made typo error please try again else the given student number is not our student based on the admitted student data since 2012!'
                );
            }
        }
    }
    ...
    private function __label($prefix, $program_type_id, $program_id, $department_id)
    {
        ...
        if ($program_type_id == 0 && $this->Session->read(
                'Auth.User'
            )['role_id'] == ROLE_REGISTRAR && $this->Session->read('Auth.User')['is_admin'] != 1) {
        ...
        if ($program_id == 0 && $this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                'Auth.User'
            )['is_admin'] != 1) {
        ...
        if (count($college_id) > 1) {
            $namee = ClassRegistry::init('College')->find(
                'first',
        ...
        if ($this->Session->check('graduateLists_for_export')) {
            ...
            if (!empty($graduateLists)) {
                foreach ($graduateLists as $key => $graduateList) {
                    ...
                    $compactData[$key]['GraduateList']['Sex'] = strcasecmp(
                        trim($graduateList['Student']['gender']),
                        'male'
                    ) == 0 ? 'M' : (strcasecmp(trim($graduateList['Student']['gender']), 'female') == 0 ? 'F' : trim(
                        $graduateList['Student']['gender']
                    ));
                    $compactData[$key]['GraduateList']['Student ID'] = $graduateList['Student']['studentnumber'];
                    $compactData[$key]['GraduateList']['College'] = (isset($graduateList['Student']['College']) && !is_null(
                        $graduateList['Student']['College']['name']
                    ) ? $graduateList['Student']['College']['name'] : '');
                    $compactData[$key]['GraduateList']['Department'] = (isset($graduateList['Student']['Department']) && !is_null(
                        $graduateList['Student']['Department']['name']
                    ) ? $graduateList['Student']['Department']['name'] : '');
                    ...
                    $compactData[$key]['GraduateList']['Credit Type'] = (isset($graduateList['Student']['Curriculum']) && !empty($graduateList['Student']['Curriculum']['type_credit']) ? (count(
                        explode('ECTS', $graduateList['Student']['Curriculum']['type_credit'])
                    ) == 2 ? 'ECTS' : 'Credit') : '');
                    ...
                    $compactData[$key]['GraduateList']['Date Graduated'] = date_format(
                        date_create($graduateList['GraduateList']['graduate_date']),
                        "M j, Y"
                    );
            ...
            $fromDate = date_format(
                date_create(
                    $seach_data['graduate_date_from']['year'] . '-' . $seach_data['graduate_date_from']['month'] . '-' . $seach_data['graduate_date_from']['day']
                ),
                "M j Y"
            );
            $toDate = date_format(
                date_create(
                    $seach_data['graduate_date_to']['year'] . '-' . $seach_data['graduate_date_to']['month'] . '-' . $seach_data['graduate_date_to']['day']
                ),
                "M j Y"
            );
    ...
    private function __excel_file_name($prefix = '', $data)
    {
        ...
        if (count($college_id) > 1) {
            ...
            if (!empty($department_id)) {
                $namee = ClassRegistry::init('Department')->field(
                    'Department.name',
