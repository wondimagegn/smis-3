...
class SectionsController extends AppController
{
	...
	public function add()
	{
		...
		if (ROLE_COLLEGE == $this->role_id) {
			$FixedSectionName = isset($collegeshortname) ? /* 'PRE-'. */ $collegeshortname . $ETY : $collegeshortname/* 'PRE-' */ . $ETY;
		} else {
	...
	function merge_sections()
	{
		...
		if (!empty($this->request->data) && isset($this->request->data['search'])) {
			...
			if (ROLE_COLLEGE != $this->role_id) {
				$selected_year_level = $this->request->data['Section']['year_level_id'];
				$conditions = array(
					'Section.college_id' => $this->college_id, 'Section.department_id' => $this->department_id,
					'Section.program_id' => $selected_program,
				...
				$conditions = array(
					'Section.college_id' => $this->college_id, 'Section.program_id' => $selected_program, 'Section.program_type_id' => $program_type_id, 'Section.academicyear' => $selected_academic_year, 'Section.archive' => 0,
					"OR" => array("Section.department_id is null", "Section.department_id" => array(0, ''))
				);
			}
			$sections = $this->Section->find('all', array(
				'conditions' => $conditions,
				'fields' => array('Section.id', 'Section.name'),
				'contain' => array('Student' => array('fields' => array(
		...
		if (!empty($this->request->data) && isset($this->request->data['merge'])) {
			...
			if ($count_selected_sections >= 2) {
				...
				if (is_array($isMergingPossible)) {
					...
					if ($this->Section->isSectionsHaveTheSameCurriculum($this->request->data)) {
						...
						if (1) {
							...
							if (!empty($deleteArchiveSec['Archive'])) {
								...
								$studentListsNotInMergedSec = array_merge($studentListsNotInMergedSec, ClassRegistry::init('StudentsSection')->find(
									'all',
							...
							if ($saveToNesSection) {
								$this->Session->setFlash(__('<span></span> Sections ' . $mergedSectionsName . ' are merged into section ' .
									$mergedSection['Section']['name'], true), 'default', array('class' => 'success-box success-message'));
								return $this->redirect(array('action' => 'display_sections', $mergedSection_id));
							} else {
								$this->Session->setFlash(__('<span></span> Sections is not merged , please try again ', true), 'default', array('class' => 'success-box success-message'));
							}
						} else {
							// roll back
						}
					...
					if ($isMergingPossible == 2) {
						$this->Session->setFlash(
							__('<span></span> Merging of the selected
			            section is not possible because year level upgrade has been
			            performed and courses published for the sections.', true),
							'default',
							array('class' => 'error-box error-message')
						);
			            ...
			            selected section is not similar.', true),
							'default',
							array('class' => 'error-box error-message')
						);
			...
			if (ROLE_COLLEGE != $this->role_id) {
				$selected_year_level = $this->request->data['Section']['year_level_id'];
				$conditions = array(
					'Section.college_id' => $this->college_id, 'Section.department_id' =>
					$this->department_id,
					'Section.program_id' => $selected_program,
					'Section.academicyear' => $selected_academic_year,
					'Section.program_type_id' => $program_type_id,
					'Section.year_level_id' => $selected_year_level, 'Section.archive' => 0
				);
			} else {
				$conditions = array(
					'Section.college_id' => $this->college_id, 'Section.program_id' =>
					$selected_program, 'Section.program_type_id' => $program_type_id,
					'Section.academicyear' => $selected_academic_year,
					'Section.archive' => 0,
					"OR" => array("Section.department_id is null", "Section.department_id" => array(0, ''))
				);
			...
			if (!$is_course_published) {
				if (($selected_section != -1) && $current_sections_occupation[$selected_section] >= $number_of_section ) {
					...
					if ($secSave) {
						...
						$this->Flash->success('Section ' . $selected_section_name . ' is split into sections ' . $split_sections . ' successfully.');
					} else {
						$this->Flash->error('Section ' . $selected_section_name . ' not able split into sections, please try again.');
					}
	...
	function deleteStudentforThisSection($section_id = null, $student_number = null)
	{
		...
		if ($this->Section->Student->CourseRegistration->ExamResult->isStudenSectionChangePossible($student_id, $section_id)) {
			...
			if ($this->Section->Student->CourseRegistration->ExamResult->isRegistredInNameOfSectionAndSubmittedGrade($student_id, $section_id)) {
				$this->Flash->error($student_number . ' can not be deleted from  ' . $section_name . ' section since one or more course registered by the student in this section, his/her grade not full submitted.');
	...
	public function deleteStudent($section_id = null, $student_numberr = null)
	{
		...
		if ($this->Section->Student->CourseRegistration->ExamResult->isStudenSectionChangePossible($student_id, $section_id)) {
			...
			if ($this->Section->Student->CourseRegistration->ExamResult->isRegistredInNameOfSectionAndSubmittedGrade($student_id, $section_id)) {
				$this->Flash->error( $student_number . ' can not be deleted from ' . $section_name .' section since one or more course registered by the student in this section, his/her grade full submitted.');
	...
	*/
	...
	public function add_student_to_section($student_id)
	{
		...
		$error_meaasge = $this->Section->chceck_all_registered_added_courses_are_graded($student_id, $section_id = null, $check_for_invalid_grades = 1,  $from_student = '', $skip_f_grade = 1, $get_error_message = 1);
		...
		if (empty($student_detail['Section'])) {
			...
			if (!empty($student_section_exam_status['StudentExamStatus'])) {
				...
				if (isset($student_section_exam_status['StudentExamStatus']['academic_status_id']) && is_numeric($student_section_exam_status['StudentExamStatus']['academic_status_id']) && $student_section_exam_status['StudentExamStatus']['academic_status_id'] != DISMISSED_ACADEMIC_STATUS_ID) {
					...
					$studentNeedsSectionAssignment = 1;
					$possibleAcademicYears = ClassRegistry::init('StudentExamStatus')->getAcademicYearRange((isset($student_detail['CourseRegistration'][0]['academic_year']) && !empty($student_detail['CourseRegistration'][0]['academic_year']) ? $student_detail['CourseRegistration'][0]['academic_year'] :  $student_detail['AcceptedStudent']['academicyear']), $this->AcademicYear->current_academicyear());
					...
					if (count($readmitted)) {
						...
						$possibleAcademicYears = ClassRegistry::init('StudentExamStatus')->getAcademicYearRange($lastReadmittedAcademicYear, $this->AcademicYear->current_academicyear());
						$studentNeedsSectionAssignment = 1;
					}
				}
		...
		if (empty($possibleAcademicYears)) {
			$possibleAcademicYears[$this->AcademicYear->current_academicyear()] = $this->AcademicYear->current_academicyear();
		}
		...
		if (!$prefreshStudent || $studentMustHaveCurriculum) {
			...
			} else if (!empty($student_detail['CourseRegistration'][0]['year_level_id']) && !empty($student_detail['CourseRegistration'][0]['year_level_id'])) {
				$studentNeedsSectionAssignment = 1;
				...
				foreach ($this->request->data['Section'] as $k => $v) {
					if (is_numeric($k)) {
						if ($v['selected_id'] == 1) {
							...
							if ((isset($new_section_detail['YearLevel']['name']) && $new_section_detail['YearLevel']['name'] == $studentYearLevel['year'] && $this->Section->Student->StudentExamStatus->get_student_exam_status($v['student_id']) && ($student_curriculum == $section_curriculum || $section_curriculum == "nostudentinsection")) || $fresh) {
		...
		if (!empty($this->request->data)) {
			...
			if (!empty($this->request->data['Section']['assigned_section'])) {
				...
				if (count($this->request->data['Section']['assigned_section']) == 1 ) {
					...
					if (is_null($student_detail['Student']['department_id']) && $student_detail['Student']['program_id'] == PROGRAM_UNDEGRADUATE) {
						$freshmanStudent = 1;
					}
		...
		if (!empty($this->request->data) && isset($this->request->data['search'])) {
			...
			if (!empty($sections)) {
				foreach ($sections as $usk => $usv) {
					if (in_array($usv['Section']['id'], $upgradable_sections) && (!isset($usv['Section']['last_year_level_section']) || (isset($usv['Section']['last_year_level_section']) && !$usv['Section']['last_year_level_section']))) {
	...
	function get_sections_by_dept_add_drop($department_id = "", $student_id = '', $year_level_name = '', $college_id = '')
	{
		$this->layout = 'ajax';
		$sections = array();
