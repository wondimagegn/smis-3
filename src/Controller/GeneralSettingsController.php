...
class GeneralSettingsController extends AppController
{
    ...
    public function index()
    {
        ...
        if (!empty($generalSetting)) {
            foreach ($generalSetting as $ack => &$ackv) {
                $programs = $this->GeneralSetting->Program->find(
                    'list',
                    array('conditions' => array('id' => unserialize($ackv['GeneralSetting']['program_id'])))
                );
                $programTypes = $this->GeneralSetting->ProgramType->find(
                    'list',
                    array('conditions' => array('id' => unserialize($ackv['GeneralSetting']['program_type_id'])))
                );
    ...
    public function view($id = null)
    {
        ...
        $generalSetting['GeneralSetting']['program_type_id'] = unserialize(
            $generalSetting['GeneralSetting']['program_type_id']
        );
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $this->request->data['GeneralSetting']['program_id'] = serialize(
                $this->request->data['GeneralSetting']['program_id']
            );
            $this->request->data['GeneralSetting']['program_type_id'] = serialize(
                $this->request->data['GeneralSetting']['program_type_id']
            );
            ...
            $this->request->data['GeneralSetting']['program_id'] = unserialize(
                $this->request->data['GeneralSetting']['program_id']
            );
            $this->request->data['GeneralSetting']['program_type_id'] = unserialize(
                $this->request->data['GeneralSetting']['program_type_id']
            );
        }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            if ($this->GeneralSetting->check_duplicate_entry($this->request->data)) {
                ...
                $this->request->data = $this->GeneralSetting->find(
                    'first',
                    array('conditions' => array('GeneralSetting.id' => $id), 'recursive' => -1)
                );
                $this->request->data['GeneralSetting']['program_id'] = unserialize(
                    $this->request->data['GeneralSetting']['program_id']
                );
