...
class DisciplinesController extends AppController
{
        ...
        */
        ...
        if (!empty($this->request->data['Discipline']['college_id'])) {
            if (!empty($this->department_ids)) {
                $departments = $this->Discipline->Student->Department->find(
                    'list',
                    array(
                        'conditions' => array(
                            'Department.college_id' =>
                                $this->request->data['Discipline']['college_id'],
    ...
    public function view($id = null)
    {
        ...
        $discipline = $this->Discipline->find('first', array(
            'conditions' =>
                array('Discipline.id' => $id),
            'recursive' => -1
        ));
        ...
        if (!empty($this->request->data) && isset($this->request->data['saveDisplinceCase'])) {
            ...
            if ($this->Discipline->duplication($this->request->data) == 0) {
                ...
                if ($this->Discipline->save($this->request->data)) {
                    $this->Session->setFlash(
                        '<span></span>' . __('The discipline  has been saved'),
                        'default',
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __('The discipline could not be saved. Please, try again.'),
                        'default',
                        array('class' => 'error-box error-message')
                    );

                    $this->request->data['continue'] = true;
                    $student_number = $this->Discipline->Student->field(
                        'studentnumber',
                ...
                $this->Session->setFlash(
                    '<span></span>' . __('You have already recorded  discipline for the selected student.'),
                    'default',
                    array('class' => 'error-box error-message')
                );
                $this->request->data['continue'] = true;
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            ...
            if (!empty($this->request->data['Search']['studentID'])) {
                ...
                if ($check_id_is_valid > 0) {
                    ...
                    $student_id = $this->Discipline->Student->field(
                        'id',
                ...
                $this->Session->setFlash(
                    '<span></span> ' . __('Please provide student number to maintain student discipline case.'),
                    'default',
                    array('class' => 'error-box error-message')
                );
            }
    ...
    public function delete($id = null)
    {
        ...
        if ($days < 30) {
            ...
            $this->Session->setFlash(
                '<span></span>' . __('Discipline case can not be  deleted. Deletion time  expired.'),
                'default',
                array('class' => 'error-box error-message')
            );
        }
