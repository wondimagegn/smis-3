...
class ExamRoomCourseConstraintsController extends AppController
{
    ...
    public $name = 'ExamRoomCourseConstraints';
    ...
    public function add()
    {
        ...
        $departments = $this->ExamRoomCourseConstraint->PublishedCourse->Department->find(
            'list',
            array('conditions' => array('Department.college_id' => $this->college_id))
        );
        $yearLevels = null;
        $yearLevels = $this->ExamRoomCourseConstraint->PublishedCourse->YearLevel->find(
            'list',
        ...
        if (!empty($this->request->data) && isset($this->request->data['search'])) {
            ...
            switch ($this->request->data) {
                case empty($this->request->data['ExamRoomCourseConstraint']['academicyear']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the academic year of the publish courses that you want to add exam room course constraints.'
                        ),
                        'default',
                ...
                case empty($this->request->data['ExamRoomCourseConstraint']['program_id']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the program of the publish courses that you want to add exam room course constraints. '
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                ...
                case empty($this->request->data['ExamRoomCourseConstraint']['program_type_id']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the program type of the publish courses that you want to add exam room course constraints. '
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                ...
                case empty($this->request->data['ExamRoomCourseConstraint']['semester']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the semester of the publish courses that you want to add exam room course constraints. '
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
            ...
            if ($everythingfine) {
                ...
                if (empty($sections_array)) {
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'There is no course to assign exam room constraints in the selected criteria.'
                        ),
                        'default',
                    ...
                    */
                    $undergraduate_program_name = $this->ExamRoomCourseConstraint->PublishedCourse->Program->field(
                        'Program.name',
                        array('Program.id' => 1)
                    );
                    $postgraduate_program_name = $this->ExamRoomCourseConstraint->PublishedCourse->ProgramType->field(
                        'ProgramType.name',
        ...
        if (!empty($this->request->data) && isset($this->request->data['submit'])) {
            if (!empty($this->request->data['ExamRoomCourseConstraint']['courses'])) {
                if (!empty($this->request->data['ExamRoomCourseConstraint']['class_rooms'])) {
                    ...
                    $isalready_record_this_constraints = $this->ExamRoomCourseConstraint->find(
                        'count',
                        array(
                            'conditions' => array(
                                'ExamRoomCourseConstraint.published_course_id' => $this->request->data['ExamRoomCourseConstraint']['courses'],
                                'ExamRoomCourseConstraint.class_room_id' => $this->request->data['ExamRoomCourseConstraint']['class_rooms']
                            )
                        )
                    );
                    if ($isalready_record_this_constraints == 0) {
                        ...
                        if ($this->ExamRoomCourseConstraint->save($this->request->data['ExamRoomCourseConstraints'])) {
                            $this->Session->setFlash(
                                '<span></span>' . __('The exam room course constraint has been saved'),
                                'default',
                            ...
                            $this->Session->setFlash(
                                '<span></span>' . __(
                                    'The exam room course constraint could not be saved. Please, try again.'
                                ),
                                'default',
                                array('class' => 'error-box error-message')
                            );
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __(
                                'The exam room constraint for the seclected course and class room is already recored.'
                            ),
                            'default',
                            array('class' => 'error-box error-message')
                        );
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __(' Please select exam rooms'),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                }
            ...
            $classRooms = $this->ExamRoomCourseConstraint->ClassRoom->find(
                'list',
                array(
        ...
        if ($this->Session->read('selected_class_room_block')) {
            ...
            if (isset($this->request->data['ExamRoomCourseConstraint']['class_room_blocks'])) {
                $selected_class_room_block = $this->request->data['ExamRoomCourseConstraint']['class_room_blocks'];
                $this->Session->write('selected_class_room_block', $selected_class_room_block);
            }
        }
        ...
        $classRoomBlocks = $this->ExamRoomCourseConstraint->ClassRoom->ClassRoomBlock->find(
            'all',
            array(
        ...
        $publishedCourses_id_array = $this->ExamRoomCourseConstraint->PublishedCourse->find(
            'list',
        ...
        $examRoomCourseConstraints = $this->ExamRoomCourseConstraint->find(
            'all',
            array(
                'conditions' => array('ExamRoomCourseConstraint.published_course_id' => $publishedCourses_id_array),
                'contain' => array(
    ...
    public function delete($id = null, $from = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span> ' . __('Invalid id for exam room course constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
        ...
        $areyou_eligible_to_delete = $this->ExamRoomCourseConstraint->beforeDeleteCheckEligibility(
            $id,
            $this->college_id
        );
        if ($areyou_eligible_to_delete == true) {
            if ($this->ExamRoomCourseConstraint->delete($id)) {
                $this->Session->setFlash(
                    '<span></span> ' . __('Exam room course constraint deleted'),
                    'default',
                    array('class' => 'success-box success-message')
                );
                if (empty($from)) {
            ...
            $this->Session->setFlash(
                '<span></span> ' . __('Exam room course constraint was not deleted.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
            ...
            $this->Session->setFlash(
                '<span></span> ' . __('You are not eligible to delete this exam room course constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
