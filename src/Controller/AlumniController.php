...
class AlumniController extends AppController {
	...
	public $components=array('EthiopicDateTime','Paginator',
	'AcademicYear','Email'); 
	...
	public function beforeFilter(){
            ...
            $this->Auth->Allow('add',
            'alumni_survey_view','member_registration','check_alumni_survey');
    }
...
$this->Paginator->paginate('Alumnus');  
	   ...
	   if (empty($alumni) && isset($this->request->data) && !empty($this->request->data)) {
			$this->Session->setFlash('<span></span>'.__('There is no student  based on the given criteria.'),'default',array('class'=>'info-box info-message'));
		...
		if(isset($this->request->data['getAlumniQuestionnaireInExcel'])) 
		{
		  $surveyQuestions = $this->Alumnus->AlumniResponse->SurveyQuestion->find('list',array('fields'=>array('SurveyQuestion.id','SurveyQuestion.question_english')));
			$student_ids = array();		
			foreach($this->request->data['Alumnus'] as 
			$key => $student) {
			    if (is_numeric($key) 
			...
			if(empty($student_ids)) {
			 $this->Session->setFlash('<span></span>'.__('You are required to select at least one student.', true), 'default', array('class'=>'error-box error-message'));
				...
				__('Baseline Survey Questionnaire generation has experiance problem  for the selected students. Please try again.'), 'default', array('class'=>'info-box info-message'));
				}
				else {
		...
		{
		  $surveyQuestions = $this->Alumnus->AlumniResponse->SurveyQuestion->find('list',array('fields'=>array('SurveyQuestion.id',
		  ...
		  foreach($this->request->data['Alumnus'] as 
			...
			if(empty($student_ids)) {
			 $this->Session->setFlash('<span></span>'.__('You are required to select at least one student.', true), 'default', array('class'=>'error-box error-message'));
			   ...
			   if($deletedCount) {
			 $this->Session->setFlash('<span></span>'.__('You have deleted '.$deletedCount.' alumni baseline survey, please ask them to fill again.', true), 'default', array('class'=>'success-box success-message'));
	   ...
	   $programs = $this->Alumnus->Student->Program->find('list');
        ...
        'programTypes','alumni','departments','default_department_id'));
    }
	...
	public function add() {
		...
		*/
		 $student=ClassRegistry::init('Student')->find('first',array('conditions'=>array('Student.studentnumber'=>$this->Auth->user('username')),'contain'=>array('GraduateList','User')));
	    ...
	    */
		...
		if ($this->Alumnus->checkIfStudentGradutingClass($student_id)==false) {
			$this->Session->setFlash('<span></span>'.__('You can not fill alumni baseline survey due to either you are not graduting class students or department did not define the final year project as thesis or project work.'),'default',array('class'=>'warning-box warning-message'));
			 return $this->redirect('/');
		}
		...
		if ($filled1stLevelQuestionair==true) {
			$this->Session->setFlash('<span></span>'.__('Thank you for completing alumni baseline questionnair. Congratulations for your graduation and part of our alumni.'),'default',array('class'=>'success-box success-message'));
		   ...
		   $this->Session->setFlash('<span></span>'.__('You are not elegible to fill the survey. The survey is intended only for graduating  class.'),'default',array('class'=>'error-box error-message'));
			return $this->redirect(array('controller'=>'exam_grades','action' => "student_grade_view"));
		}
		...
		$this->request->data=$this->Alumnus->formatResponse($this->request->data);
			...
			if ($this->Alumnus->saveAll($this->request->data)) {
					 $this->Session->setFlash('<span></span>'.__('THANK YOU FOR YOUR PARTICIPATION!'),
			        'default',array('class'=>'success-box success-message'));
					return $this->redirect(array('controller'=>'exam_grades','action' => "student_grade_view"));
			
			} else {
				 $this->Session->setFlash('<span></span>'.__('Your response could not be saved. Please try again.'),
		...
		$regions=$this->Alumnus->Student->Region->find('list',
	...
	public function add_baselinesurvey_onbehalf() {
	   ...
	   if (!empty($this->request->data['Alumnus']['studentID'])) {
		  ...
		  if ($student_id_valid>0 ) {
			   $everythingfine=true;
			   $student_id=$this->Alumnus->Student->field('id',
			   array('studentnumber'=>
		...
		$this->Session->setFlash('<span></span> '.
		__('Please provide student number to  view profile.'),'default',array('class'=>'error-box error-message'));  		    
	   }
 	  }
	  if($everythingfine){
	  $student=ClassRegistry::init('Student')->find('first',array('conditions'=>array('Student.id'=>$student_id),'contain'=>array('GraduateList','User')));
		...
		'sexes','university','student','regions','student_id'));
	...
	public function check_alumni_survey() {
		 ...
		 if (!empty($this->request->data) && isset($this->request->data['check'])) { 
		     ...
		     $studentValid=$this->Alumnus->Student->find('first',array('conditions'=>array('Student.studentnumber'=>trim($this->request->data['Alumnus']['studentID'])),
		     'recursive'=>-1
		     ));
		     if(isset($studentValid) && !empty($studentValid)){
		     	 $alumniDetail=$this->Alumnus->find('first',array('conditions'=>array('Alumnus.student_id'=>$studentValid['Student']['id']),
		'contain'=>array('AlumniResponse'=>array('SurveyQuestion','SurveyQuestionAnswer'),'Student'=>array('GraduateList'))     	 
		     	 ));
		     	if(!isset($alumniDetail) && empty($alumniDetail)){
		     	   $this->Session->setFlash('<span></span> '.__('The student is not an alumni member and has not completed alumni survey.'),'default',array('class'=>'info-box info-message')); 
		     	} 
		     	debug($alumniDetail);
		     	 
				$surveyQuestions = $this->Alumnus->AlumniResponse->SurveyQuestion->find('all',array('contain'=>array('SurveyQuestionAnswer')));
				$student=$this->Alumnus->Student->find('first',
				array('conditions'=>array('Student.id'=>$studentValid['Student']['id']),'contain'=>array('Region','Curriculum')));	
				...
				$university=ClassRegistry::init('University')->getStudentUnivrsity($studentValid['Student']['id']);
		     	 ...
		     	 $this->set(compact('alumniDetail',
		     	 'student','regions',
		     	 'university','surveyQuestions'));
		     } else {
		        $this->Session->setFlash('<span></span> '.__('The provided student number is not valid.'),'default',array('class'=>'error-box error-message'));  
	...
	if ($this->request->is('post') && 
		isset($this->request->data['applyOnline']) && !empty($this->request->data['applyOnline'])) {
			...
			if(ClassRegistry::init('AlumniMember')->save($data)) {
		        ...
		        $Email->viewVars(array('message'=>$message));
		          ...
		          try{
						if($Email->send()) {
						$this->Session->setFlash('<span></span>'.__(" Now you are officially Arbaminch University alumni member. You will recieve information related to alumni events through your email. "),'default',array('class'=>'success-box success-message'));
				...
				$this->Session->setFlash('<span></span>'.__('The membership  could not be saved, please try again.'),'default',array('class'=>'error-box	error-message'));
		...
		$titles=array('Professor'=>'Professor','Dr.'=>'Dr.',
		'Mrs'=>'Mrs','Mr'=>'Mr','Ms'=>'Ms');
		$programs = array(
		'Degree'=>'Degree',
		'Master'=>'Master',
		...
		$institute_colleges=ClassRegistry::init('College')->find('list',array('fields'=>array('name','name'),'order'=>array('College.name ASC')));
