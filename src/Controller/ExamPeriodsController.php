...
class ExamPeriodsController extends AppController
{
    ...
    public function index()
    {
        ...
        if (empty($examPeriods)) {
            $this->Session->setFlash(
                '<span></span> ' . __(' There is no exam period in the selected criteria.'),
                'default',
        ...
        if (!empty($this->request->data)) {
            ...
            switch ($this->request->data) {
                ...
                case empty($this->request->data['ExamPeriod']['program_id']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __('Please provide final exam period program, it is required.'),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                    break;
                case empty($program_type_count) :
                    $this->Session->setFlash(
                        '<span></span> ' . __('Please select at least one program type.'),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                    break;
                case empty($year_level_count) :
                    $this->Session->setFlash(
                        '<span></span> ' . __('Please select at least one year level.'),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                    break;
            ...
            if ($everythingfine) {
                ...
                if ($this->ExamPeriod->alreadyRecorded($this->request->data)) {
                    ...
                    if ($is_saved == true) {
                        $this->Session->setFlash(
                            '<span></span>' . __('The exam period has been saved'),
                            'default',
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __('The exam period could not be saved. Please, try again.'),
                            'default',
                            array('class' => 'error-box error-message')
                        );
                    }
                    ...
                    if (isset($error['already_recorded_exam_perid'])) {
                        $this->Session->setFlash(
                            '<span></span>' . $string . ', First you have to delete or edit this exam period in.',
                            "session_flash_link",
                            array(
    ...
    public function edit($id = null)
    {
        ...
        if (!$id && empty($this->request->data)) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid exam period'),
                'default',
                array('class' => 'error-box error-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        if ($areyou_eligible_to_delete == true) {
            ...
            if (!empty($this->request->data)) {
                ...
                $isAny_ExamExcludedDateAndSessions_out_side_edited_ExamPeriod = $this->ExamPeriod->ExamExcludedDateAndSession->count_ExamExcludedDateAndSessions_out_side_edited_ExamPeriod(
                    $this->request->data['ExamPeriod']['id'],
                    $start_date,
                    $end_date
                );
                if (empty($isAny_ExamExcludedDateAndSessions_out_side_edited_ExamPeriod)) {
                    if ($this->ExamPeriod->save($this->request->data)) {
                        $this->Session->setFlash(
                            '<span></span>' . __('The exam period has been edited'),
                            'default',
                            array('class' => 'success-box success-message')
                        );
                        return $this->redirect(array('action' => 'index'));
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __(
                            'Exam period was not edited because there is one or more exam excluded date and session record in previous exam period dates which is out of the range of edited exam period dates. Thus, first delete those exam excluded date and sessions records.'
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
            ...
            $this->Session->setFlash(
                '<span></span> ' . __('You are not eligible to delete this exam period.'),
                'default',
                array('class' => 'error-box error-message')
            );
            return $this->redirect(array('action' => 'index'));
    ...
    public function delete($id = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid id for exam period'),
                'default',
                array('class' => 'error-box error-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        if ($areyou_eligible_to_delete == true) {
            ...
            $isAny_ExamExcludedDateAndSessions_in_this_ExamPeriod = $this->ExamPeriod->ExamExcludedDateAndSession->count_ExamExcludedDateAndSessions_in_this_ExamPeriod(
                $id
            );
            if (empty($isAny_ExamExcludedDateAndSessions_in_this_ExamPeriod)) {
                if ($this->ExamPeriod->delete($id)) {
                    $this->Session->setFlash(
                        '<span></span>' . __('Exam period deleted'),
                        'default',
                        array('class' => 'success-box success-message')
                    );
                    return $this->redirect(array('action' => 'index'));
                ...
                $this->Session->setFlash(
                    '<span></span>' . __('Exam period was not deleted'),
                    'default',
                    array('class' => 'error-box error-message')
                );
                return $this->redirect(array('action' => 'index'));
                ...
                $this->Session->setFlash(
                    '<span></span>' . __(
                        'Exam period was not deleted because there is one or more exam excluded date and session record in this exam period dates.First Delete Exam excluded date and sessions records that lie on this Exam period dates.'
                    ),
                    'default',
                    array('class' => 'error-box error-message')
                );
