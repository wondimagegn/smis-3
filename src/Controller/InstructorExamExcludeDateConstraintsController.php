...
class InstructorExamExcludeDateConstraintsController extends AppController
{
    ...
    public $name = 'InstructorExamExcludeDateConstraints';
    ...
    public function add()
    {
        ...
        $departments = $this->InstructorExamExcludeDateConstraint->Staff->Department->find(
            'list',
        ...
        if (!empty($this->request->data) && isset($this->request->data['search'])) {
            ...
            switch ($this->request->data) {
                case empty($this->request->data['InstructorExamExcludeDateConstraint']['academicyear']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the academic year of the exam period that you want to add instructor exam exclude date constraints.'
                        ),
                        'default',
                ...
                case empty($this->request->data['InstructorExamExcludeDateConstraint']['department_id']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the department of the exam period that you want to add instructor exam exclude date constraints. '
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                ...
                case empty($this->request->data['InstructorExamExcludeDateConstraint']['semester']) :
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'Please select the semester of the exam period that you want to add instructor exam exclude date constraints. '
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
            ...
            if ($everythingfine) {
                ...
                if (empty($instructors_list)) {
                    $this->Session->setFlash(
                        '<span></span> ' . __(
                            'There is no instructor to add  instructor exam exclude date constraints in the selected criteria.'
                        ),
                        'default',
        ...
        if (!empty($this->request->data) && isset($this->request->data['submit'])) {
            ...
            if ($count_selected_sessions != 0) {
                ...
                foreach ($selected_sessions_array as $date_key => $date_value) {
                    ...
                    foreach ($date_value as $session_key => $session_value) {
                        ...
                        if ($this->InstructorExamExcludeDateConstraint->save(
                            $this->request->data['InstructorExamExcludeDateConstraints']
                        )) {
                            $issave = true;
                        }
                    }
                ...
                if ($issave == true) {
                    $this->Session->setFlash(
                        '<span></span>' . __('The instructor exam exclude date constraint has been saved.'),
                        'default',
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __(
                            'The instructor exam exclude date constraint could not be saved. Please, try again.'
                        ),
                        'default',
                        array('class' => 'error-box error-message')
                    );
            ...
            $already_recorded_instructor_exam_excluded_date_constraints = $this->InstructorExamExcludeDateConstraint->get_already_recorded_instructor_exam_excluded_date_constraint(
            ...
            $this->set(
                compact(
                    ...
                    'selected_academicyear',
                    'selected_semester',
                    'already_recorded_instructor_exam_excluded_date_constraints',
                    'instructors_list'
                )
            );
        }
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($this->request->data)) {
            if ($this->InstructorExamExcludeDateConstraint->save($this->request->data)) {
                ...
                $this->Session->setFlash(
                    __('The instructor exam exclude date constraint could not be saved. Please, try again.')
                );
            }
        }
        if (empty($this->request->data)) {
    ...
    public function delete($id = null, $from = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span> ' . __('Invalid id for instructor exam exclude date constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
        ...
        $deleted_instructor_exam_exclude_date_constraint_data = $this->InstructorExamExcludeDateConstraint->find(
            'first',
            array('conditions' => array('InstructorExamExcludeDateConstraint.id' => $id), 'recursive' => -1)
        );
        $this->Session->write(
            'selected_academicyear',
            $deleted_instructor_exam_exclude_date_constraint_data['InstructorExamExcludeDateConstraint']['academic_year']
        );
        $this->Session->write(
            'selected_semester',
            $deleted_instructor_exam_exclude_date_constraint_data['InstructorExamExcludeDateConstraint']['semester']
        );
        if (!empty($deleted_instructor_exam_exclude_date_constraint_data['InstructorExamExcludeDateConstraint']['staff_id'])) {
            ...
            $selected_department = $this->InstructorExamExcludeDateConstraint->Staff->field(
                'Staff.department_id',
                array('Staff.id' => $selected_instructor_id)
            );
            $this->Session->write('selected_department', $selected_department);
        } else {
            if (!empty($deleted_instructor_exam_exclude_date_constraint_data['InstructorExamExcludeDateConstraint']['staff_for_exam_id'])) {
        ...
        if ($this->InstructorExamExcludeDateConstraint->delete($id)) {
            $this->Session->setFlash(
                '<span></span> ' . __('Instructor exam exclude date constraint deleted'),
                'default',
                array('class' => 'success-box success-message')
            );
            if (empty($from)) {
        ...
        $this->Session->setFlash(
            '<span></span> ' . __('Instructor exam exclude date constraint was not deleted.'),
            'default',
            array('class' => 'error-box error-message')
        );
        if (empty($from)) {
    ...
    public function _get_instructors_list($department_id = null, $academicyear = null, $semester = null)
    {
        ...
        if ($department_id == 10000) {
            $instructors_list = $this->InstructorExamExcludeDateConstraint->StaffForExam->find(
                'all',
                array(
                    'conditions' => array(
            ...
            $instructors_list = $this->InstructorExamExcludeDateConstraint->Staff->find(
                'all',
                array(
                    'conditions' => array(
    ...
    public function get_instructor_exam_exclude_date_constraints_details($data = null)
    {
    ...
    public function _get_exam_periods($academicyear = null, $semester = null)
    {

        if (!empty($academicyear) && !empty($semester)) {
            $examPeriods = ClassRegistry::init('ExamPeriod')->find(
                'all',
                array(
                    'fields' => array('ExamPeriod.start_date', 'ExamPeriod.end_date'),
                    'conditions' => array(
                        'ExamPeriod.academic_year' => $academicyear,
