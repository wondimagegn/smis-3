...
class ApplicablePaymentsController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->paginate = array(
            'order' => array('ApplicablePayment.created DESC'),
            'contain' => array('Student' => array('Department', 'College', 'ProgramType', 'Program'))
        );
        $this->__init_search();
        ...
        if (!empty($this->request->data['ApplicablePayment']['college_id'])) {
            if (!empty($this->department_ids)) {
                $departments = $this->ApplicablePayment->Student->Department->find(
                    'list',
                    array(
                        'conditions' => array(
                            'Department.college_id' =>
                                $this->request->data['ApplicablePayment']['college_id'],
    ...
    public function view($id = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid applicable payment'),
                'default',
        ...
        if (!empty($this->request->data) && isset($this->request->data['saveApplicablePayment'])) {
            ...
            if ($this->ApplicablePayment->duplication($this->request->data) == 0) {
                if ($this->ApplicablePayment->save($this->request->data)) {
                    $this->Session->setFlash(
                        '<span></span>' . __('The applicable payment has been saved'),
                        'default',
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __('The applicable payment could not be saved. Please, try again.'),
                        'default',
                        array('class' => 'error-box error-message')
                    );
                    $this->request->data['continue'] = true;
                    $student_number = $this->ApplicablePayment->Student->field(
                        'studentnumber',
                ...
                $this->Session->setFlash(
                    '<span></span>' . __(
                        'You have already recorded the applicable payment for the selected student for ' . $this->request->data['ApplicablePayment']['academic_year'] . ' of semester ' . $this->request->data['ApplicablePayment']['semester'] . '.'
                    ),
                    'default',
                    array('class' => 'error-box error-message')
                );
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            ...
            if (!empty($this->request->data['ApplicablePayment']['studentID'])) {
                ...
                if ($check_id_is_valid > 0) {
                    $everythingfine = true;
                    $student_id = $this->ApplicablePayment->Student->field(
                        'id',
    ...
    public function delete($id = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span>' . __('Invalid id for applicable payment'),
                'default',
                array('class' => 'error-box error-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        if ($this->ApplicablePayment->delete($id)) {
            $this->Session->setFlash(
                '<span></span>' . __('Applicable payment deleted'),
                'default',
                array('class' => 'success-box success-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        $this->Session->setFlash(
            '<span></span>' . __('Applicable payment was not deleted'),
            'default',
            array('class' => 'error-box error-message')
        );
        return $this->redirect(array('action' => 'index'));
    ...
    public function mass_applicablepayment_import()
    {
        ...
        if (!empty($this->request->data) && is_uploaded_file(
                $this->request->data['ApplicablePayment']['File']['tmp_name']
            )) {
            ...
            if (strcasecmp($this->request->data['ApplicablePayment']['File']['type'], 'application/vnd.ms-excel')) {
                $this->Session->setFlash(
                    '<span></span>' . __(
                        'Importing Error. Please  save your excel file as "Excel 97-2003 Workbook" type while you saved the file and import again. Try also to use other 97-2003 file types if you are using office 2010 or recent versions. Current file format is: ' . $this->request->data['ApplicablePayment']['File']['type']
                    ),
                    'default',
                    array('class' => 'error-box error-message')
                );
            ...
            if (empty($data->sheets[0]['cells'][1])) {
                $this->Session->setFlash(
                    '<span></span>' .
                    __(
                        'Importing Error. Please insert your filed name (studentnumber,tutition_fee,meal,
               accomodation, health,sponsor_type,sponsor_name,sponsor_address)  at first row of your excel file.',
                        true
                    ),
                    'default',
                    array('class' => 'error-box error-message')
            ...
            if (count($non_existing_field) > 0) {
                ...
                for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
                    ...
                    for ($j = 1; $j <= count($fields_name_applicablepayment_table); $j++) {
                        ...
                        if ($fields_name_applicablepayment_table[$j] == "studentnumber") {
                            ...
                            if (isset($currentStudentNumber)
                                ...
                                $validStudent = $this->ApplicablePayment->Student->find(
                                    'first',
                                    array(
                                        'conditions' => array(
                                ...
                                debug($validStudent);
                                debug(
                                    $this->ApplicablePayment->Student->find(
                                        'first',
                                        array(
                                            'conditions' => array(
                                                'Student.studentnumber like ' => $currentStudentNumber . '%'
                        ...
                        if ($fields_name_applicablepayment_table[$j] == "tutition_fee"
                            ...
                            $data->sheets[0]['cells'][$i][$j] != "" && !(strcasecmp(
                                    $data->sheets[0]['cells'][$i][$j],
                                    'true'
                                ) || strcasecmp($data->sheets[0]['cells'][$i][$j], 'false'))) {
                        ...
                        if ($fields_name_applicablepayment_table[$j] == "meal" &&
                            isset($data->sheets[0]['cells'][$i][$j]) && $data->sheets[0]['cells'][$i][$j] != "" && !(strcasecmp(
                                    $data->sheets[0]['cells'][$i][$j],
                                    'true'
                                ) || strcasecmp($data->sheets[0]['cells'][$i][$j], 'false'))) {
                            //TODO: Uncomment the following two lines
                                        ...
                                        if ($fields_name_applicablepayment_table[$j] == "meal") {
                                            ...
                                            if ($fields_name_applicablepayment_table[$j] == "health") {
                                                ...
                                                if ($fields_name_applicablepayment_table[$j] == "sponsor_type") {
                                                    ...
                                                    if ($fields_name_applicablepayment_table[$j] == "sponsor_address") {
                                                        $row_data['sponsor_address'] = $data->sheets[0]['cells'][$i][$j];
                                                    } else {
                                                        if ($fields_name_applicablepayment_table[$j] == "sponsor_name") {
                    ...
                    $is_duplicated = $this->ApplicablePayment->find(
                        'count',
            ...
            if (!empty($xls_data)) {
                ...
                foreach ($xls_data as $xlk => &$xlv) {
                    ...
                    $duplicationExist = $this->ApplicablePayment->find('count', array(
                        'conditions' => array(
                            'ApplicablePayment.student_id' => $xlv['ApplicablePayment']['student_id'],
                    ...
                    $validACc = $this->ApplicablePayment->Student->find('count', array(
                        'conditions' => array(
                            'Student.id' => $xlv['ApplicablePayment']['student_id'],
                ...
                if (!empty($reformat_for_saveAll['ApplicablePayment']) && $this->ApplicablePayment->saveAll(
                        $reformat_for_saveAll['ApplicablePayment'],
                        array('validate' => 'first')
                    )) {
                    $this->Session->setFlash(
                        '<span></span>Success. Imported ' . count($reformat_for_saveAll['ApplicablePayment'])
                        . ' records.',
                        'default',
                        array('class' => 'success-box success-message')
                    );
                    ...
                    if (!empty($invalidAC)) {
                        $this->Session->setFlash(
                            '<span></span>Error. You can not maintain applicable for students before they are admitted. Applicable payment academic year always greater than  student admission academic year. ',
                            'default',
                            array('class' => 'error-box error-message')
                        );
                    } else {
