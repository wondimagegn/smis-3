...
class MealAttendancesController extends AppController
{
        ...
        if (!empty($meal_hall_ids)) {
            ...
            if (!empty($this->request->data) && (isset($this->request->data['submit']) || isset($this->request->data['MealAttendance']['studentnumber']))) {
                ...
                if (!empty($studentnumber)) {
                    ...
                    $student_id = $this->MealAttendance->Student->field(
                        'Student.id',
                        array('Student.studentnumber' => $studentnumber)
                    );
                    $accepted_student_id = $this->MealAttendance->Student->field(
                        'Student.accepted_student_id',
                        array('Student.studentnumber' => $studentnumber)
                    );

                    if (!empty($student_id)) {
                        $students = $this->MealAttendance->Student->find(
                            'first',
                            array(
                                'fields' => array('Student.full_name', 'Student.studentnumber', 'Student.gender'),
                        ...
                        $is_assigned_this_meal_hall = $this->MealAttendance->Student->MealHallAssignment->find(
                            'count',
                            array(
                                'conditions' => array(
                                    'MealHallAssignment.meal_hall_id' => $meal_hall_id,
                                    'MealHallAssignment.academic_year' => $current_academicyear,
                                    'OR' => array(
                                        'MealHallAssignment.student_id' => $student_id,
                                        'MealHallAssignment.accepted_student_id' => $accepted_student_id
                                    )
                                )
                            )
                        );
                        ...
                        CostShare->field(
                            'CostShare.cafeteria_fee',
                            array(
                                'CostShare.academic_year' =>
                                    $current_academicyear,
                                'CostShare.student_id' => $student_id
                            )
                        );
                        ...
                        if ($this->MealAttendance->Student->StudentExamStatus->elegibleForService(
                            ...
                            $is_already_served = 0;
                            $is_already_served = $this->MealAttendance->find(
                                'count',
                                array(
                                    'conditions' => array(
                                        'MealAttendance.meal_type_id' => $this->request->data['MealAttendance']['meal_type_id'],
                                        'MealAttendance.student_id' => $student_id,
                                        'MealAttendance.created LIKE' => $current_date . '%'
                                    )
                                )
                            );
                            ...
                            if ($is_already_served == 0) {
                                ...
                                if ($this->MealAttendance->save($attendance)) {
                                    $this->Session->setFlash(
                                        '<span></span>' . __('The student is elegible to be serve this meal.'),
                                        'default',
                                    ...
                                    $this->Session->setFlash(
                                        '<span></span>' . __(
                                            'The meal attendance could not be saved. Please try again.'
                                        ),
                                        'default',
                                ...
                                $this->Session->setFlash(
                                    '<span></span>' . __(
                                        'The student is already served this meal. So he/she is not eligible to get addtional meal.'
                                    ),
                                    'default',
                                    array('class' => 'error-box error-message')
                                );
                            ...
                            if (isset($error['error'])) {
                                ...
                                if (empty($is_assigned_this_meal_hall)) {
                                    $this->Session->setFlash(
                                        '<span></span>' . __(
                                            'The student is not assigned in this meal hall. So he/she is not eligible to get meal service in this meal hall.'
                                        ),
                                        'default',
                                        array('class' => 'error-box error-message')
                                    );
                                    ...
                                    if ((empty($cafeteria_cost_sharing) && $exception_allowed == 3)) {
                                        $this->Session->setFlash(
                                            '<span></span>' . __(
                                                'The student didn\'t fill cafeteria cost sharing fee. So he/she is not eligible to get meal service in this meal hall.'
                                            ),
                                            'default',
                                            array('class' => 'error-box error-message')
                                        );
                                    ...
                                    if ($exception_allowed == 2) {
                                        $this->Session->setFlash(
                                            '<span></span>' . __(
                                                'The student  is not allowed to get meal service. He/she is denied the meal service by exception.'
                                            ),
                                            'default',
                                            array('class' => 'error-box error-message')
                                        );
                        ...
                        */
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __(
                                'The student is not yet admitted. So he/she is not eligible to get meal service in this meal hall.'
                            ),
                            'default',
                            array('class' => 'error-box error-message')
                        );
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __('Please provide student ID.'),
                        'default',
            ...
            $this->Session->setFlash(
                '<span></span>' . __(
                    'Currently there is no meal hall assigned to you. Please contact meal hall administrator.'
                ),
                'default',
                array('class' => 'error-box er-message')
            );
    ...
    public function meal_attendance_report()
    {
        ...
        if (!empty($selected_date)) {
            $selected_academic_year = $this->AcademicYear->get_academicyear(
                $this->request->data['Search']['date']['month'],
                $this->request->data['Search']['date']['year']
            );
        } else {
        ...
        if (!empty($this->request->data)) {
            ...
            if (!empty($selected_meal_hall_id)) {
                $meal_hall_attendances_details[$selected_meal_hall_id]['served'] = $this->MealAttendance->get_mealhall_attendance(
                    $selected_meal_hall_id,
                    $selected_academic_year,
                    $selected_formatted_date
                );
                $meal_hall_attendances_details[$selected_meal_hall_id]['total_assigned'] = $this->MealAttendance->Student->MealHallAssignment->get_mealhall_assigned_students_count(
                    $selected_academic_year,
                    $selected_meal_hall_id
                );
                $meal_hall_attendances_details[$selected_meal_hall_id]['meal_hall_name'] = $mealHalls[$selected_meal_hall_id];
                $meal_hall_attendances_details[$selected_meal_hall_id]['campus'] = $this->MealAttendance->get_meal_hall_campus(
                    $selected_meal_hall_id
                );
                //show meal attendances for the selected date of all meal hall
            } else {
                foreach ($mealHalls as $mhk => $mhv) {
                    $meal_hall_attendances_details[$mhk]['served'] = $this->MealAttendance->get_mealhall_attendance(
                        $mhk,
                        $selected_academic_year,
                        $selected_formatted_date
                    );
                    $meal_hall_attendances_details[$mhk]['total_assigned'] = $this->MealAttendance->Student->MealHallAssignment->get_mealhall_assigned_students_count(
                        $selected_academic_year,
                        $mhk
                    );
