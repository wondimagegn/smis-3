...
class CourseDropsController extends AppController {
	...
	function index()
	{
		...
		if (!empty($this->request->data)) {
			...
			if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
				...
				if (!empty($this->department_ids)) {
					...
					if (!empty($this->request->data['Search']['college_id'])) {
						...
						$options['conditions'][] = array('Student.college_id' => array_keys($colleges) , 'Student.department_id IS NULL');
					}
				}

				if ($this->request->data['Search']['status'] == 'notprocessed') {
					$options['conditions'][] = array('CourseDrop.registrar_confirmation is null');
				} else if ($this->request->data['Search']['status'] == 'accepted') {
					$options['conditions'][] = array(
						'OR' => array(
							'CourseDrop.department_approval' => 1,
			...
			if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
				$departments =  $this->CourseDrop->Student->Department->find('list', array('conditions' => array('Department.college_id' => $this->college_ids, 'Department.active' => 1)));
				$options['conditions'][] = array(
				...
				$departments =  $this->CourseDrop->Student->Department->find('list', array('conditions' => array('Department.id' => $this->department_ids)));
				$options['conditions'][] = array(
					'CourseDrop.department_approval is null',
		...
		$this->set(compact('programs', 'programTypes', 'departments', 'colleges', 'limit', 'name', 'studentnumber', 'turn_off_search', 'default_department_id', 'default_college_id'));
	}
	...
	function mass_drop()
	{
			...
			if ($everythingfine) {
				...
				if (!empty($publishedCourses)) {
					...
					if (!empty($list_of_students_registered)) {
						...
						$this->Flash->info( __('There is no student who have been registred for the published courses that need mass drop in the given criteria. Mass drop required department to publish as drop for sections.'));
					}
				} else {
					$this->Flash->info(__('There is no courses published  for the mass drop for a given  criteria.'));
					//$this->redirect(array('action'=>'index'));
				}
			}
		...
		if (!empty($this->request->data) && isset($this->request->data['massdrop'])) {
			...
			if (!empty($this->request->data['CourseDrop']['minute_number'])) {
				...
				if (count($this->request->data['CourseDrop']) > 0) {
					...
					if (count($already_dropped_courses) == count($this->request->data['CourseDrop'])) {
						$this->Flash->error(__('The courses for the sections has already mass dropped to all students registred. You do not need to drop it again.'));
						$this->redirect(array('action' => 'index'));
					} else {
						unset($this->request->data);
						$this->request->data = $already_dropped_courses;
					...
					if ($this->CourseDrop->saveAll($selected_courses_drop['CourseDrop'], array('validate' => 'first'))) {
						$this->Flash->success('The course drop has been saved');
		...
		$departments = $this->CourseDrop->CourseRegistration->PublishedCourse->Department->find('list', array(
			'conditions' => array(
			...
			if (!empty($this->request->data['CourseDrop'])) {
				if ($this->CourseDrop->saveAll($this->request->data['CourseDrop'], array('validate' => 'first'))) {
					if ($this->role_id == ROLE_DEPARTMENT) {
						$this->Flash->success( __('The course drop has been approved and notified to registrar for confirmation.'));
					} else if ($this->role_id == ROLE_REGISTRAR) {
						$this->Flash->success( __('The course drop has been confirmed.'));
					}
					$flag = true;
		...
		if ($this->role_id == ROLE_REGISTRAR) {
			...
			if (!empty($this->department_ids)) {
				if (!isset($section_organized_published_course)) {
					...
					$section_organized_published_course = $this->CourseDrop->list_course_drop_request($this->role_id, $this->department_ids, $this->AcademicYear->current_academicyear());
			...
			if (!empty($this->college_ids)) {
				if (!isset($section_organized_published_course)) {
					...
					$section_organized_published_course = $this->CourseDrop->list_course_drop_request($this->role_id, null, $this->AcademicYear->current_academicyear(), $this->college_ids);
			...
			if (!isset($section_organized_published_course)) {
				...
				$section_organized_published_course = $this->CourseDrop->list_course_drop_request($this->role_id, $this->department_id, $this->AcademicYear->current_academicyear());
			...
			if (!isset($section_organized_published_course)) {
				...
				$section_organized_published_course = $this->CourseDrop->list_course_drop_request($this->role_id, null, $this->AcademicYear->current_academicyear(), $this->college_id);
	...
	function delete($id = null)
	{
		...
		if ((isset($courseDropDetails['CourseRegistration']['ExamResult']) && !empty($courseDropDetails['CourseRegistration']['ExamResult'])) || (isset($courseDropDetails['CourseRegistration']['ExamGrade']) && !empty($courseDropDetails['CourseRegistration']['ExamGrade']))) {
			...
			if ((isset($courseDropDetails['CourseDrop']['registrar_confirmation']) && !empty($courseDropDetails['CourseDrop']['registrar_confirmation'])) || (isset($courseDropDetails['CourseDrop']['department_approval']) && !empty($courseDropDetails['CourseDrop']['department_approval']))) {
				...
				$error_message .= ' Course Drop is approved by either Registrar or Departmnt.';
				$deletion_allowed = false;
			...
			if (!isset($courseDropDetails['CourseRegistration']) || (isset($courseDropDetails['CourseRegistration']) && empty($courseDropDetails['CourseRegistration']))) {
				...
				$error_message .= ' Course Drop is no longer associated to Course Registration (Course Registration ID not found).';
				$deletion_allowed = true;
			}
			...
			if (!isset($courseDropDetails['CourseRegistration']['PublishedCourse']) || (isset($courseDropDetails['CourseRegistration']['PublishedCourse']) && empty($courseDropDetails['CourseRegistration']['PublishedCourse']))) {
				...
				$error_message .= ' Course Drop have is no longer associated to PublishedCourse (PublishedCourse ID not found).';
				$deletion_allowed = true;
			}
			...
			if (isset($courseDropDetails['CourseRegistration']['PublishedCourse']['id']) && !isset($courseDropDetails['CourseRegistration']['PublishedCourse']['Course']['id'])) {
				debug('Course Drop is no longer associated to PublishedCourse (PublishedCourse ID not found)');
				$error_message .= ' Course Drop is no longer associated to PublishedCourse (PublishedCourse ID not found).';
				$deletion_allowed = true;
			}
			...
			if (isset($courseDropDetails['CourseDrop']['forced']) && !empty($courseDropDetails['CourseDrop']['forced'])) {
				...
				$error_message .= ' Course Drop is a Forced Drop.';
				$deletion_allowed = true;
			}
