...
class ResultEntryAssignmentsController extends AppController
{
    ...
    public function index()
    {
        ...
        if (!empty($this->department_ids) || !empty($this->college_ids)) {
            $departments = $this->ResultEntryAssignment->CourseRegistration->Student->Department->allDepartmentsByCollege2(
                0,
                $this->department_ids,
                $this->college_ids
            );
        } else {
            $departments = $this->ResultEntryAssignment->CourseRegistration->Student->Department->find(
                'list',
                array('conditions' => array('Department.id' => $this->department_id), 'recursive' => -1)
            );
        }

        $programsss = $this->ResultEntryAssignment->PublishedCourse->Section->Program->find(
            'list',
            array('conditions' => array('Program.id' => $this->program_ids, 'Program.active' => 1))
        );
        $program_typesss = $this->ResultEntryAssignment->PublishedCourse->Section->ProgramType->find(
            'list',
            array('conditions' => array('ProgramType.id' => $this->program_type_ids, 'ProgramType.active' => 1))
        );
    ...
    public function assign_result_entry($published_course_id = null)
    {
        ...
        if (isset($published_course_id) && !empty($published_course_id)) {
            $pub = $this->ResultEntryAssignment->PublishedCourse->find(
                'first',
        ...
        $program_types = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->Section->ProgramType->find(
            'list'
        );
        ...
        if (isset($this->request->data['assignAddCourseGradeEntry'])) {
            ...
            if (isset($isIdExist) && !empty($isIdExist)) {
                ...
                $isRegAdd = $this->ResultEntryAssignment->isRegisteredAndAddedCourse(
                    $this->request->data['ResultEntryAssignment']['exam_published_course_id'],
                    $isIdExist['Student']['id']
                );
                $courseRegDetail = array();

                if ($isRegAdd == 0) {
                    $publishedDetail = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->find(
                        'first',
                        array(
                            'conditions' => array('PublishedCourse.id' => $this->request->data['ResultEntryAssignment']['exam_published_course_id']),
                            'recursive' => -1
                        )
                    );
                    //add course them
                ...
                if (isset($resultyEntryAssignments) && !empty($resultyEntryAssignments)) {
                    if ($this->ResultEntryAssignment->save($resultyEntryAssignments)) {
                        ...
                        return $this->redirect(
                            array(
                                'controller' => 'resultEntryAssignments',
                                'action' => 'assign_result_entry',
                                $this->request->data['ResultEntryAssignment']['exam_published_course_id']
                            )
                        );
                    }
            ...
            if (isset($this->request->data['assignGradeEntry'])) {
                ...
                if (trim($this->request->data['ResultEntryAssignment']['minute_number']) == "") {
                            ...
                            if (is_numeric($key) && $resultyentry['gp'] == 1) {
                                ...
                                if ($isRegAdd == 0) {
                                    ...
                                    if (isset($courseRegDetail) && !empty($courseRegDetail)) {
                                        ...
                                        $regDetail = $this->ResultEntryAssignment->CourseRegistration->find(
                                            'first',
                                            array(
                                                'conditions' => array(
                                        ...
                                        if (isset($regDetail) && !empty($regDetail)) {
                                            $not_processed_entry = $this->ResultEntryAssignment->find(
                                                'count',
                                                array(
                                                    'conditions' => array(
                                                        'ResultEntryAssignment.course_registration_id' => $regDetail['CourseRegistration']['id'],
                                                        'ResultEntryAssignment.course_registration_id NOT IN (SELECT course_registration_id FROM exam_grades WHERE course_registration_id IS NOT NULL)'
                                                    ),
                                                    'contain' => array()
                                                )
                                            );
                    ...
                    if (isset($resultyEntryAssignments['ResultEntryAssignment']) && !empty($resultyEntryAssignments['ResultEntryAssignment'])) {
                        if ($this->ResultEntryAssignment->saveAll($resultyEntryAssignments['ResultEntryAssignment'])) {
                            $this->Flash->success(
                                __(
                                    'The resulty entry assignment has been saved. Assigned instructor will also get notified.'
                                )
                            );
                        } else {
                            $this->Flash->error(
                                __('The resulty entry assignment  could not be saved. Please, try again.'),
                                'default',
        ...
        if (!empty($this->request->data) && isset($this->request->data['listPublishedCourses'])) {
            ...
            if (empty($publishedCourses)) {
                $this->Flash->info(
                    __(
                        'Couldn\'t find any student who is not registered for the selected course and doesn\'t have a grade submission history.'
                    )
                );
            } else {
        ...
        if (!empty($published_course_id) || (isset($this->request->data['ResultEntryAssignment']['published_course_id']) && $this->request->data['ResultEntryAssignment']['published_course_id'] != 0)) {
            ...
            $students_no_entry = $this->ResultEntryAssignment->Student->getStudentNotRegisteredPublishourse(
                $published_course_id
            );
            ...
            $sectionsHaveSameCourses = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->listSimilarPublishedCoursesForCombo(
                $published_course['PublishedCourse']['id']
            );

            $selectedPublishedCourseDetail = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->find(
                'first',
                array(
    ...
    public function delete($id = null)
    {
        ...
        $anyGradeSubmitted = $this->ResultEntryAssignment->find(
            'first',
            array('conditions' => array('ResultEntryAssignment.id' => $id), 'recursive' => -1)
        );
        ...
        if ($this->ResultEntryAssignment->delete() && empty($anyGradeSubmitted['ResultEntryAssignment']['result'])) {
            ...
            if (!empty($anyGradeSubmitted['ResultEntryAssignment']['result'])) {
    ...
    public function get_student_to_add_course($published_course_id)
    {
        ...
        if (isset($published_course_id) && !empty($published_course_id)) {
            $publishedCourseDetail = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->find(
                'first',
                array(
                    'conditions' => array('PublishedCourse.id' => $published_course_id),
                    'contain' => array(
            ...
            $sectionsHaveSameCourses = $this->ResultEntryAssignment->CourseRegistration->PublishedCourse->listSimilarPublishedCoursesForCombo(
                $publishedCourseDetail['PublishedCourse']['id']
            );
        }
    ...
    public function deleteAssignment($id = null)
    {
        ...
        if ($this->ResultEntryAssignment->delete() && empty($anyGradeSubmitted['ResultEntryAssignment']['result'])) {
            $this->Flash->success(__('The result entry assignment has been deleted.'));
        } elseif (!empty($anyGradeSubmitted['ResultEntryAssignment']['result'])) {
            $this->Flash->error(__('The result entry assignment have assesment and It could not be deleted.'));
        } else {
            $this->Flash->error(__('The result entry assignment could not be deleted. Please, try again.'));
        }
    ...
    public function __init_search()
    {
        ...
        } elseif ($this->Session->check('search_data_result_entry_assignment')) {
