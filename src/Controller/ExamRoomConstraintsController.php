...
class ExamRoomConstraintsController extends AppController
{
    ...
    public $name = 'ExamRoomConstraints';
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data) && isset($this->request->data['submit'])) {
            ...
            $date_array = $this->_get_exam_periods(
                $this->request->data['ExamRoomConstraint']['academicyear'],
                $this->request->data['ExamRoomConstraint']['semester']
            );
            ...
            if ($count_selected_sessions != 0) {
                ...
                if ($issave == true) {
                    $this->Session->setFlash(
                        '<span></span>' . __('The exam room constraint has been saved.'),
                        'default',
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __('The exam room constraint could not be saved. Please, try again.'),
                        'default',
        ...
        $classRoomBlocks = $this->ExamRoomConstraint->ClassRoom->ClassRoomBlock->find(
            'all',
            array(
        ...
        if (!empty($selected_class_room_block)) {
            $classRooms = $this->ExamRoomConstraint->ClassRoom->find(
                'list',
                array(
        ...
        if (!empty($selected_class_room) && !empty($selected_academicyear) && !empty($selected_semester)) {
            ...
            $already_recorded_exam_room_constraints_by_date = $this->ExamRoomConstraint->get_already_recorded_exam_room_constraint(
                $selected_class_room
            );
        }
        ...
        $this->set(
            compact(
                ...
                'selected_class_room',
                'date_array',
                'already_recorded_exam_room_constraints_by_date'
            )
        );
    ...
    public function delete($id = null, $from = null)
    {
        ...
        if (!$id) {
            $this->Session->setFlash(
                '<span></span> ' . __('Invalid id for exam room constraint.'),
                'default',
                array('class' => 'error-box error-message')
            );
            if (empty($from)) {
        ...
        $deleted_exam_room_constraint_data = $this->ExamRoomConstraint->find(
            'first',
            array('conditions' => array('ExamRoomConstraint.id' => $id))
        );
        $this->Session->write(
            'selected_academicyear',
            $deleted_exam_room_constraint_data['ExamRoomConstraint']['academic_year']
        );
        $this->Session->write(
            'selected_semester',
            $deleted_exam_room_constraint_data['ExamRoomConstraint']['semester']
        );
        $this->Session->write(
            'selected_class_room',
            $deleted_exam_room_constraint_data['ExamRoomConstraint']['class_room_id']
        );

        $class_room_block_id = $this->ExamRoomConstraint->ClassRoom->field(
            'ClassRoom.class_room_block_id',
            array('ClassRoom.id' => $deleted_exam_room_constraint_data['ExamRoomConstraint']['class_room_id'])
        );
        $this->Session->write('selected_class_room_block', $class_room_block_id);
        if ($this->ExamRoomConstraint->delete($id)) {
            $this->Session->setFlash(
                '<span></span> ' . __('Exam room constraint deleted'),
                'default',
                array('class' => 'success-box success-message')
            );
            if (empty($from)) {
        ...
        $this->Session->setFlash(
            '<span></span> ' . __('Exam room constraint was not deleted.'),
            'default',
            array('class' => 'error-box error-message')
        );
        if (empty($from)) {
    ...
    public function get_exam_period_and_already_recorded_data($data = null)
    {
