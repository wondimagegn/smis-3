...
class ExamTypesController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($this->MenuOptimized->check(
                $this->Auth->user(),
                'controllers/ExamTypes/add'
            ) && $this->role_id == ROLE_INSTRUCTOR) {
            return $this->redirect(array('action' => 'add'));
        } else {
            if ($this->MenuOptimized->check(
                    $this->Auth->user(),
                    'controllers/ExamTypes/exam_type_mgt_for_instructor'
                ) && $this->role_id == ROLE_DEPARTMENT) {
                return $this->redirect(array('action' => 'exam_type_mgt_for_instructor'));
            } else {
                if ($this->MenuOptimized->check(
                        $this->Auth->user(),
                        'controllers/ExamTypes/college_exam_type_mgt_for_instructor'
                    ) && $this->role_id == ROLE_COLLEGE) {
                    return $this->redirect(array('action' => 'college_exam_type_mgt_for_instructor'));
                } else {
                    $this->Flash->warning(
                        __(
                            'You don\'t have the previlage to access this area. Check your permissions in permission managent and add to your account if available, or contact system administrator for further assistance.'
                        )
                    );
    ...
    public function add($published_course_id = null)
    {
        ...
        } elseif ($this->MenuOptimized->check(
                $this->Auth->user(),
                'controllers/ExamTypes/exam_type_mgt_for_instructor'
            ) && $this->role_id == ROLE_DEPARTMENT) {
            return $this->redirect(array('action' => 'exam_type_mgt_for_instructor'));
    ...
    public function college_exam_type_mgt_for_instructor($published_course_id = null)
    {
        ...
        if ($this->role_id == ROLE_COLLEGE || (isset ($this->college_id) && !empty ($this->college_id))) {
            ...
            if (!empty ($this->request->data)) {
                $publishedCourses = $this->ExamType->PublishedCourse->CourseInstructorAssignment->listOfCoursesCollegeFreshTakingOrgBySection(
                    $this->college_id,
                    $this->request->data['ExamType']['acadamic_year'],
                    $this->request->data['ExamType']['semester'],
                    $this->request->data['ExamType']['program_id'],
                    $this->request->data['ExamType']['program_type_id']
                );
            ...
            $this->Flash->error(
                __(
                    'You need to have assigned college through college or department or instructor role to access this area. Please contact your system administrator to get college or department or instructor role.'
                )
            );
            return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
        }
    ...
    public function exam_type_mgt_for_instructor($published_course_id = null)
    {
        ...
        if ($this->role_id == ROLE_DEPARTMENT || (isset ($this->department_id) && !empty ($this->department_id))) {
            ...
            if (!empty ($this->request->data)) {
                ...
                $publishedCourses = $this->ExamType->PublishedCourse->CourseInstructorAssignment->listOfCoursesSectionsTakingOrgBySection(
                    $department_id,
                    $this->request->data['ExamType']['acadamic_year'],
                    $this->request->data['ExamType']['semester'],
                    $this->request->data['ExamType']['program_id'],
            ...
            $this->set(
                compact('programs', 'program_types', 'published_course_id', 'published_course_combo_id', 'edit')
            );
        } else {
            $this->Flash->warning(
                __(
                    'You need to have department role to access this area. Please contact your system administrator to get either department or instructor role.'
                )
            );
            return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
        }
    ...
    private function __exam_type_mgt($published_course_id = null, $sourse = 'instructor')
    {
        ...
        $instructor_id = $this->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
            'id',
        ...
        if (!empty ($published_course_id)) {
            $exam_types = $this->ExamType->find(
                'all',
                array('conditions' => array('ExamType.published_course_id' => $published_course_id), 'recursive' => -1)
            );
            $published_course_combo_id = $published_course_id;
        }
        ...
        if (!empty ($published_course_id)) {
            $instructor_id_for_checking = $this->ExamType->PublishedCourse->CourseInstructorAssignment->field(
                'staff_id',
                array(
            ...
            $assigned_instructor_user_id = $this->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                'Staff.user_id',
                array('Staff.id' => $instructor_id_for_checking)
            );
            $active_account = $this->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->User->field(
                'active',
            ...
            if (!(($this->department_id == (isset($published_course_department['Section']['Department']['id']) ? $published_course_department['Section']['Department']['id'] : "") && $active_account == 0) || ($this->college_id == (isset($published_course_department['PublishedCourse']['college_id']) ? $published_course_department['PublishedCourse']['college_id'] : "") && $active_account == 0) || ($instructor_id_for_checking == $instructor_id))) {
                $this->Flash->error(
                    __(
                        'Sorry the selected published course is not assigned to you to manage its exam type. Please select a valid published course again.'
                    )
                );
                return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
            }
            ...
            $login_instructor_id = $this->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                'id',
                array('Staff.user_id' => $this->Auth->user('id'))
            );
        ...
        if (!empty ($this->request->data)) {
            ...
            if ((isset ($course_exam_grade[0]['CourseRegistration'][0]['ExamGrade']) && count(
                        $course_exam_grade[0]['CourseRegistration'][0]['ExamGrade']
                    ) > 0) || (isset ($course_exam_grade[0]['CourseAdd'][0]['ExamGrade']) && count(
                        $course_exam_grade[0]['CourseAdd'][0]['ExamGrade']
                    ) > 0)) {
                $this->Flash->error(__('You can not apply changes on the exam setup of a grade submitted course.'));
                return $this->redirect(
                    array(
                        'action' => (strcasecmp($sourse, 'instructor') == 0 ? 'add' : 'exam_type_mgt_for_instructor'),
                        $published_course_id
                    )
                );
            }
                ...
                foreach ($this->request->data['ExamType'] as $key => &$exam_type) {
                    if (is_array($exam_type)) {
                        if (!isset ($exam_type['mandatory'])) {
                            ...
                            if ($exam_type['mandatory'] == 1) {
                                $mandatory_exam++;
                            }
                        }

                        if ($edit == 1 && isset ($exam_type['id'])) {
                            $exam_type_detail = $this->ExamType->find(
                                'first',
                                array('conditions' => array('id' => $exam_type['id']), 'recursive' => -1)
                            );
                            ...
                            if ($exam_type_detail['ExamType']['published_course_id'] != $published_course_id) {
                                ...
                                if ($exam_type['percent'] != "") {
                                    ...
                                    if ($exam_type_exam_result['ExamResult']['result'] > $exam_type['percent']) {
                                        $this->Flash->error(
                                            __(
                                                'There is/are already recorded exam result for "' . $exam_type['exam_name'] . '" and the maximum percentage can only be ' . $exam_type_exam_result['ExamResult']['result'] . '. Please delete the already recorded exam result/s to enter a larger percentage.'
                                            )
                                        );
                                        return $this->redirect(
                                            array(
                                                'action' => (strcasecmp(
                                                    $sourse,
                                                    'instructor'
                                                ) == 0 ? 'add' : 'exam_type_mgt_for_instructor'),
                                                $published_course_id
                                            )
                                        );
                                    }
                        ...
                        if (trim($exam_type['percent']) != "" && is_numeric(
                                $exam_type['percent']
                            ) && $exam_type['percent'] > 0 && $exam_type['percent'] <= 100) {
            ...
            if ($fraud) {
                $this->Flash->error(
                    __(
                        'The system encountered a problem while processing your exam setup submission. Please try your submission again.'
                    )
                );
                return $this->redirect(
                    array(
            ...
            if ($edit == 1 && !empty ($published_course_id)) {
                ...
                if (isset($deleted_exam_types) && !empty($deleted_exam_types)) {
                    foreach ($deleted_exam_types as $key => $user_deleted_exam_type_id) {
                        $exam_result_count = $this->ExamType->ExamResult->find(
                            'count',
                            array(
                        ...
                        if ($exam_result_count > 0) {
                            $exam_type_name = $this->ExamType->field(
                                'exam_name',
                                array('ExamType.id' => $user_deleted_exam_type_id)
                            );
                            $this->Flash->error(
                                __(
                                    $exam_type_name . ' exam type has exam result and could not be deleted. Please delete the exam result before you make deletion.'
                                )
                            );
                            return $this->redirect(
                                array(
            ...
            if ($duplicate_exam_type) {
                ...
                if ($percent_sum_validation && $percent_sum != 100) {
                    $this->Flash->error(__('The sum of exam percentage should be equal with 100.'));
                } else {
                    if ($percent_sum_validation && $mandatory_exam == 0) {
                        $this->Flash->error(
                            __(
                                'You are required to select at least one exam (for example final exam) as a mandatory exam.'
                            )
                        );
                    } else {
                        if ($this->ExamType->saveAll($this->request->data['ExamType'], array('validate' => 'first'))) {
                            if (!empty ($deleted_exam_types)) {
                                if (!$this->ExamType->deleteAll(array('ExamType.id' => $deleted_exam_types), false)) {
                                    $this->Flash->error(
                                        __(
                                            'Exam type entry/update is saved but deletion for exam type is interrupted. Please check your exam type entry and/or update for consistency.'
                                        )
                                    );
                                    return $this->redirect(
                                        array(
                            ...
                            return $this->redirect(
                                array('controller' => 'examResults', 'action' => 'add', $published_course_id)
                            );
                            return $this->redirect(
                                array(
                                    'action' => (strcasecmp(
        ...
        if (strcasecmp($sourse, 'instructor') == 0) {
            $publishedCourses = $this->ExamType->PublishedCourse->CourseInstructorAssignment->listOfCoursesInstructorAssignedBySection(
            ...
            if (!empty ($this->request->data) || $published_course_id) {
                $department_id = $this->department_id;
                if (empty ($this->request->data)) {
                    $published_course_detail = $this->ExamType->PublishedCourse->find(
                        'first',
    ...
    public function get_exam_type_entry_form($published_course_id = null)
    {
        ...
        if ($this->Auth->user('id')) {
            ...
            if (!empty($published_course_id)) {
                ...
                if (ENABLE_MOODLE_INTEGRATION == 1 && is_numeric(
                        ACY_BACK_FOR_MOODLE_INTEGRATION
                    ) && ACY_BACK_FOR_MOODLE_INTEGRATION) {
                    $ac_yearsForMoodle = $this->AcademicYear->academicYearInArray(
                        ((explode('/', $current_acy)[0]) - ACY_BACK_FOR_MOODLE_INTEGRATION),
                        (explode('/', $current_acy)[0])
                    );
                } else {
                    if (ENABLE_MOODLE_INTEGRATION == 1) {
                ...
                if (isset($ac_yearsForMoodle) && !empty($ac_yearsForMoodle) && !empty($published_course_department['PublishedCourse'])) {
                    if (is_array($ac_yearsForMoodle)) {
                        if (in_array(
                            $published_course_department['PublishedCourse']['academic_year'],
                            $ac_yearsForMoodle
                        )) {
                            $enable_for_moodle = 1;
                        }
                    } else {
                        if ($ac_yearsForMoodle == $published_course_department['PublishedCourse']['academic_year']) {
                            $enable_for_moodle = 1;
                        }
                    }
                    ...
                    if ($published_course_department['PublishedCourse']['enable_for_moodle']) {
                        ...
                        $enrolled_primary_instructor = ClassRegistry::init('MoodleCourseEnrollment')->find(
                            'count',
                            array(
                                'conditions' => array(
                                    'MoodleCourseEnrollment.published_course_id' => $published_course_department['PublishedCourse']['id'],
                        ...
                        $enrolled_secondary_instructor = ClassRegistry::init('MoodleCourseEnrollment')->find(
                            'count',
                            array(
                                'conditions' => array(
                                    'MoodleCourseEnrollment.published_course_id' => $published_course_department['PublishedCourse']['id'],
                        ...
                        $this->set(
                            compact('enrolled_students', 'enrolled_primary_instructor', 'enrolled_secondary_instructor')
                        );
                    }
                }
                ...
                if (1) {
                    ...
                    $course_assigned = $this->ExamType->PublishedCourse->CourseInstructorAssignment->find(
                        'count',
                        array(
    ...
    public function view($pid)
    {
        ...
        $total_registered = ClassRegistry::init('CourseRegistration')->find(
            'count',
    ...
    public function assessement_template($published_course_id = null)
    {
        ...
        if (isset ($published_course_id) && !empty ($published_course_id)) {
            ...
            if (isset ($this->request->data['listPublishedCourses'])) {
                ...
                $student_course_register_and_adds = $this->ExamType->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingPublishedCourse(
                    $published_course_id
                );
                ...
                $filename = 'Result_Import_Template_' . (str_replace(
                        ' ',
                        '_',
                        $publishedCourseDetails['Section']['name']
                    )) . '_' . $publishedCourseDetails['Course']['course_code'] . '_' . date('Y-m-d');
                $this->set(
                    compact(
                        'students',
                        'total_student_count',
            ...
            $this->Flash->error(
                __(
                    'You need to have department or instructor role to access this area. Please contact your system administrator to get either department or instructor role.'
                )
            );
            return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
        }
    ...
    public function enable_course_for_moodle($published_course_id = null)
    {
        ...
        if (!empty($published_course_id)) {
            ...
            if (empty($publishedCourse['CourseRegistration'])) {
                $this->Flash->info(
                    'No Student is registered for ' . $publishedCourse['Course']['course_title'] . ' (' . $publishedCourse['Course']['course_code'] . ') course. Please try again when students are registered for the course.'
                );
            } elseif (empty($publishedCourse['CourseInstructorAssignment'])) {
                $this->Flash->info(
                    'No Instructor is assigned for ' . $publishedCourse['Course']['course_title'] . ' (' . $publishedCourse['Course']['course_code'] . ') course. Please try again when instructor is assigned for the course.'
                );
            } else {
                ...
                if ($this->ExamType->PublishedCourse->saveField('enable_for_moodle', $enable_course)) {
                    if ($enable_course == 1) {
                        ...
                        if (!empty($publishedCourse['Course']) && !empty($publishedCourse['PublishedCourse'])) {
                            $checK_existing_moodle_course = ClassRegistry::init('MoodleCourse')->find(
                                'count',
                                array('conditions' => array('MoodleCourse.published_course_id' => $published_course_id))
                            );
                            ...
                            if (!$checK_existing_moodle_course) {
                                ...
                                $moodleCourseUpdate['MoodleCourse'][$pc_count]['course_title'] = (trim(
                                        $publishedCourse['Course']['course_title']
                                    ) . ' (' . $publishedCourse['PublishedCourse']['academic_year'] . ', ' . $publishedCourse['PublishedCourse']['semester'] . ')');
                                $moodleCourseUpdate['MoodleCourse'][$pc_count]['course_code_pid'] = (trim(
                        ...
                        if (isset($moodleCourseUpdate['MoodleCourse']) && !empty($moodleCourseUpdate['MoodleCourse'])) {
                            if (ClassRegistry::init('MoodleCourse')->saveAll(
                                $moodleCourseUpdate['MoodleCourse'],
                        ...
                        if (!empty($publishedCourse['CourseRegistration']) && count(
                                $publishedCourse['CourseRegistration']
                            ) > 0) {
                            foreach ($publishedCourse['CourseRegistration'] as $key => $registration) {
                                $checK_existing_moodle_enrollment = ClassRegistry::init('MoodleCourseEnrollment')->find(
                                    'count',
                                    array(
                                        'conditions' => array(
                                            'MoodleCourseEnrollment.published_course_id' => $published_course_id,
                                            'MoodleCourseEnrollment.username' => (isset($registration['Student']['User']['username']) ? (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($registration['Student']['User']['username']))
                                            )) : (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($registration['Student']['studentnumber']))
                                            ))),
                                        )
                                    )
                                );
                                ...
                                if (!$checK_existing_moodle_enrollment) {
                                    ...
                                    $moodleCourseEnrollmentUpdate['MoodleCourseEnrollment'][$count]['username'] = (isset($registration['Student']['User']['username']) ? (str_replace(
                                        '/',
                                        '.',
                                        strtolower(trim($registration['Student']['User']['username']))
                                    )) : (str_replace(
                                    ...
                                    $existing_moodle_user = ClassRegistry::init('MoodleUser')->find('count', array(
                                        'conditions' => array(
                                            'OR' => array(
                                                'MoodleUser.username' => (isset($registration['Student']['User']['username']) ? (str_replace(
                                                    '/',
                                                    '.',
                                                    strtolower(trim($registration['Student']['User']['username']))
                                                )) : (str_replace(
                                                ...
                                                'MoodleUser.idnumber' => (trim(
                                                    $registration['Student']['studentnumber']
                                                )),
                                                'MoodleUser.email' => (isset($registration['Student']['User']['email']) && !empty(
                                                trim(
                                                    $registration['Student']['User']['email']
                                                )
                                                ) ? (trim($registration['Student']['User']['email'])) : (!empty(
                                                trim(
                                                    $registration['Student']['email']
                                                )
                                                ) ? (trim($registration['Student']['email'])) : (str_replace(
                                                        '/',
                                                        '.',
                                                        strtolower(trim($registration['Student']['studentnumber']))
                                    ...
                                    if (!$existing_moodle_user) {
                                        if (empty(trim($registration['Student']['User']['username']))) {
                                            $moodleUserUpdate['MoodleUser'][$count]['username'] = (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($registration['Student']['studentnumber']))
                                            ));
                                            $moodleUserUpdate['MoodleUser'][$count]['password'] = (MOODLE_PASSWORD_ENCRYPRION_ALGORITHM == 'sha1' ? sha1(
                                                ($registration['Student']['first_name'] . '@' . date('Y'))
                                            ) : (MOODLE_PASSWORD_ENCRYPRION_ALGORITHM == 'md5' ? md5(
                                                ($registration['Student']['first_name'] . '@' . date('Y'))
                                            ) : $registration['Student']['first_name'] . '@' . date('Y')));
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['firstname'] = (!empty(
                                        trim(
                                            $registration['Student']['first_name']
                                        )
                                        ) ? (ucfirst(
                                            strtolower(trim($registration['Student']['first_name']))
                                        )) : (ucfirst(
                                            strtolower(trim($registration['Student']['User']['first_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['lastname'] = (!empty(
                                        trim(
                                            $registration['Student']['middle_name']
                                        )
                                        ) ? (ucfirst(
                                            strtolower(trim($registration['Student']['middle_name']))
                                        )) : (ucfirst(
                                            strtolower(trim($registration['Student']['User']['middle_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['middlename'] = (!empty(
                                        trim(
                                            $registration['Student']['last_name']
                                        )
                                        ) ? (ucfirst(
                                            strtolower(trim($registration['Student']['last_name']))
                                        )) : (ucfirst(
                                            strtolower(trim($registration['Student']['User']['last_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['email'] = (!empty(
                                        trim(
                                            $registration['Student']['email']
                                        )
                                        ) ? (trim($registration['Student']['email'])) : (!empty(
                                        trim(
                                            $registration['Student']['User']['email']
                                        )
                                        ) ? (trim($registration['Student']['User']['email'])) : (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($registration['Student']['User']['username']))
                                            ) . INSTITUTIONAL_EMAIL_SUFFIX)));
                                        $moodleUserUpdate['MoodleUser'][$count]['institution'] = (!empty(
                                        trim(
                                            $registration['Student']['College']['name']
                                        )
                                        ) ? (trim($registration['Student']['College']['name'])) : (Configure::read(
                                            'CompanyName'
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['department'] = (!empty(
                                        trim(
                                            $registration['Student']['Department']['name']
                                        )
                                        ) ? (trim($registration['Student']['Department']['name'])) : 'N/A');
                                        $moodleUserUpdate['MoodleUser'][$count]['idnumber'] = (!empty(
                                        trim(
                                            $registration['Student']['studentnumber']
                                        )
                                        ) ? (trim($registration['Student']['studentnumber'])) : (!empty(
                                        trim(
                                            $registration['Student']['email']
                                        )
                                        ) ? (trim($registration['Student']['email'])) : (!empty(
                                        ...
                                        ) ? (trim($registration['Student']['User']['email'])) : ((strtolower(
                                                trim($registration['Student']['User']['first_name'])
                                            )) . '.' . (strtolower(
                                                trim($registration['Student']['User']['middle_name'])
                                            )) . INSTITUTIONAL_EMAIL_SUFFIX))));
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['mobile'] = (!empty(
                                        trim(
                                            $registration['Student']['phone_mobile']
                                        )
                                        ) ? (trim($registration['Student']['phone_mobile'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['phone'] = (!empty(
                                        trim(
                                            $registration['Student']['phone_home']
                                        )
                                        ) ? (trim($registration['Student']['phone_home'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['address'] = (!empty(
                                        trim(
                                            $registration['Student']['College']['name']
                                        )
                                        ) ? (trim(
                                            $registration['Student']['College']['Campus']['name']
                                        )) : (Configure::read('CompanyName')));
                                        ...
                                        if (!empty($otps_to_save) && !in_array(
                                                $registration['Student']['id'],
                                                array_keys($otps_to_save)
                                            )) {
                                            $otps_to_save[$registration['Student']['id']] = array(
                                                ...
                                                'password' => ($registration['Student']['first_name'] . '@' . date(
                                                        'Y'
                                                    )),
                            ...
                            foreach ($publishedCourse['CourseAdd'] as $key => $add) {
                                $checK_existing_moodle_enrollment = ClassRegistry::init('MoodleCourseEnrollment')->find(
                                    ...
                                    array(
                                        'conditions' => array(
                                            'MoodleCourseEnrollment.published_course_id' => $published_course_id,
                                            'MoodleCourseEnrollment.username' => (isset($add['Student']['User']['username']) && !empty(
                                            trim(
                                                $add['Student']['User']['username']
                                            )
                                            ) ? (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($add['Student']['User']['username']))
                                            )) : (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($add['Student']['studentnumber']))
                                            ))),
                                        )
                                    )
                                );
                                ...
                                if (!$checK_existing_moodle_enrollment) {
                                    ...
                                    $moodleCourseEnrollmentUpdate['MoodleCourseEnrollment'][$count]['username'] = (isset($add['Student']['User']['username']) ? (str_replace(
                                        '/',
                                        '.',
                                        strtolower(trim($add['Student']['User']['username']))
                                    )) : (str_replace('/', '.', strtolower(trim($add['Student']['studentnumber'])))));
                                    $moodleCourseEnrollmentUpdate['MoodleCourseEnrollment'][$count]['user_role'] = 'student';
                                    ...
                                    $existing_moodle_user = ClassRegistry::init('MoodleUser')->find('count', array(
                                        'conditions' => array(
                                            'OR' => array(
                                                'MoodleUser.username' => (str_replace(
                                                    '/',
                                                    '.',
                                                    strtolower(trim($add['Student']['studentnumber']))
                                                )),
                                                'MoodleUser.idnumber' => trim($add['Student']['studentnumber']),
                                                'MoodleUser.email' => (isset($add['Student']['User']['email']) && !empty(
                                                trim(
                                                    $add['Student']['User']['email']
                                                )
                                                ) ? (trim($add['Student']['User']['email'])) : (!empty(
                                                trim(
                                                    $add['Student']['email']
                                                )
                                                ) ? (trim($add['Student']['email'])) : (str_replace(
                                                        '/',
                                                        '.',
                                                        strtolower(trim($add['Student']['studentnumber']))
                                                    ) . INSTITUTIONAL_EMAIL_SUFFIX)))
                                    ...
                                    if (!$existing_moodle_user) {
                                        ...
                                        ) ? (ucfirst(strtolower(trim($add['Student']['first_name'])))) : (ucfirst(
                                            strtolower(trim($add['Student']['User']['first_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['lastname'] = (!empty(
                                        trim(
                                            $add['Student']['middle_name']
                                        )
                                        ) ? (ucfirst(strtolower(trim($add['Student']['middle_name'])))) : (ucfirst(
                                            strtolower(trim($add['Student']['User']['middle_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['middlename'] = (!empty(
                                        trim(
                                            $add['Student']['last_name']
                                        )
                                        ) ? (ucfirst(strtolower(trim($add['Student']['last_name'])))) : (ucfirst(
                                            strtolower(trim($add['Student']['User']['last_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['email'] = (!empty(
                                        trim(
                                            $add['Student']['email']
                                        )
                                        ) ? (trim($add['Student']['email'])) : (!empty(
                                        trim(
                                            $add['Student']['User']['email']
                                        )
                                        ) ? (trim($add['Student']['User']['email'])) : (str_replace(
                                                '/',
                                                '.',
                                                strtolower(trim($add['Student']['User']['username']))
                                            ) . INSTITUTIONAL_EMAIL_SUFFIX)));
                                        ...
                                        ) ? (trim($add['Student']['College']['name'])) : (Configure::read(
                                            'CompanyName'
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['department'] = (!empty(
                                        trim(
                                            $add['Student']['Department']['name']
                                        )
                                        ) ? (trim($add['Student']['Department']['name'])) : 'N/A');
                                        $moodleUserUpdate['MoodleUser'][$count]['idnumber'] = (!empty(
                                        trim(
                                            $add['Student']['studentnumber']
                                        )
                                        ) ? (trim($add['Student']['studentnumber'])) : (!empty(
                                        trim(
                                            $add['Student']['email']
                                        )
                                        ) ? (trim($add['Student']['email'])) : (!empty(
                                        ...
                                        ) ? (trim($add['Student']['User']['email'])) : ((strtolower(
                                                trim($add['Student']['User']['first_name'])
                                            )) . '.' . (strtolower(
                                                trim($add['Student']['User']['middle_name'])
                                            )) . INSTITUTIONAL_EMAIL_SUFFIX))));
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['mobile'] = (!empty(
                                        trim(
                                            $add['Student']['phone_mobile']
                                        )
                                        ) ? (trim($add['Student']['phone_mobile'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['phone'] = (!empty(
                                        trim(
                                            $add['Student']['phone_home']
                                        )
                                        ) ? (trim($add['Student']['phone_home'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['address'] = (!empty(
                                        trim(
                                            $add['Student']['College']['name']
                                        )
                                        ) ? (trim($add['Student']['College']['Campus']['name'])) : (Configure::read(
                                            'CompanyName'
                                        )));
                        ...
                        if (!empty($publishedCourse['CourseInstructorAssignment']) && count(
                            ...
                            foreach ($publishedCourse['CourseInstructorAssignment'] as $key => $inst_ass) {
                                $checK_existing_moodle_enrollment = ClassRegistry::init('MoodleCourseEnrollment')->find(
                                    ...
                                    array(
                                        'conditions' => array(
                                            ...
                                            'MoodleCourseEnrollment.username' => (!empty(
                                            trim(
                                                $inst_ass['Staff']['email']
                                            )
                                            ) ? (trim($inst_ass['Staff']['email'])) : (!empty(
                                            trim(
                                                $inst_ass['Staff']['User']['email']
                                            )
                                            ) ? (trim($inst_ass['Staff']['User']['email'])) : ((strtolower(
                                                    trim($inst_ass['Staff']['first_name'])
                                                )) . '.' . (strtolower(
                                                    trim($inst_ass['Staff']['middle_name'])
                                                )) . INSTITUTIONAL_EMAIL_SUFFIX)))
                                        )
                                    )
                                );
                                ...
                                if (!$checK_existing_moodle_enrollment) {
                                    ...
                                    $moodleCourseEnrollmentUpdate['MoodleCourseEnrollment'][$count]['username'] = (!empty(
                                    trim(
                                        $inst_ass['Staff']['email']
                                    )
                                    ) ? (trim($inst_ass['Staff']['email'])) : (!empty(
                                    ...
                                    if (!$existing_moodle_user) {
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['username'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['email']
                                        )
                                        ) ? (trim($inst_ass['Staff']['email'])) : (!empty(
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['firstname'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['first_name']
                                        )
                                        ) ? (ucfirst(strtolower(trim($inst_ass['Staff']['first_name'])))) : (ucfirst(
                                            strtolower(trim($inst_ass['Staff']['User']['first_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['lastname'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['middle_name']
                                        )
                                        ) ? (ucfirst(strtolower(trim($inst_ass['Staff']['middle_name'])))) : (ucfirst(
                                            strtolower(trim($inst_ass['Staff']['User']['middle_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['middlename'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['last_name']
                                        )
                                        ) ? (ucfirst(strtolower(trim($inst_ass['Staff']['last_name'])))) : (ucfirst(
                                            strtolower(trim($inst_ass['Staff']['User']['last_name']))
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['email'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['email']
                                        ...
                                        trim(
                                            $inst_ass['Staff']['User']['email']
                                                ...
                                                trim($inst_ass['Staff']['User']['first_name'])
                                            )) . '.' . (strtolower(
                                                trim($inst_ass['Staff']['User']['middle_name'])
                                            )) . INSTITUTIONAL_EMAIL_SUFFIX)));
                                        $moodleUserUpdate['MoodleUser'][$count]['institution'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['College']['name']
                                        )
                                        ) ? (trim($inst_ass['Staff']['College']['name'])) : (Configure::read(
                                            'CompanyName'
                                        )));
                                        $moodleUserUpdate['MoodleUser'][$count]['department'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['Department']['name']
                                        )
                                        ) ? (trim($inst_ass['Staff']['Department']['name'])) : 'N/A');
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['idnumber'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['staffid']
                                        )
                                        ) ? (trim($inst_ass['Staff']['staffid'])) : null);
                                        ...
                                        $moodleUserUpdate['MoodleUser'][$count]['description'] = (!empty($inst_ass['Staff']['Position']['position']) ? ($inst_ass['Staff']['Position']['position'] . ' at ' . (!empty(
                                            trim(
                                                $inst_ass['Staff']['Department']['name']
                                            )
                                            ) ? (trim(
                                                    $inst_ass['Staff']['Department']['name']
                                                )) . ' ' . $inst_ass['Staff']['Department']['type'] : (Configure::read(
                                                'CompanyName'
                                            )))) : ('Staff member of ' . (Configure::read('CompanyName'))));
                                        $moodleUserUpdate['MoodleUser'][$count]['mobile'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['phone_mobile']
                                        )
                                        ) ? (trim($inst_ass['Staff']['phone_mobile'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['phone'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['phone_office']
                                        )
                                        ) ? (trim($inst_ass['Staff']['phone_office'])) : null);
                                        $moodleUserUpdate['MoodleUser'][$count]['address'] = (!empty(
                                        trim(
                                            $inst_ass['Staff']['College']['name']
                                        )
                                        ) ? (trim($inst_ass['Staff']['College']['Campus']['name'])) : (Configure::read(
                                            'CompanyName'
                                        )));
                        ...
                        if (!empty($moodleUserUpdate['MoodleUser'])) {
                            if (ClassRegistry::init('MoodleUser')->saveAll(
                                $moodleUserUpdate['MoodleUser'],
                                array('validate' => 'first')
                            )) {
                        ...
                        $checK_existing_moodle_enrollment = ClassRegistry::init('MoodleCourseEnrollment')->find(
                            'count',
                            array('conditions' => array('MoodleCourseEnrollment.published_course_id' => $published_course_id))
                        );

                        if ($checK_existing_moodle_enrollment) {
                            if (ClassRegistry::init('MoodleCourseEnrollment')->deleteAll(
                                array('MoodleCourseEnrollment.published_course_id' => $published_course_id),
                                false
                            )) {
                                $existing_moodle_course = ClassRegistry::init('MoodleCourse')->find(
                                    'count',
                                    array('conditions' => array('MoodleCourse.published_course_id' => $published_course_id))
                                );
                                if ($existing_moodle_course) {
                                    if (ClassRegistry::init('MoodleCourse')->deleteAll(
                                        array('MoodleCourse.published_course_id' => $published_course_id),
                                        false
                                    )) {
                                        $this->Flash->success(
                                            'Deleted ' . $publishedCourse['Course']['course_title'] . ' (' . $publishedCourse['Course']['course_code'] . ')  course from ' . MOODLE_SITE_URL . ' and unenrolled all ' . ($checK_existing_moodle_enrollment) . ' users .'
                                        );
                                    }
                                }
                            }
                            ...
                            if ($existing_moodle_course) {
                                ...
                                $this->Flash->success(
                                    'Disabled ' . $publishedCourse['Course']['course_title'] . ' (' . $publishedCourse['Course']['course_code'] . ')  course from ' . MOODLE_SITE_URL . '.'
                                );
                            }
                        }
                    }
            ...
            return $this->redirect(
                array(
                    'action' => ($this->Session->read(
                        'Auth.User'
                    )['role_id'] == ROLE_INSTRUCTOR ? 'add' : 'exam_type_mgt_for_instructor'),
                    /* $published_course_id */
                )
            );
        }
    ...
    public function sync_new_enrollments($published_course_id = null)
    {
        ...
        if (!empty($published_course_id)) {
            ...
            $existing_moodle_course_enrollment_student_ids_to_exclude = ClassRegistry::init('MoodleUser')->find(
                'list',
                array(
                    'conditions' => array(
            ...
            $existing_moodle_course_enrollment_staff_ids_to_exclude = ClassRegistry::init('MoodleUser')->find(
                'list',
                array(
                    'conditions' => array(
            ...
            $publishedCourse = $this->ExamType->PublishedCourse->find('first', array(
                ...
                'contain' => array(
                    'CourseInstructorAssignment' => array(
                        'conditions' => array(
                            'CourseInstructorAssignment.staff_id NOT IN (' . join(
                                ',',
                                $existing_moodle_course_enrollment_staff_ids_to_exclude
                            ) . ')'
                        ),
                        'Staff' => array(
                            'Department' => array(
                    ...
                    'CourseRegistration' => array(
                        'conditions' => array(
                            'CourseRegistration.student_id NOT IN (' . join(
                                ',',
                                $existing_moodle_course_enrollment_student_ids_to_exclude
                            ) . ')'
                        ),
                        'Student' => array(
                            'Department' => array(
                    ...
                    'CourseAdd' => array(
                        'conditions' => array(
                            ...
                            'CourseAdd.student_id NOT IN (' . join(
                                ',',
                                $existing_moodle_course_enrollment_student_ids_to_exclude
                            ) . ')'
                        ),
            ...
            if (empty($publishedCourse['CourseRegistration']) && empty($publishedCourse['CourseInstructorAssignment']) && empty($publishedCourse['CourseAdd'])) {
                $this->Flash->info(
                    'No new uses to sync.. You can try again later if you think there are students left to sync when they are registred or add your course.'
                );
            } else {
                if ($publishedCourse['PublishedCourse']['enable_for_moodle']) {
                        ...
                        foreach ($publishedCourse['CourseRegistration'] as $key => $registration) {
                            ...
                            if (!$checK_existing_moodle_enrollment) {
                                ...
                                if (!$existing_moodle_user) {
                                    ...
                                    ) ? (ucfirst(strtolower(trim($registration['Student']['first_name'])))) : (ucfirst(
                                        strtolower(trim($registration['Student']['User']['first_name']))
                                    )));
                                    $moodleUserUpdate['MoodleUser'][$count]['lastname'] = (!empty(
                                    trim(
                                    ...
                                    ) ? (ucfirst(strtolower(trim($registration['Student']['middle_name'])))) : (ucfirst(
                                        strtolower(trim($registration['Student']['User']['middle_name']))
                                    )));
                                    $moodleUserUpdate['MoodleUser'][$count]['middlename'] = (!empty(
                                    trim(
                                    ...
                                    ) ? (ucfirst(strtolower(trim($registration['Student']['last_name'])))) : (ucfirst(
                                        strtolower(trim($registration['Student']['User']['last_name']))
                                    )));
                                    $moodleUserUpdate['MoodleUser'][$count]['email'] = (!empty(
                                    trim(
                        ...
                        foreach ($publishedCourse['CourseAdd'] as $key => $add) {
                            ...
                            if (!$checK_existing_moodle_enrollment) {
                                ...
                                if (!$existing_moodle_user) {
                                    ...
                                    ) ? (trim($add['Student']['College']['name'])) : (Configure::read('CompanyName')));
                                    $moodleUserUpdate['MoodleUser'][$count]['department'] = (!empty(
                                    trim(
                                        $add['Student']['Department']['name']
                                    )
                                ...
                                $existing_moodle_user = ClassRegistry::init('MoodleUser')->find('count', array(
                                    'conditions' => array(
                                        'OR' => array(
                                            'MoodleUser.username' => $inst_ass['Staff']['User']['username'],
                                            //'MoodleUser.idnumber' => $inst_ass['Staff']['staffid'],
                                            'MoodleUser.email' => (!empty(trim($inst_ass['Staff']['email'])) ? (trim(
                                                $inst_ass['Staff']['email']
                                            )) : (!empty(trim($inst_ass['Staff']['User']['email'])) ? (trim(
                                                $inst_ass['Staff']['User']['email']
                                            )) : ((strtolower(
                                                    trim($inst_ass['Staff']['first_name'])
                                                )) . '.' . (strtolower(
                    ...
                    if (!empty($moodleUserUpdate['MoodleUser'])) {
                        if (ClassRegistry::init('MoodleUser')->saveAll(
                            ...
                            //$this->Flash->success('Created ' . (count($moodleUserUpdate['MoodleUser'])) . ' new accounts.');
                            $message_to_display .= 'Created ' . (count(
                                    $moodleUserUpdate['MoodleUser']
                                )) . ' new accounts,';
                            if (!empty($otps_to_save)) {
                                ClassRegistry::init('Otp')->saveAll($otps_to_save, array('validate' => 'first'));
                            ...
                            $message_to_display .= 'Failed to create ' . (count(
                                    $moodleUserUpdate['MoodleUser']
                                )) . ' new accounts,';
                        }
                    }
