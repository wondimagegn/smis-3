...
class ExamResultsController extends AppController
{
    ...
    public function beforeRender(Event $event)
    {
        ...
        if ($this->role_id == ROLE_INSTRUCTOR) {
            $acyear_array_data = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find(
                'list',
                array(
                    'conditions' => array(
                        'CourseInstructorAssignment.staff_id in (select id from staffs where user_id="' . $this->Auth->user(
                            'id'
                        ) . '")'
                    ),
                    'fields' => array(
    ...
    public function __grade_change_request($source = 'instructor')
    {
        ...
        if (isset($this->request->data['ExamResult']['published_course_id'])) {
            ...
            $student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingPublishedCourse(
                $published_course_id
            );
            ...
            if (!empty($student_course_register_and_adds)) {
                for ($i = 0; $i <= (count($students) + count($student_adds) + count($student_makeup)); $i++) {
                    if (isset($this->request->data['grade_change_request_' . $i])) {
                        ...
                        $days_available_for_grade_change = ClassRegistry::init(
                            'AcademicCalendar'
                        )->daysAvaiableForGradeChange();
                        $days_available_for_grade_change = ClassRegistry::init(
                            'GeneralSetting'
                        )->daysAvaiableForGradeChange($published_course_id);
                        //Considering makeup exam grade submission
                        $date_grade_submited = ClassRegistry::init(
                            'AcademicCalendar'
                        )->getPublishedCourseGradeSubmissionDate($published_course_id);
						...
						*/
                        ...
                        if (isset($exam_grade_detail['CourseRegistration']) && !empty($exam_grade_detail['CourseRegistration']) && $exam_grade_detail['CourseRegistration']['id'] != "") {
                            $any_grade_on_process = $this->ExamResult->CourseRegistration->isAnyGradeOnProcess(
                                $exam_grade_detail['CourseRegistration']['id']
                            );
                            $last_grade = $this->ExamResult->CourseRegistration->getCourseRegistrationLatestGrade(
                                $exam_grade_detail['CourseRegistration']['id']
                            );
                        } else {
                            $any_grade_on_process = $this->ExamResult->CourseAdd->isAnyGradeOnProcess(
                                $exam_grade_detail['CourseAdd']['id']
                            );
                            $last_grade = $this->ExamResult->CourseAdd->getCourseRegistrationLatestGrade(
                                $exam_grade_detail['CourseAdd']['id']
                            );
                        }

                        $published_course_department = $this->ExamResult->CourseRegistration->PublishedCourse->find(
                            'first',
                            array(
                        ...
                        $instructor_id = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                            'id',
                        ...
                        $instructor_active = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                            'active',
                        ...
                        if (!empty($exam_grade_detail['CourseRegistration']['id']) && $exam_grade_detail['CourseRegistration']['id'] > 0) {
                            $makeup_exam_detail = $this->ExamResult->ExamType->PublishedCourse->CourseRegistration->MakeupExam->find(
                                'first',
                                array(
                                    'conditions' => array(
                                        'MakeupExam.course_registration_id' => $exam_grade_detail['CourseRegistration']['id'],
                            ...
                            if (!empty($exam_grade_detail['CourseAdd']['id']) && $exam_grade_detail['CourseAdd']['id'] > 0) {
                                $makeup_exam_detail = $this->ExamResult->ExamType->PublishedCourse->CourseRegistration->MakeupExam->find(
                                    'first',
                                    array(
                                        'conditions' => array(
                        ...
                        if (!empty($exam_grade_detail) && ($this->Session->read(
                                    'Auth.User'
                                )['role_id'] == ROLE_INSTRUCTOR || (($this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_DEPARTMENT || $this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_COLLEGE) && !$instructor_active))) {
                            if ((/* !isset($exam_grade_detail['CourseRegistration']) || empty($exam_grade_detail['CourseRegistration']) ||  (isset($exam_grade_detail['CourseRegistration']['id']) && empty($exam_grade_detail['CourseRegistration']['id'])) || */ (isset($exam_grade_detail['CourseRegistration']['published_course_id']) && !empty($exam_grade_detail['CourseRegistration']['published_course_id']) && $published_course_id == $exam_grade_detail['CourseRegistration']['published_course_id']) || (!empty($instructor_id_for_checking) && $instructor_id == $instructor_id_for_checking) || ($this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_DEPARTMENT && $this->department_id == $published_course_department['Section']['department_id']) || ($this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_COLLEGE && empty($published_course_department['Section']['department_id']) && $this->college_id == $published_course_department['Section']['college_id'])) &&
                                (/* !isset($exam_grade_detail['CourseAdd']) || empty($exam_grade_detail['CourseAdd']) || (isset($exam_grade_detail['CourseAdd']['id']) && empty($exam_grade_detail['CourseAdd']['id'])) || */ (!empty($instructor_id_for_checking) && $instructor_id == $instructor_id_for_checking) || (isset($exam_grade_detail['CourseAdd']['published_course_id']) && !empty($exam_grade_detail['CourseAdd']['published_course_id']) && $published_course_id == $exam_grade_detail['CourseAdd']['published_course_id']) || ($this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_DEPARTMENT && $this->department_id == $published_course_department['Section']['department_id']) || ($this->Session->read(
                                            'Auth.User'
                                        )['role_id'] == ROLE_COLLEGE && empty($published_course_department['Section']['department_id']) && $this->college_id == $published_course_department['Section']['college_id'])) &&
                                ...
                                ) && $exam_grade_detail['ExamGrade']['department_approval'] == 1 && $exam_grade_detail['ExamGrade']['registrar_approval'] == 1 && $grade_change_deadline >= date(
                                    'Y-m-d H:i:s'
                                ) && !$any_grade_on_process
                            ) {
                                if ($this->request->data['ExamGradeChange'][$i]['result'] == "" || !is_numeric(
                                        $this->request->data['ExamGradeChange'][$i]['result']
                                    ) || $this->request->data['ExamGradeChange'][$i]['result'] < 0 || $this->request->data['ExamGradeChange'][$i]['result'] > 100 || trim(
                                        $this->request->data['ExamGradeChange'][$i]['reason']
                                    ) == "") {
                                    $this->Flash->error(
                                        __(
                                            'The system unable to process your grade change request due to incomplete data. Please make sure that you submited exam result, reason for change and the new grade is different from the previous one.'
                                        )
                                    );
                                } else {
                                    $this->request->data['ExamGradeChange'][$i]['grade'] = $this->ExamResult->getTotalResultGrade(
                                        $this->request->data['ExamGradeChange'][$i]['result'],
                                        $published_course_id
                                    );
                                    ...
                                    if ($last_grade != $this->request->data['ExamGradeChange'][$i]['grade']) {
                                        ...
                                        if ($this->ExamResult->CourseRegistration->ExamGrade->ExamGradeChange->save(
                                            $this->request->data['ExamGradeChange'][$i],
                                            array('validate' => false)
                                        )) {
                                            //Retrieving student detail to get his/her name
                                            if (isset($exam_grade_detail['CourseRegistration']) && !empty($exam_grade_detail['CourseRegistration']) && $exam_grade_detail['CourseRegistration']['id'] != "") {
                                                $student_detail = $this->ExamResult->CourseRegistration->find(
                                                    'first',
                                                    array(
                                                        'conditions' => array('CourseRegistration.id' => $exam_grade_detail['CourseRegistration']['id']),
                                                        'contain' => array('Student')
                                                    )
                                                );
                                            } else {
                                                $student_detail = $this->ExamResult->CourseAdd->find(
                                                    'first',
                                                    array(
                                                        'conditions' => array('CourseAdd.id' => $exam_grade_detail['CourseAdd']['id']),
                                                        'contain' => array('Student')
                                                    )
                                                );
                                            }
                                            ...
                                            $this->Flash->success(
                                                __(
                                                    'Grade change request for ' . ($student_detail['Student']['first_name'] . ' ' . $student_detail['Student']['middle_name'] . ' ' . $student_detail['Student']['last_name'] . ' (' . $student_detail['Student']['studentnumber'] . ')') . ' is sent to ' . (!empty($student_detail['Student']['department_id']) ? 'department' : 'freshman program') . ' for approval.'
                                                )
                                            );
                                        } else {
                                            $this->Flash->error(
                                                __(
                                                    'The system is unable to process your grade change request. Please try again.'
                                                )
                                            );
                                        }
                                        ...
                                        $this->Flash->error(
                                            __(
                                                'There is no difference between the already recorded and new grade. Please make sure that you submitted a different grade.'
                                            )
                                        );
                                    }
                                }
                                ...
                                $this->Flash->error(
                                    __(
                                        'The system unable to process your grade change request. You are either accessing a different course or trying to send grade change request for non submitted or on process or rejected grade. Please try your request again within the grade change period.'
                                    )
                                );
    ...
    public function add($published_course_id = null)
    {
        ...
        if (!empty($published_course_id) && $published_course_id > 0) {
            ...
            $list_of_assigned_courses_for_instructor = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find(
                'list',
                array(
                    'conditions' => array('CourseInstructorAssignment.staff_id' => $instructor_id),
                    'fields' => array(
            ...
            if ((!empty($list_of_assigned_courses_for_instructor) && !in_array(
                        $published_course_id,
                        $list_of_assigned_courses_for_instructor
                    )) || empty($list_of_assigned_courses_for_instructor)) {
                // trying to cheat
                $this->Flash->warning(
                    __(
                        'You are trying to access published course that is not assigned to you!, This incident will be reported. Please do not try this again.'
                    )
                );
                $message = '<u>' . $this->Session->read('Auth.User')['full_name'] . ' (' . $this->Session->read(
                        'Auth.User'
                    )['username'] . ')</u> is trying to access not assigned course exam result management. Please give appropriate warning.';
        ...
        if (/* !$published_course_id && */ !empty($this->request->data)) {
            ...
            if ($this->Session->read(
                    'Auth.User'
                )['role_id'] == ROLE_INSTRUCTOR && isset($this->request->data['ExamResult']['published_course_id']) && !empty($this->request->data['ExamResult']['published_course_id'])) {
        ...
        if (/* $this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT ||  */ $this->Session->read(
            ...
            if (!empty($this->request->data)) {
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                    ...
                    $publishedCourses = $this->ExamResult->CourseRegistration->PublishedCourse->CourseInstructorAssignment->listOfCoursesCollegeFreshTakingOrgBySection(
                        $college_id,
            ...
            if (empty($grade_scale) && !empty($published_course_id)) {
                $grade_scale = $this->ExamResult->CourseRegistration->PublishedCourse->getGradeScaleDetail(
                    $published_course_id
                );
            }
            ...
            $this->Flash->error(
                __(
                    'You need to have either college or department role to access this area. Please contact your system administrator to get college or department role.'
                )
            );
    ...
    public function submit_grade_for_instructor($published_course_id = null)
    {
        ...
        if ($this->Session->read(
            ...
            if (!empty($published_course_id)) {
                ...
                $published_course = $this->ExamResult->CourseRegistration->PublishedCourse->find(
                    'first',
            ...
            $this->Flash->error(
                __(
                    'You need to have department role to access this area. Please contact your system administrator to get department role.'
                )
            );
            return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
        }
    ...
    private function __exam_result_and_grade_mgt($published_course_id = null, $sourse = 'instructor')
    {
        ...
        if ($sourse == "instructor" && empty($published_course_id)) {
            $selectedAcadamicYear = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find(
                'first',
                array(
                    'conditions' => array(
                        'CourseInstructorAssignment.staff_id in (select id from staffs where user_id="' . $this->Auth->user(
        ...
        if (!empty($published_course_id)) {
            ...
            $assigned_instructor_user_id = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                'Staff.user_id',
                array('Staff.id' => $instructor_id_for_checking)
            );
            $active_account = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->User->field(
                'active',
            ...
            $login_instructor_id = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field(
                'id',
                array('Staff.user_id' => $this->Auth->user('id'))
            );
        ...
        if (!$do_not_manipulate && !empty($this->request->data)) {
            ...
            if (isset($this->request->data['saveExamResult']) || isset($this->request->data['previewExamGrade'])) {
                ...
                foreach ($this->request->data['ExamResult'] as $key => $exam_result) {
                    ...
                    if (is_array($exam_result)) {
                            ...
                            if (isset($exam_result['id'])) {
								...
								*/
                                ...
                                if ($exam_result['course_add'] == 0) {
                                    $exam_result_published_course_id = $this->ExamResult->CourseRegistration->field(
                                        'published_course_id',
                                        array('CourseRegistration.id' => $exam_result['course_registration_id'])
                                    );
                                } else {
                                    $exam_result_published_course_id = $this->ExamResult->CourseAdd->field(
                                        'published_course_id',
                                        array('CourseAdd.id' => $exam_result['course_registration_id'])
                                    );
                                }
                        ...
                        if ($fraud) {
                            $this->Flash->error(
                                __(
                                    'The system encountered an error while processing your exam result submission. Please try your submission again.'
                                )
                            );
                            //$save_is_ok = false;
                            return $this->redirect(
                                array(
                                    'action' => (strcasecmp($sourse, 'instructor') == 0 ? 'add' : (strcasecmp(
                                        $sourse,
                                        'college'
                                    ) == 0 ? 'submit_freshman_grade_for_instructor' : 'submit_grade_for_instructor')),
                                    $published_course_id
                                )
                            );
                        }
                        ...
                        if (trim($exam_result['result']) != "") {
                            ...
                            if (isset($exam_result['id'])) {
                                $exam_type_id = $this->ExamResult->field(
                                    'exam_type_id',
                                    array('id' => $exam_result['id'])
                                );
                            } else {
                            ...
                            if ($exam_percent == "" || $exam_name == "" || $exam_type_id == "") {
                                $this->Flash->error(
                                    __(
                                        'Exam setup is not found for some results, Probably you applied changes on the exam setup. Please provide the exam result again.'
                                    )
                                );
                                //$this->redirect(array('action' => 'add', $published_course_id));
                                $save_is_ok = false;
                            } else {
                                if (is_numeric($exam_result['result']) && $exam_result['result'] > $exam_percent) {
                                    $this->Flash->error(
                                        __(
                                            'Exam result for ' . $exam_name . ' exam can only be a maximum of ' . $exam_percent . '. To change the percentage, please go to the exam setup section.'
                                        )
                                    );
                                    //$this->redirect(array('action' => 'add', $published_course_id));
                                    $save_is_ok = false;
                ...
                if ($exam_inconsistentcy_found) {
                    $this->Flash->error(
                        __(
                            'The system encountered an error while processing your exam result submission. Please make sure that the exam results are consistent with what you are trying to enter and try your submission again.'
                        )
                    );
                    //$save_is_ok = false;
                    return $this->redirect(
                ...
                if (isset($this->request->data['MakeupExam']) && !empty($this->request->data['MakeupExam'])) {
                    foreach ($this->request->data['MakeupExam'] as $key => $makeup_exam) {
                        $makeup_exam_published_course_id = $this->ExamResult->CourseRegistration->MakeupExam->field(
                            'published_course_id',
                            array('id' => $makeup_exam['id'])
                        );
                        if ($makeup_exam_published_course_id != $published_course_id) {
                            $this->Flash->error(
                                __(
                                    'Sorry, the system is unable to get some students for makeup exam result entry. This usually happens when the department apply a change while you make exam result submission. Please re-submit your exam result entry.'
                                )
                            );
                            $save_is_ok = false;
                        } else {
                            if (trim($makeup_exam['result']) != "" && (!is_numeric(
                                        $makeup_exam['result']
                                    ) || $makeup_exam['result'] > 100 || $makeup_exam['result'] < 0)) {
                                //$this->ExamResult->saveAll($exam_results, array('validate' => 'only'));
                                $this->Flash->error(
                                    __(
                                        'The system is unable to record exam result entry. Please make sure that all your exam result entry is valid.'
                                    )
                                );
                                $save_is_ok = false;
                            } else {
                                $makeup_exam_results[$key]['id'] = $makeup_exam['id'];
                                $makeup_exam_results[$key]['result'] = (trim(
                                    $makeup_exam['result']
                                ) === "" ? null : $makeup_exam['result']);
                            }
                        }
                    }
                }
                ...
                if ($save_is_ok) {
                    if ($this->ExamResult->saveAll($exam_results)) {
                        if (!empty($exam_result_delete_ids)) {
                            if (!$this->ExamResult->deleteAll(array('ExamResult.id' => $exam_result_delete_ids),
                                false)) {
                                $this->Flash->error(
                                    __(
                                        'Exam result entry/update is saved but deletion for previous exam result is interrupted. Please check your exam result entry and/or update for consistency.'
                                    )
                                );
                                return $this->redirect(
                                    array(
                        ...
                        if (!empty($makeup_exam_results)) {
                            ...
                            if (!ClassRegistry::init('MakeupExam')->saveAll(
                                $makeup_exam_results,
                                array('validate' => false)
                            )) {
                                $this->Flash->error(
                                    __(
                                        'Exam result entry/update for makeup exam is failed. Please re-enter the makeup exam result.'
                                    )
                                );
                                return $this->redirect(
                                    array(
            ...
            if (isset($this->request->data['previewExamGrade'])) {
                ...
                $course_required_scale = $this->ExamResult->CourseRegistration->PublishedCourse->isPublishedCourseRequiredScale(
                    $published_course_id
                );
                $students = $student_course_register_and_adds['register'];
                $student_adds = $student_course_register_and_adds['add'];
                ...
                if (!isset($grade_scale['error'])) {
                    ...
                    $student_makeup = $this->ExamResult->generateCourseGrade(
                        $student_makeup,
                        $grade_scale,
                        $exam_types
                    );
                ...
                if (isset($this->request->data['submitExamGrade'])) {
                    ...
                    if (isset($grade_submit_status['error'])) {
                        ...
                        if (count($grade_submit_status['course_registration_add']) > 0 || count(
                                $grade_submit_status['makeup_exam']
                            ) > 0) {
                            $this->Flash->success(
                                __(
                                    'Exam grade for ' . (count($grade_submit_status['course_registration_add']) + count(
                                            $grade_submit_status['makeup_exam']
                                        )) . ' student' . ((count(
                                            $grade_submit_status['course_registration_add']
                                        ) + count(
                                            $grade_submit_status['makeup_exam']
                                        )) > 1 ? 's' : '') . ' is successfully submitted to the department for approval. You can cancel your grade submission to apply changes, if needed, before it is approved by the department. Once the grades are approved, neither you can cancel the submitted grade nor edit exam result except for students with "In Progress" grade submission status.'
                                )
                            );
                        } else {
                            $this->Flash->info(
                                __(
                                    'Exam grade is already submitted to the department for approval. No need to submit that again.'
                                )
                            );
                        }
                    }
                    ...
                    if (isset($this->request->data['cancelExamGrade'])) {
                        /*************************** Begin "Cancel Submit Grade" button  **********************************/

                        $student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingPublishedCourse(
                            $published_course_id
                        );
                        $grade_cancelation_status = $this->ExamResult->cancelSubmitedGrade(
                            $published_course_id,
                            $student_course_register_and_adds
                        );
                        ...
                        if (isset($grade_cancelation_status['error'])) {
                            ...
                            if (count($grade_cancelation_status['course_registration_add']) > 0 || count(
                                    $grade_cancelation_status['makeup_exam']
                                ) > 0) {
                                $this->Flash->success(
                                    __(
                                        'Exam grade submission cancellation for ' . (count(
                                                $grade_cancelation_status['course_registration_add']
                                            ) + count($grade_cancelation_status['makeup_exam'])) . ' student' . ((count(
                                                $grade_cancelation_status['course_registration_add']
                                            ) + count(
                                                $grade_cancelation_status['makeup_exam']
                                            )) > 1 ? 's' : '') . ' was successfull. Please edit exam result, preview and submit the exam grade before grade submission deadline.'
                                    )
                                );
                            } else {
                                $this->Flash->info(
                                    __(
                                        'Exam grade is on process by the department and registrar. Unable to process your exam grade cancellation request.'
                                    )
                                );
                            }
                        }
                        ...
                        if (isset($this->request->data['cancelExamGradePreview'])) {
                            ...
                            if (isset($this->request->data['exportExamGrade'])) {
                                $this->autoLayout = false;

                                $publish_course_detail_info = $this->ExamResult->ExamType->PublishedCourse->find(
                                    'first',
                                    array(
                                        'conditions' => array(
                                            'PublishedCourse.id' => $published_course_id
                                ...
                                $filename = "Mark_Sheet_" . $publish_course_detail_info['Course']['course_code'] . '_' . (str_replace(
                                    ...
                                    )) . '_' . $publish_course_detail_info['PublishedCourse']['semester'] . '_' . date(
                                        'Y-m-d'
                                    ); //. ($publish_course_detail_info['PublishedCourse']['semester'] == 'I' ? '1st' : ($publish_course_detail_info['PublishedCourse']['semester'] == 'II' ? '2nd' : ($publish_course_detail_info['PublishedCourse']['semester'] == 'III' ? '3rd' : $publish_course_detail_info['PublishedCourse']['semester']))) . 'Semester';
                                $this->set(
                                    compact(
        ...
        if (!empty($published_course_id) && is_numeric($published_course_id) && $published_course_id > 0) {
            $published_course_detail = $this->ExamResult->CourseAdd->PublishedCourse->find(
                'first',
            ...
            if (isset($this->request->data['ExamResult']['semester']) && !empty($this->request->data['ExamResult']['semester'])) {
        ...
        if (strcasecmp($sourse, 'instructor') == 0) {
            $publishedCourses = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->listOfCoursesInstructorAssignedBySection(
                $selected_acadamic_year,
                $selected_semester,
                $instructor_id
            );
        } else {
            if (!empty($this->request->data) || $published_course_id) {
                if (/* empty($this->request->data) && */ $published_course_id && is_numeric(
                        $published_course_id
                    ) && $published_course_id > 0) {
                    $published_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find(
                        'first',
                        array('conditions' => array('PublishedCourse.id' => $published_course_id), 'recursive' => -1)
                    );
                    ...
                    if (isset($this->request->data['ExamResult'])) {
                ...
                if (strcasecmp($sourse, 'college') == 0) {
                    $publishedCourses = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->listOfCoursesCollegeFreshTakingOrgBySection(
                        $this->college_id,
                        $acadamic_year,
                        $semester,
                        $program_id,
        ...
        if (isset($published_course_id) && !empty($published_course_id)) {
            $date_grade_submited = ClassRegistry::init('AcademicCalendar')->getLastGradeChangeDate(
                $published_course_id
            );
        } else {
            $date_grade_submited = date('Y-m-d H:i:s');
    ...
    private function __data_entry_assignment($published_course_id = null, $sourse = 'instructor')
    {
        ...
        if (!empty($published_course_id)) {
            ...
            $student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsRequiresGradeEntryExamPublishedCourse(
                $published_course_id
            );
            ...
            $grade_submission_status = $this->ExamResult->getExamGradeEntrySubmissionStatus(
                $published_course_id,
                $student_course_register_and_adds
            );
            $published_course_combo_id = $published_course_id;
        ...
        if (!$do_not_manipulate && !empty($this->request->data)) {
            ...
            if (isset($this->request->data['saveExamResult']) || isset($this->request->data['previewExamGrade'])) {
                ...
                if (isset($this->request->data['ResultEntryAssignment'])) {
                    foreach ($this->request->data['ResultEntryAssignment'] as $key => $entry_exam) {
                        $entry_exam_published_course_id = $this->ExamResult->CourseRegistration->ResultEntryAssignment->field(
                            'published_course_id',
                            array('id' => $entry_exam['id'])
                        );
                        if ($entry_exam_published_course_id != $published_course_id) {
                            $this->Flash->warning(
                                __(
                                    'Sorry, the system is unable to get some students for result entry. This usually happens when the department apply a change while you make exam result submission. Please re-submit your exam result entry.'
                                )
                            );
                            $save_is_ok = false;
                        } else {
                            if (trim($entry_exam['result']) != "" && (!is_numeric(
                                        $entry_exam['result']
                                    ) || $entry_exam['result'] > 100 || $entry_exam['result'] < 0)) {
                                $this->Flash->error(
                                    __(
                                        'The system unable to record exam result entry. Please make sure that all your exam result entry is valid.'
                                    )
                                );
                                $save_is_ok = false;
                            } else {
                                $entry_exam_results[$key]['id'] = $entry_exam['id'];
                                $entry_exam_results[$key]['result'] = (trim(
                                    $entry_exam['result']
                                ) === "" ? null : $entry_exam['result']);
                            }
                        }
                    }
                }
                ...
                if ($save_is_ok) {
                    ...
                    if (!empty($entry_exam_results)) {
                        ...
                        if (!ClassRegistry::init('ResultEntryAssignment')->saveAll(
                            $entry_exam_results,
                            array('validate' => false)
                        )) {
                            $this->Flash->error(
                                __('Exam result entry/update is failed. Please re-enter the  exam result.')
                            );
                            return $this->redirect(
                                array(
                                    'action' => (strcasecmp(
                                        $sourse,
                                        'instructor'
                                    ) == 0 ? 'submit_assigned_grade' : (strcasecmp(
                                        $sourse,
                                        'college'
                                    ) == 0 ? 'submit_freshman_grade_for_instructor' : 'submit_grade_for_instructor')),
                                    $published_course_id
            ...
            if (isset($this->request->data['previewExamGrade'])) {
            ...
            } elseif (isset($this->request->data['submitExamGrade'])) {
                /*************************** Begin "Submit Grade" button  **********************************/
                ...
                $grade_submit_status = $this->ExamResult->submitGradeEntryAssignment(
                    $student_course_register_and_adds,
                    $students_course_in_progress,
                    $grade_scale
                );
                    ...
                    if (count($grade_submit_status['course_registration_add']) > 0 || count(
                        ...
                        $this->Flash->success(
                            __(
                                'Exam grade for ' . (count($grade_submit_status['course_registration_add']) + count(
                                        $grade_submit_status['makeup_exam']
                                    )) . ' student' . ((count($grade_submit_status['course_registration_add']) + count(
                                        $grade_submit_status['makeup_exam']
                                    )) > 1 ? 's' : '') . ' is successfully submitted to the department for approval. You can cancel your grade submission to apply changes, if needed, before it is approved by the department. Once the grades are approved, neither you can cancel the submitted grade nor edit exam result except for students with "In Progress" grade submission status.'
                            )
                        );
                        ...
                        $this->Flash->warning(
                            __('Exam grade is already successfully submitted to the department for approval.')
                        );
                    }
                }
                ...
                return $this->redirect(
                    array(
                        'action' => (strcasecmp($sourse, 'instructor') == 0 ? 'submit_assigned_grade' : (strcasecmp(
                            $sourse,
                            'college'
                        ) == 0 ? 'submit_freshman_grade_for_instructor' : 'submit_grade_for_instructor')),
                        $published_course_id
                ...
                if (isset($this->request->data['cancelExamGrade'])) {
                    ...
                    $grade_cancelation_status = $this->ExamResult->cancelSubmitedGradeEntry(
                        $published_course_id,
                        $student_course_register_and_adds
                    );
                            ...
                            $this->Flash->success(
                                __(
                                    'Exam grade submission cancellation for ' . (count(
                                            ...
                                            $grade_cancelation_status['makeup_exam']
                                        )) > 1 ? 's' : '') . ' is successfully done. Please edit exam result, preview and submit the exam grade within the given grade submission period.'
                                )
                            );
                        } else {
                            $this->Flash->info(
                                __(
                                    'Exam grade is on process by the department and registrar to proceed with your exam grade cancellation request.'
                                )
                            );
                        }
                    }
        ...
        if (strcasecmp($sourse, 'instructor') == 0) {
            $publishedCourses = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->listOfAssignedGradeEntryAssignedBySection(
                $selected_acadamic_year,
                $selected_semester,
                $instructor_id
            );
            ...
            if (!empty($this->request->data) || $published_course_id) {
                ...
                if (strcasecmp($sourse, 'college') == 0) {
                    ...
                    $publishedCourses = $this->ExamResult->CourseRegistration->PublishedCourse->CourseInstructorAssignment->listOfAssignedGradeEntryAssignedBySection(
                        $this->department_id,
                        $acadamic_year,
                        $semester,
                        $program_id,
        ...
        $section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find(
            'first',
            array(
                'conditions' => array('PublishedCourse.id' => $published_course_id),
                'contain' => array('Section', 'Course')
    ...
    private function __exam_result_and_grade_mgt_fx($published_course_id = null, $sourse = 'instructor')
    {
        ...
        if (!empty($published_course_id)) {
            ...
            $student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingFxExamPublishedCourse(
                $published_course_id
            );
            //$students = $student_course_register_and_adds['register'];
            $student_makeup = $student_course_register_and_adds['makeup'];
        ...
        if (
            ...
            if (
                    ...
                    if (is_array($exam_result)) {
                        ...
                        if ($fraud) {
                            $this->Session->setFlash(
                                '<span></span>' . __(
                                    'The system encounter a problem while processing your exam result submission. Please try your submission again.'
                                ),
                                'default',
                        ...
                        if (trim($exam_result['result']) != "") {
                            ...
                            if ($exam_percent == "" || $exam_name == "" || $exam_type_id == "") {
                                $this->Session->setFlash(
                                    '<span></span>' . __(
                                        'Exam setup is not found for some result and I think you apply changes on the exam setup. Please provide the exam result again.'
                                    ),
                                    'default',
                                    array('class' => 'error-message error-box')
                                );
                ...
                if ($exam_inconsistentcy_found) {
                    $this->Session->setFlash(
                        '<span></span>' . __(
                            'The system encounter a problem while processing your exam result submission. Please make sure that the following exam result is consistent with what you are trying to enter and try your submission again.'
                        ),
                        'default',
                        array('class' => 'error-message error-box')
                    );
                    ...
                    return $this->redirect(
                        array(
                            'action' => (strcasecmp($sourse, 'instructor') == 0 ? 'submit_fx_grade' : (strcasecmp(
                                $sourse,
                                'college'
                            ) == 0 ? 'submit_freshman_grade_for_instructor' : 'submit_grade_for_instructor')),
                            $published_course_id
                    ...
                    if ($this->ExamResult->saveALL($exam_results)) {
                        if (!empty($exam_result_delete_ids)) {
                            if (!$this->ExamResult->deleteAll(array('ExamResult.id' => $exam_result_delete_ids),
                                ...
                                $this->Session->setFlash(
                                    '<span></span>' . __(
                                        'Exam result entry/update is saved but deletion for exam result is interrupted. Please check your exam result entry and/or update for consistency.'
                                    ),
                                    'default',
                                    array('class' => 'error-message error-box')
                                );
                        ...
                        if (!isset($this->request->data['previewExamGrade'])) {
                            $this->Session->setFlash(
                                '<span></span>' . __('Exam result has been saved'),
                                'default',
                        ...
                        $this->Session->setFlash(
                            '<span></span>' . __('The exam result could not be saved. Please, try again.'),
                            'default',
                            array('class' => 'error-message error-box')
                        );
                    }
            ...
            if (isset($this->request->data['previewExamGrade'])) {
                ...
                if (!isset($grade_scale['error'])) {
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __($grade_scale['error']),
                        'default',
                        array('class' => 'error-message error-box')
                    );
                    return $this->redirect(
            ...
            /*************************** Begin "Submit Grade" button  **********************************/
                ...
                if (isset($this->request->data['submitExamGrade'])) {
                    ...
                    if (isset($grade_submit_status['error'])) {
                        $this->Session->setFlash(
                            '<span></span>' . __($grade_submit_status['error']),
                            'default',
                            array('class' => 'success-message success-box')
                        );
                    } else {
                        if (count($grade_submit_status['course_registration_add']) > 0 || count(
                            ...
                            $this->Session->setFlash(
                                '<span></span>' . __(
                                    'Exam grade is already successfully submitted to the department for approval.'
                                ),
                                'default',
                ...
                /*************************** Begin "Cancel Submit Grade" button  **********************************/
                    ...
                    if (isset($this->request->data['cancelExamGrade'])) {
                        ...
                        if (isset($grade_cancelation_status['error'])) {
                            $this->Session->setFlash(
                                '<span></span>' . __($grade_cancelation_status['error']),
                                'default',
                                array('class' => 'success-message success-box')
                            );
                        } else {
        ...
        if (strcasecmp($sourse, 'instructor') == 0) {
            $publishedCourses = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->listOfFxCoursesInstructorAssignedBySection(
                $selected_acadamic_year,
                $selected_semester,
                $instructor_id
            );
            ...
            if (!empty($this->request->data) || $published_course_id) {
                ...
                if (strcasecmp($sourse, 'college') == 0) {
                    ...
                    $publishedCourses = $this->ExamResult->CourseRegistration->PublishedCourse->CourseInstructorAssignment->listOfFxCoursesInstructorAssignedBySection(
                        $this->department_id,
                        $acadamic_year,
                        $semester,
                        $program_id,
        ...
        $lastGradeSubmissionDate = ClassRegistry::init('AcademicCalendar')->getFxPublishedCourseGradeSubmissionDate(
            $published_course_id
        );
    ...
    public function get_exam_result_entry_form($published_course_id = null)
    {
        ...
        if ($this->Auth->user('id')) {
            ...
            if (!empty($published_course_id)) {
                ...
                if ($active_account == 1 && $instructor_id_for_checking != $login_instructor_id) {
                    $view_only = true;
                    $pc_given_by_department = $this->ExamResult->CourseRegistration->PublishedCourse->field(
                        'given_by_department_id',
                        array('PublishedCourse.id' => $published_course_id)
                    );
                    $show_user_deactivation_link = (!empty($pc_given_by_department) && $this->Session->read(
                        'Auth.User'
                    )['role_id'] == ROLE_DEPARTMENT && isset($this->department_id) && $this->department_id == $pc_given_by_department ? true : false);
                }
    ...
    public function get_exam_result_fx_entry_form($published_course_id = null)
    {
    ...
    public function get_registrar_assigned_grade_entry_form($published_course_id = null)
    {
    ...
    public function cancel_grade_change_request($grade_change_id = null)
    {
        ...
        if ($grade_change_id) {
            ...
            if (($login_instructor_id != $instructor_id_for_checking &&
                ...
                if ($this->ExamResult->CourseRegistration->ExamGrade->ExamGradeChange->delete($grade_change_id)) {
                    $this->Flash->success(
                        __(
                            'Exam grade change request for ' . ($student_detail['Student']['first_name'] . ' ' . $student_detail['Student']['middle_name'] . ' ' . $student_detail['Student']['last_name'] . ' (' . $student_detail['Student']['studentnumber'] . ')') . ' is cancelled.'
                        )
                    );
                } else {
            ...
            return $this->redirect(
                array(
                    'action' => (strcasecmp(
                        $this->request->action,
                        'add'
                    ) == 0 ? 'add' : (!empty($student_detail['Student']['department_id']) ? 'submit_grade_for_instructor' : 'submit_freshman_grade_for_instructor')),
                    $published_course_id
                )
            );
        } else {
    ...
    public function _generate_student_academic_status($section_id = null)
    {
        ...
        if (!empty($section_id)) {
            ...
            $getListOfPublishedCourses = $this->ExamResult->CourseRegistration->PublishedCourse->find(
                'list',
                array('order' => array('PublishedCourse.semester' => 'ASC'))
            );
        }
                    ...
                    if (trim($exam_result['result']) != "") {
                        ...
                        if ($save_is_ok) {
                            ...
                            if (isset($data['ExamResult']['id']) && !empty($data['ExamResult']['id'])) {
                                $alreadyRecored = $this->ExamResult->find(
                                    'first',
                                    array(
    ...
    public function rollback_grade_submission($published_course_id = null)
    {
        ...
        $grade_scale = array();
        $programs = $this->ExamResult->CourseRegistration->PublishedCourse->Section->Program->find(
            'list',
            array('conditions' => array('Program.id' => $this->program_ids, 'Program.active' => 1))
        );
        $program_types = $this->ExamResult->CourseRegistration->PublishedCourse->Section->ProgramType->find(
            'list',
            array('conditions' => array('ProgramType.id' => $this->program_type_ids, 'ProgramType.active' => 1))
        );
        ...
        if (isset($this->request->data['rollback']) && !empty($this->request->data['rollback'])) {
            ...
            if (!empty($courseRegistrationAndGrade)) {
                ...
                if (ClassRegistry::init('ExamGrade')->saveAll(
                    $courseRegistrationAndGrade['ExamGrade'],
                    array('validate' => false)
                )) {
                    $selected_count = count($student_ids_to_regenarate_status);
                    $this->Flash->success(
                        __(
                            'The selected ' . $selected_count . ' submitted ' . ($selected_count == 1 ? 'grade is' : 'grades are') . ' rolled back successfully for resubmission by the assigned course instructor.'
                        )
                    );
                    ...
                    if (!empty($student_ids_to_regenarate_status)) {
                        foreach ($student_ids_to_regenarate_status as $key => $stdnt_id) {
                            ...
                            $status_status = ClassRegistry::init(
                                'StudentExamStatus'
                            )->regenerate_all_status_of_student_by_student_id($stdnt_id, 0);
                        }
                    }
                    ...
                    if (!empty($rolledback_grades)) {
                        ...
                        ClassRegistry::init('AutoMessage')->sendNotificationOnRegistrarGradeRollback(
                            $rolledback_grades,
                            $this->Session->read('Auth.User')['full_name'],
                            $this->Session->read('Auth.User')['id'],
                            $department_approved_bys,
                            $this->request->data['ExamResult']['pc_id']
                        );
                    }
        ...
        if (!empty($this->department_ids)) {
            $departments = $this->ExamResult->CourseRegistration->Student->Department->find(
                'list',
                array('conditions' => array('Department.id' => $this->department_ids, 'Department.active' => 1))
            );
        } else {
            if (!empty($this->college_ids)) {
                $colleges = $this->ExamResult->CourseRegistration->Student->College->find(
                    'list',
        ...
        if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
            ...
            $acyear_list = $this->AcademicYear->academicYearInArray(
                (explode('/', $current_acy)[0]) - (ACY_BACK_FOR_ROLLING_BACK_GRADE_SUBMISSION + 5),
                (explode('/', $current_acy)[0])
            );
        } else {
            $acyear_list = $this->AcademicYear->academicYearInArray(
                (explode('/', $current_acy)[0]) - ACY_BACK_FOR_ROLLING_BACK_GRADE_SUBMISSION,
                (explode('/', $current_acy)[0])
            );
        }
    ...
    public function submit_grade_registrar($published_course_id = null)
    {
        ...
        if (isset($this->request->data['saveGrade']) && !empty($this->request->data['saveGrade'])) {
            ...
            $scale_id = ClassRegistry::init('GradeScale')->getGradeScaleIdGivenPublishedCourse(
                $this->request->data['PublishedCourse']['id']
            );
            ...
            if (!empty($this->request->data['ExamResult'])) {
                foreach ($this->request->data['ExamResult'] as $key => $student) {
                    if (is_int($key) && $student['gp'] == 1 && $scale_id != 0 && !empty($student['grade'])) {
                        ...
                        $courseRegistrationAndGrade['ExamGrade'][$count]['registrar_reason'] = 'Registrar Data Entry interface';
                        $courseRegistrationAndGrade['ExamGrade'][$count]['registrar_approval_date'] = date(
                            'Y-m-d H:i:s'
                        );
                        $courseRegistrationAndGrade['ExamGrade'][$count]['department_approval_date'] = date(
                            'Y-m-d H:i:s'
                        );
                        $courseRegistrationAndGrade['ExamGrade'][$count]['department_approved_by'] = $this->Auth->user(
                            'id'
                        );
                        $courseRegistrationAndGrade['ExamGrade'][$count]['registrar_approved_by'] = $this->Auth->user(
                            'id'
                        );
            ...
            if (!empty($courseRegistrationAndGrade)) {
                ...
                if (ClassRegistry::init('ExamGrade')->saveAll(
                    ...
                    if (ClassRegistry::init(
                        'ExamGrade'
                    )->CourseRegistration->Student->StudentExamStatus->updateAcdamicStatusByPublishedCourse(
                        $published_course_id
                    )) {
                    }
                    ...
                    $this->Flash->success(
                        __(
                            'Exam grade saving for ' . ($st_count) . ' ' . ($st_count == 1 ? 'student' : 'students') . ' for ' . $publishedCourse['Course']['course_code_title'] . ' course was successful.'
                        )
                    );
        ...
        if ($this->Auth->user('id')) {
            ...
            if (!empty($published_course_id)) {
                ...
                if (isset($section_and_course_detail['CourseInstructorAssignment'][0]['Staff'])) {
                    ...
                    $assigned_instructor_fullname_and_username = (trim(
                            (ucwords(
                                strtolower(
                                    preg_replace(
                                        $search_x_in_name,
                                        '',
                                        $section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['full_name']
                                    )
                                )
                            ))
                    ...
                    $assigned_instructor_fullname_with_rank = (isset($section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['Title']['title']) ? $section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['Title']['title'] . '. ' : '') . (trim(
                            ucwords(
                                strtolower(
                                    preg_replace(
                                        $search_x_in_name,
                    ...
                    if ($section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['active'] || (isset($section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['User']) && $section_and_course_detail['CourseInstructorAssignment'][0]['Staff']['User']['active'])) {
                        if (($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                                    'Auth.User'
                                )['is_admin'] == 1 && ALLOW_ADMIN_REGISTRAR_GRADE_ENTRY_ON_NOT_DEACTIVATED_INSTRUCTOR_ASSIGNMENT == 0) || ($this->Session->read(
                                    'Auth.User'
                                )['role_id'] == ROLE_REGISTRAR && $this->Session->read(
                                    'Auth.User'
                                )['is_admin'] != 1 && ALLOW_NON_ADMIN_REGISTRAR_GRADE_ENTRY_ON_NOT_DEACTIVATED_INSTRUCTOR_ASSIGNMENT == 0)) {
                            $view_only = true;
                        } else {
                ...
                if (isset($section_and_course_detail['Section']['id'])) {
                    $selected_section_name = ClassRegistry::init('Section')->get_section_detailed_name(
                        $section_and_course_detail['Section']['id'],
                        0,
                        0
                    );
                    $selected_section_detailed = (explode(
                        '~',
                        ClassRegistry::init('Section')->get_section_detailed_name(
                            $section_and_course_detail['Section']['id'],
                            1,
                            1
                        )
    ...
    public function import_result($published_course_id = null)
    {
        ...
        if (!empty($this->request->data) && is_uploaded_file($this->request->data['File']['tmp_name'])) {
            ...
            $checkIfGradeSubmitted = $this->ExamResult->CourseRegistration->ExamGrade->isGradeSubmittedForPublishedCourse(
                $this->request->data['ExamResult']['published_course_id']
            );

            if ($checkIfGradeSubmitted) {
                $this->Flash->error(
                    __(
                        'Exam grade has been submitted already you can not modify it by uploading rather you need to modify or request grade change online.'
                    )
                );
            }
            ...
            if (strcasecmp($ext, 'csv') != 0) {
                $this->Flash->error(
                    __(
                        'Importing Error. Please save your file as "CSV" type while you saved the file and import again. Current file format is: ' . $ext
                    )
                );
            ...
            if (empty($headers)) {
                $this->Flash->error(
                    __('Importing Error!! Please insert  StudentID filed name at first row of your excel file.')
                );
                return $this->redirect(array('controller' => 'examResults', 'action' => 'import_result'));
                //return;
            }
            ...
            if (count($non_existing_field) > 0) {
                ...
                $this->Flash->error(
                    __('Importing Error. ' . $field_list . ' are required in the excel file you imported at first row.')
                );
                return $this->redirect(array('controller' => 'examResults', 'action' => 'import_result'));
                //return;
                ...
                // if not create
                $setup = $this->ExamResult->ExamType->examSetupCreation(
                    $this->request->data['ExamResult']['published_course_id'],
                    $fields_name_acceptedStudents_table
                );
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                    ...
                    foreach ($data as $k => $v) {
                        ...
                        if ($fields_name_acceptedStudents_table[$k] == 'StudentID' && (!isset($fieldValue) || trim(
                                    $fieldValue
                                ) == null)) {
                                ...
                                if (isset($assement[0]) && !empty($assement[0])) {
                                    ...
                                    if (isset($resRecorded['ExamResult']['id']) && !empty($resRecorded['ExamResult']['id'])) {
                                        ...
                                        if (isset($regFind['CourseRegistration']['id']) && !empty($regFind['CourseRegistration']['id'])) {
                                            ...
                                            if (isset($addFind['CourseAdd']['id']) && !empty($addFind['CourseAdd']['id'])) {
                ...
                if (count($non_valide_rows) > 0) {
                    $row_list = "";
                    $this->Flash->error(
                        __('Importing Error!! Please correct the following listed rows in your CSV excel file.')
                    );
    ...
    public function download_exam_template($published_course_id = null)
    {
