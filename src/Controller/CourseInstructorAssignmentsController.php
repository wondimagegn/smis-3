...
class CourseInstructorAssignmentsController extends AppController
{
	...
	function delete($id = null, $published_course_id = null)
	{
		if (!empty($published_course_id)) {
			...
			$course_instructor_assignment_data = $this->CourseInstructorAssignment->PublishedCourse->find('first', array('fields' => array(
				'PublishedCourse.academic_year', 'PublishedCourse.semester', 'PublishedCourse.program_id',
				'PublishedCourse.program_type_id', 'PublishedCourse.year_level_id',
			), 'conditions' => array('PublishedCourse.id' => $published_course_id, 'PublishedCourse.drop' => 0), 'contain' => array()));
