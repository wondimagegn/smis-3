...
class ProgramTypeTransfersController extends AppController
{
    ...
    public $name = 'ProgramTypeTransfers';
    ...
    public function index()
    {
        ...
        $this->paginate = array(
            'contain' => array(
                'ProgramType',
                'Student' => array('Department', 'College', 'ProgramType', 'Program')
            )
        );
        ...
        if (!empty($this->request->data) && isset($this->request->data['saveTransfer'])) {
            ...
            if ($this->ProgramTypeTransfer->getProgramTransferDate($this->request->data) &&
                ...
                if ($this->ProgramTypeTransfer->save($this->request->data['ProgramTypeTransfer'])) {
                    ...
                    $this->Session->setFlash(
                        '<span></span>' . __('The program type transfer could not be saved. Please, try again..'),
                        'default',
                        array('class' => 'error-box error-message')
                    );

                    $this->request->data['continue'] = true;
                    $student_number = $this->ProgramTypeTransfer->Student->field(
                        'studentnumber',
        ...
        if (!empty($this->request->data) && isset($this->request->data['continue'])) {
            ...
            if (!empty($this->request->data['ProgramTypeTransfer']['studentID'])) {
                ...
                if ($check_id_is_valid > 0) {
                    $everythingfine = true;
                    $student_id = $this->ProgramTypeTransfer->Student->field(
                        'id',
                ...
                $this->Session->setFlash(
                    '<span></span> ' . __('Please provide student number to maintain student program transfer.'),
                    'default',
                    array('class' => 'error-box error-message')
                );
            }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->ProgramTypeTransfer->delete($id)) {
            $this->Session->setFlash(
                '<span></span>' . __('Program type transfer deleted'),
                'default',
                array('class' => 'success-box success-message')
            );
            return $this->redirect(array('action' => 'index'));
        ...
        $this->Session->setFlash(
            '<span></span>' . __('Program type transfer was not deleted'),
            'default',
            array('class' => 'error-box error-message')
        );
        return $this->redirect(array('action' => 'index'));
    ...
    public function notify_program_transfer_to_department()
    {
