...
class DepartmentStudyProgramsController extends AppController
{
    ...
    public $name = 'DepartmentStudyPrograms';
    ...
    public function beforeRender(Event $event)
    {
        ...
        $thisacademicyear = $this->AcademicYear->currentAcademicYear();
    ...
    public function index()
    {
        ...
        if (!empty($this->request->data) && isset($this->request->data['DepartmentStudyProgram'])) {
            ...
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
                ...
                $attachedStudyProgramsIDs = $this->DepartmentStudyProgram->find(
                    'list',
                    array(
                        'conditions' => array(
                            'DepartmentStudyProgram.department_id' => $this->department_id,
                ...
                if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                    ...
                    if (!empty($this->request->data['DepartmentStudyProgram']['department_id'])) {
                        ...
                        $attachedStudyProgramsIDs = $this->DepartmentStudyProgram->find(
                            ...
                            array(
                                'conditions' => array(
                                    'DepartmentStudyProgram.department_id' => $this->request->data['DepartmentStudyProgram']['department_id'],
                                    'DepartmentStudyProgram.apply_for_current_students' => $apply_for_current_students
                                ),
                                'fields' => array(
                                    'DepartmentStudyProgram.study_program_id',
                        ...
                        $attachedStudyProgramsIDs = $this->DepartmentStudyProgram->find(
                            ...
                            array(
                                'conditions' => array(
                                    'DepartmentStudyProgram.department_id' => $department_ids,
                                    'DepartmentStudyProgram.apply_for_current_students' => $apply_for_current_students
                                ),
                                'fields' => array(
                                    'DepartmentStudyProgram.study_program_id',
                    ...
                    if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR) {
                        ...
                        if (!empty($this->request->data['DepartmentStudyProgram']['department_id'])) {
                            ...
                            $attachedStudyProgramsIDs = $this->DepartmentStudyProgram->find(
                                'list',
                                array(
                                    'conditions' => array('DepartmentStudyProgram.apply_for_current_students' => $apply_for_current_students),
                                    'fields' => array(
                                        'DepartmentStudyProgram.study_program_id',
                                        'DepartmentStudyProgram.study_program_id'
                                    )
            ...
            if ($this->Session->read('Auth.User')['role_id'] == ROLE_COLLEGE) {
                ...
                $options[] = array(
                    'DepartmentStudyProgram.department_id' => array_keys($departments),
                    'DepartmentStudyProgram.apply_for_current_students' => $apply_for_current_students
                );
                $attachedStudyProgramsIDs = $this->DepartmentStudyProgram->find(
                    'list',
                ...
                $departments = $this->DepartmentStudyProgram->Department->find(
                    'list',
        ...
        if ($this->Session->read('Auth.User')['role_id'] == ROLE_DEPARTMENT) {
            ...
            $department_college_id = $this->DepartmentStudyProgram->Department->field(
        ...
        $studyPrograms = $this->DepartmentStudyProgram->StudyProgram->find(
            'list',
            array(
                'conditions' => array('StudyProgram.id' => $attachedStudyProgramsIDs, 'StudyProgram.active' => 1),
                'order' => array('StudyProgram.local_band', 'StudyProgram.study_program_name')
            )
        );
        ...
        $academic_year = $this->DepartmentStudyProgram->find(
            'list',
            array(
                'fields' => array('DepartmentStudyProgram.academic_year', 'DepartmentStudyProgram.academic_year'),
                'group' => array('DepartmentStudyProgram.academic_year')
            )
        );
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
            if ($this->DepartmentStudyProgram->isUniqueDepartmentStudyProgram($this->request->data)) {
                ...
                $this->Flash->error(
                    'Department Study Program already exists. Change Study Program, Program Modality or Qualification and try again.'
                );
            }
        }
        ...
        $departments = $this->DepartmentStudyProgram->Department->find(
            ...
            array(
                ...
                'order' => array('Department.college_id', 'Department.name')
            )
        );
        $studyPrograms = $this->DepartmentStudyProgram->StudyProgram->find(
            'list',
            array(
                'conditions' => array('StudyProgram.active' => 1),
                'order' => array('StudyProgram.local_band', 'StudyProgram.study_program_name')
            )
        );
        $programModalities = $this->DepartmentStudyProgram->ProgramModality->find('list');
    ...
    public function edit($id = null)
    {
        ...
        if (empty($this->request->data)) {
            $departmentStudyProgram = $this->DepartmentStudyProgram->find(
                'first',
                array('conditions' => array('DepartmentStudyProgram.id' => $id), 'recursive' => -1)
            );
    ...
    public function get_department_study_programs_combo($curriculum_id = null)
    {
            ...
            if (!empty($curriculumDetail)) {
                $qualification_ids = ClassRegistry::init('Qualification')->find(
                    ...
                    array(
                        ...
                        'conditions' => array('Qualification.program_id' => $curriculumDetail['Curriculum']['program_id'])
                    )
                );
    ...
    public function get_selected_department_department_study_programs(
        $department_id = null,
        $all = 0/* , $for_current_students_only = '' */
    )
    {
