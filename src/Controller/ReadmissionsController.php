...
class ReadmissionsController extends AppController
{
    ...
    public function beforeRender(Event $event)
    {
        ...
        $acyearNext = ClassRegistry::init('StudentExamStatus')->getNextSemster(
            $this->AcademicYear->current_academicyear(),
            null
        );
        $acyear_array_data[$this->AcademicYear->currentAcademicYear()] = $this->AcademicYear->current_academicyear();
            ...
            if ($this->role_id == ROLE_COLLEGE) {
                ...
                if ($this->role_id == ROLE_REGISTRAR) {
                    if (!empty($this->department_ids)) {
                        ...
                        if (!empty($this->college_ids)) {
                            $colleges = $this->Readmission->Student->College->find(
                                'list',
                                array('conditions' => array('College.id' => $this->college_ids, 'College.active' => 1))
                            );
                        }
                    }
                    $programs = $this->Readmission->Student->Program->find(
                        'list',
                        array('conditions' => array('Program.id' => $this->program_id))
                    );
                    $programTypes = $this->Readmission->Student->ProgramType->find(
                        'list',
    ...
    public function apply()
    {
        ...
        $elegible = $this->Readmission->Student->Clearance->elegibleForReadmission(
            $this->student_id,
            $this->AcademicYear->current_academicyear()
        );
        ...
        if ($elegible == 0 || $elegible == 5) {
            ...
            if (!empty($this->request->data)) {
                ...
                if ($checkDuplication == 0) {
                    ...
                    $this->Flash->error(
                        'You have already requested readmission application for ' . $this->request->data['Readmission']['academic_year'] . ' of semester ' . $this->request->data['Readmission']['semester'] . '.'
                    );
                }
            }
        }
    ...
    public function ac_approve_readmission()
    {
        ...
        if ($this->role_id == ROLE_COLLEGE) {
            $department_ids = $this->Readmission->Student->Department->find(
                'list',
                array('conditions' => array('Department.college_id' => $this->college_id, 'Department.active' => 1))
            );
            ...
            if ($this->role_id == ROLE_DEPARTMENT) {
                $department_ids = $departments = $this->Readmission->Student->Department->find(
                    'list',
                    array('conditions' => array('Department.id' => $this->department_id))
                );
            } else {
                if ($this->role_id == ROLE_REGISTRAR) {
                    if (!empty($this->department_ids)) {
                        ...
                        if (!empty($this->college_ids)) {
                            $college_ids = $colleges = $this->Readmission->Student->College->find(
                                'list',
                                array('conditions' => array('College.id' => $this->college_ids, 'College.active' => 1))
                            );
                        }
        ...
        if (!empty($this->request->data) && isset($this->request->data['saveIt'])) {
                ...
                foreach ($this->request->data['Readmission'] as $in => &$va) {
                    if ($va['academic_commision_approval'] != "") {
                        ...
                        if ($va['academic_commision_approval'] == 1) {
                            $this->Readmission->Student->StudentsSection->id = $this->Readmission->Student->StudentsSection->field(
                                'StudentsSection.id',
            ...
            if ($this->Readmission->saveAll($this->request->data['Readmission'], array('validate' => 'first'))) {
                $this->Flash->success(
                    'The selected readmission applicant(s) request(s) have been approved and students will be notified.'
                );
                return $this->redirect(array('action' => 'index'));
            } else {
    ...
    public function process_readmission_application()
    {
        ...
        if (!empty($this->department_ids)) {
            ...
            $departments = $this->Readmission->Student->Department->find(
                'list',
                array('conditions' => array('Department.id' => $department_id, 'Department.active' => 1))
            );
        ...
        if (!empty($this->request->data) && isset($this->request->data['saveIt'])) {
            ...
            if ($this->Readmission->saveAll($this->request->data['Readmission'], array('validate' => 'first'))) {
                $this->Flash->success(
                    'The selected readmission applicant(s) request(s) is/are dispathed to academic commission decision.'
                );
                return $this->redirect(array('action' => 'index'));
            } else {
     ...
     ***/
    public function apply_readmission_for_student($department_id = null, $program_id = null, $program_type_id = null)
    {
        ...
        if ($this->role_id == ROLE_REGISTRAR) {
            ...
            $program_types = $programTypes = $this->Readmission->Student->ProgramType->find(
                'list',
                array('conditions' => array('ProgramType.id' => $this->program_type_id))
            );
        } else {
        ...
        $departments = $this->Readmission->Student->Department->allDepartmentsByCollege2(
            0,
            $this->department_ids,
            $this->college_ids,
            1
        ...
        if (isset($this->request->data) && isset($this->request->data['addStudentToReadmissionList'])) {
                        ...
                        if ($sl_count == 0) {
                            ...
                            $check_clearance = ClassRegistry::init('Clearance')->find('count', array(
                                'conditions' => array(
                                    ...
                                    'Clearance.request_date >= ' => $this->AcademicYear->getAcademicYearBegainingDate(
                                        $this->request->data['Readmission']['academic_year'],
                                        $this->request->data['Readmission']['semester']
                                    )
                                )
            ...
            if (empty($readmission_list)) {
                $this->Flash->error(
                    'You are required to select at least one student to be included in the readmission list.'
                );
            } else {
                if ($this->Readmission->saveAll($readmission_list, array('validate' => false))) {
                    $this->Flash->success(
                        count(
                            $readmission_list
                        ) . ' students are included in the readmission list. After registrar filtering and academic commission approval, the student will be readmitted.'
                    );
                    if (!empty($clearance_list_by_readmission['Clearance'])) {
                        ...
                        debug(
                            $this->Readmission->Student->Clearance->saveAll(
                                $clearance_list_by_readmission['Clearance'],
                    ...
                    $this->Flash->error(
                        'The system unable to include the selected students in the readmission list. Please try again.'
                    );
                    //return $this->redirect(array('action' => 'apply_readmission_for_student'));
                }
            }
    ...
    public function ajax_readmitted_year($student_id)
    {
        ...
        $student_detail = $this->Readmission->Student->find(
            'first',
        ...
        if (!empty($possibleAcademicYears)) {
            foreach ($possibleAcademicYears as $Y) {
                $wasReadmitted = $this->Readmission->find(
                    'first',
                    array(
                        'conditions' => array(
                            'Readmission.student_id' => $student_id,
                            'Readmission.academic_year' => $Y
                        ),
                        'recursive' => -1
                    )
                );
    ...
    public function readmission_data_entry()
    {
                ...
                if (!empty($this->request->data['Readmission'])) {
                    foreach ($this->request->data['Readmission'] as $key => $readmission) {
                        if ($readmission['gp'] == 1 && !empty($readmission['semester'])) {
                            ...
                            $readmissionApplication['Readmission'][$count]['registrar_approved_by'] = $this->Auth->user(
                                'id'
                            );
                            $readmissionApplication['Readmission'][$count]['academic_commision_approval'] = 1;
                            $readmissionApplication['Readmission'][$count]['academic_commission_approved_by'] = $this->Auth->user(
                                'id'
                            );
                            $readmissionApplication['Readmission'][$count]['academic_commission_approval_date'] = date(
                                'Y-m-d'
                            );
        ...
        $this->redirect(
            array(
                ...
                $this->request->data['Search']['student_id']
            )
        );
    }
}
